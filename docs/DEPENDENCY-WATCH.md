...
| shadcn/ui çalışma zamanı: `@radix-ui/react-*` · `class-variance-authority` · `tailwind-merge` · `clsx` | ~~1.1.23 (react-dialog örnek)~~ → **6.4'te KURULANLAR:** `react-select` **2.3.7** · `react-checkbox` **1.3.11** · `react-radio-group` **1.4.7** · `react-slider` **1.4.7** · `react-switch` **1.3.7** · `react-tabs` **1.1.21** · `react-popover` **1.1.23** · `clsx` **2.1.1** · `tailwind-merge` **3.6.0** · 🆕 **6.5'te EKLENEN BEŞ İLKEL** (sürümler `package.json`dan okundu, kopyalanmadı): `react-dialog` **1.1.23** (Dialog **ve** Sheet) · `react-tooltip` **1.2.16** · `react-toast` **1.2.23** · `react-avatar` **1.2.6** · `react-progress` **1.1.16** · ⚠️ `class-variance-authority` **KURULDU, KULLANILMADI, KALDIRILDI** (6.4: hiçbir bileşen `cva()` çağırmıyor — varyant haritası `Record<Variant, string>` ile **tip seviyesinde kapsayıcı**, cva bunu zayıflatırdı; SAPMA-026) · ⚠️ **`cmdk` HİÇ KURULMADI** (6.0 onu bu satıra yazmamıştı, yani sürümü/peer'ları **doğrulanmamış**; Combobox onun yerine ARIA desenini elde uyguluyor) | ~~Faz 6~~ ✅ **6.4 + 6.5'te KURULDU** — 12 Radix ilkeli. Kalan bileşenler (6.6 alan-özel, 6.7 DataTable) yeni ilkel **gerektirmiyor**: TanStack satırları ayrı | ⚠️ **BU SATIR HİÇ YOKTU.** shadcn/ui bir **paket değil**, bileşenleri kopyalayan bir CLI — bu yüzden takip edilecek şey onun **çalışma zamanı bağımlılıkları**. Radix **ilkel başına ayrı paket** yayınlıyor (`@radix-ui/react-dialog`, `-popover`, `-tooltip`…), yani kurulacak paket sayısı 6.4/6.5'te hangi ilkellerin kullanıldığına bağlı ve **o gün sayılır**. ⚠️ Radix `jsdom`da doldurma isteyecek — aşağıdaki `jsdom` satırına bak. |
