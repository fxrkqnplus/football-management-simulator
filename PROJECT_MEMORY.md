...
| | |
|---|---|
| **Aktif faz / alt görev** | **FAZ 4 — Veritabanı Şeması II.** Liste ROADMAP'te (`8203ac1`, K11). **4.0 · 4.0b · 4.1 · 4.2 kapandı.** ⏸️ **SIRADAKİ: 4.3** — `people` + `players` + migration `0005`. Fazın **ilk migration**'ı; `down` + round-trip `it()` + ER sabitleri + **`SET NULL` DAVRANIŞ testi** birlikte gelir |
| **Son tamamlanan** | ✅ **4.2 — `fk-policy.ts` V3'e genişletildi. Fazın ilk KOD alt görevi, migration yok.** Üçüncü olgu `allSourceColumnsNullable` eklendi, `SET NULL` üretiliyor, sıra `① dictionary → ② independent → ③ nullable → ④ satellite`. **Regresyon: Faz 3'ün 12 FK'sı 12/12.** İki zorunlu mutasyon: sıra bozulunca **8 test**, `SET NULL`→`CASCADE` olunca **1 test** kırıldı. 🆕 **Nullability tek tanıma indirildi** (`schema-state/foreign-key-nullability.ts`) — ama **tek yüklem değil, iki adlandırılmış okuma**: ER *"herhangi biri"*, `ON DELETE` *"hepsi"* soruyor (günlük #8) |
...
| **CI — liste sorgusuyla** | ✅ **4.1'in koşusu İŞLENDİ: `33252480626`** (`c0e335b`, `push`) → **6/6 iş yeşil, amd64 + arm64**. Dalda **3 push koşusu, üçü de yeşil**; `pull_request` olayı yok (PR faz sonunda), iptal edilen koşu yok. **`1c93890`'dan beri 22 ardışık yeşil push koşusu, 0 kırmızı** — 18 (`feature/faz-03-database`) + 1 (`develop` merge) + 3 (bu dal). Elle artırılmadı |
...
| **Kapı kapsamı — ÖLÇÜLDÜ, varsayılmadı** | 🆕 **4.2'de kapılar GERÇEKTEN BAKTI** — 4.0/4.0b/4.1 yalnızca Markdown değiştirmişti. `format:check` bu kez **reddetti** (`fk-policy.test.ts`), `lint` **iki hata** buldu (`simple-import-sort` × 2), `typecheck` **TS2678** verdi (②'nin erken dönüşü `independent`i daralttığı için `case 'independent'` ulaşılamaz). ⚠️ **TS2678 ücretsiz bir nöbetçi çıktı:** sıranın bağlayıcılığı yalnızca yorumda değil **tip seviyesinde** de görünür — ②'yi ③'ün altına taşımak derlemeyi kırıyor |
...
| **test** | ✅ **761 test / 53 dosya** (744/52'den; +17 test, +1 dosya `foreign-key-nullability.test.ts`) · ✅ **`pnpm test:db` 163 test / 8 dosya** — sayı **değişmedi**, çünkü 4.2 yeni bir entegrasyon vakası açmadı: **canlı şemada `SET NULL` alan TEK BİR FK YOK** (üç nullable FK'nın üçü de bağımsız kaynaklı → ②'de RESTRICT). ⚠️ Yani `SET NULL` dalı **entegrasyonda bugün erişilmiyor**; ilk gerçek vaka **4.3'ün `players.club_id`'si** ve o boşluk koşan bir testle görünür tutuldu |
| **kapsam** | ✅ **DÖRT METRİK DE YÜKSELDİ** — satır %86,99 → **%87,09** · ifade %87,04 → **%87,15** · dal %88,68 → **%88,78** · fonksiyon %80,00 → **%80,16**. Eşik %70, DÜŞÜRÜLMEDİ, dosya dışlanmadı. `fk-policy.ts` ve `foreign-key-nullability.ts` saf ve tam test edilmiş; `er-diagram.ts` **%100 fonksiyon · %100 satır**. ⚠️ Kapsam **4.3'ten itibaren DÜŞECEK** (şema dosyaları paydaya %0 ile girer) ve bu **beklenen** |
| **Şema durumu** | **11 / 11 master tablo** + **4 indeks** + **12 FK** — **4.0 şemaya DOKUNMADI**. Migration zinciri 5 dosya (`0000`…`0004`), **beşinin de elle yazılmış `down`u var** (ölçüldü: `drizzle/*.sql` **5**, `drizzle/down/*.sql` **5**). `comparedFacts` alt sınırı **1.627** (`COMPARED_FACTS_FLOOR`) |
| **Veri durumu** | **2 / 11 tablo doludur** (`countries` 6, `competitions` 11). ℹ️ Faz 4'ün hafıza boşluğu kapandı: 3.8'in seed'i `source` sütununa **`procedural`** yazdı; gerekçe `tools/data-cli/src/seed/world-seed-data.ts` başlığında ve G-14'te — `PROJECT_MEMORY.md` bunu taşımıyordu |
...
| **Faz 4 kabul kriterleri** | **0 / 6** — 4.0/4.0b/4.1 kod yazmadı. Kriter listesi **iki kez değişti**: 4.0'da nitelik CHECK'i (SAPMA-028), 4.1'de kriter 3 daraldı — *"20–24 yaş, sağ bek, CA>120"* **5.000 oyuncu hacminde** < 50 ms; `değer<15M` yüklemi **Faz 32**'nin kabul kriterine taşındı (SAPMA-031) |
| **⚠️ Faz 4'ün ilk günü — SQL'den ÖNCE iki ölçüm** | ① **Tablo envanteri çelişkisi:** ROADMAP 19 tablo sayıyor; `spec/01` §3.1'de bunların **11'i** var, **3'ü §3.2 save katmanında** (`contracts`·`contract_clauses`·`injuries`), **7'si hiç yok**. SAPMA-021'in üç katı. ② **`fk-policy.ts` Faz 4'ün FK'larında yanlış cevap üretecek:** `people` `key` taşımıyor + giden FK'sı var → kural onu **uydu** sayar → ondan çıkan her FK **CASCADE**; oysa `players.club_id` `spec/01`'de *"`null` = serbest oyuncu"* diye yazılı, yani doğru davranış **`SET NULL`** — ve `expectedDeleteAction` bugün `SET NULL` **döndüremiyor**. ①②'nin ölçümü bu oturumda sunuldu, **kararları onayla birlikte ROADMAP'e yazılacak** |
