...
| | |
|---|---|
| **Aktif faz / alt görev** | **FAZ 4 — Veritabanı Şeması II.** Liste ROADMAP'te (`8203ac1`, K11). **FAZ 4a (4.0 → 4.7) KAPANDI**, **FAZ 4b açık**. ⏸️ **SIRADAKİ: 4.9** — **5.000 sahte oyuncu seed'i** + determinizm ölçümü (iki koşu birebir aynı; `created_at`/`updated_at` **gürültülü** dışlanır). ⚠️ `clubs` boş olduğu için **5.000'in 5.000'i serbest oyuncu** — bilinçli, ve 4.10'a not bırakılır. 🆕 **4.9 kapsamı GERİ KAZANDIRACAK:** 4.8'in ölçümü gösterdi ki şema değişikliği tek başına payı sabit bırakıp paydayı büyütüyor; kapsanabilir kod (3.8'de `seed-sql.ts` 38/38'di) payı yükseltiyor |
| **Son tamamlanan** | ✅ **4.8 — transfer arama indeksleri (`0011`): `players (primary_position, current_ability)` + `people (birth_date)`.** Tablo **22'de sabit**, FK **32'de sabit** — `0011` şemaya **sütun eklemiyor**, yalnızca iki `CREATE INDEX`. 🆕 **PROMPTUN BİR NOTU ÖLÇÜMLE ÇÜRÜTÜLDÜ (D7, ikinci vaka):** prompt *"CA>120 → `player_attributes`"* ve *"üç tablo, iki JOIN"* diyordu; `current_ability` **`players`**ta (`players.ts:188`), `player_attributes`ta **0 eşleşme** → **iki tablo, bir JOIN**. Fark belirleyiciydi: üç tablo olsaydı bileşik indeks imkânsızdı. Ve `spec/01` §3.1'in 4.1 kararı (satır 700–705) zaten bunu yazıyordu. 🆕 **YAŞ İFADESİ İNDEKSLENEMİYOR — ölçüldü:** `age(timestamptz)` ve `now()` **`s` (STABLE)**, PG yaş ifadesini indekste **reddediyor**, `IMMUTABLE` bir ifade **kabul** ediliyor (iki yönlü). ⚠️ **3.7'nin `immutable_unaccent` çıkışı GEÇERSİZ:** orada iddia *"nadiren yanlış"*tı (bedeli `REINDEX`), yaş **her gün** değişir. Aralık sorgu tarafında sabit tarihlere çevriliyor (`ageRangeToBirthDateRange`, tek modül) — `search.ts` deseni **kopyalanmadı, uyarlandı** ve fark dosya başlığında yazılı. Bileşik indeksin **sırası** eşitlik → aralık kuralından türetildi (ters sırada mevki eşitliği indeksin arama sınırına giremezdi) |
| **⚠️ 4.8'İN EN PAHALI BULGUSU — `0011` KAYBOLMUŞ BİR SINIFI GERİ GETİRDİ (SIFIR KAYIP)** | Günlük #23: 4.5 `0008`i *"`loss.ts`in dördüncü sınıfı: sıfır kayıp, zincirde ilk"* diye ölçmüştü; `0009` eklenince o sınıf **erişilemez** oldu ve bir `fixtureChain` ile korundu. `loss.ts` yalnızca `'table'` ve `'column'` sayıyor (ölçüldü) — bir indeksin düşmesi **ikisi de değil**, yani `0011`in geri alınması `allowDataLoss` **verilmeden** geçiyor ve sınıf **gerçek zincirde** yeniden ölçülebilir. ⚠️ Test bunu **bayraksız** yazarak iddia ediyor: `allowDataLoss: true` yazılsaydı aynı sonuç çıkardı ve **hiçbir şey kanıtlamazdı** (koruma ısırmadığında da geçerdi). **Genel biçim: #23 bir sınıfın kaybolabileceğini gösterdi; bu vaka kaybolan bir sınıfın GERİ DÖNEBİLECEĞİNİ gösteriyor — ve dönüş de kayıp kadar sessiz** |
...
| **4.8'in commit'leri** | **`9d848fa`** (içerik: `transfer-search.ts` + testi, iki `index(...)`, `0011` + elle `down`, dört test dosyası, ROADMAP, `spec/09` §11.5) · **`<bu commit>`** (kayıt: ANLIK DURUM, günlük #32–#33, rapor arşivi) |
...
| **⚠️ CI — 4.7'NİN KOŞUSU KIRMIZI VE SERİ KIRILDI** | **`33419337117`** (`0459fa5`, `push`) → **`conclusion: failure`**. Altı işin **beşi yeşil** (Kalite kapıları ×2, Entegrasyon ×2, İmaj arm64); kırılan tek iş **İmaj (amd64)**. Log okundu: `docker.io/library/caddy:2-alpine` metadata çekilirken Docker Hub auth token isteği **16 dakika** sonra `connection timed out`. **Kod değişikliğiyle ilgisi yok** — aynı commit'te arm64 imajı başarılı. ⚠️ **AMA BU BİR İPTAL DEĞİL, BİR BAŞARISIZLIK:** *"iptal ne yeşil ne kırmızıdır"* kuralı burada **geçerli değil**. `1c93890`'dan beri süren **27 ardışık yeşil** seri bu koşuda **sonlandı**. Sayım 4.11'de yeniden işlenir. Koşu **yeniden denenmedi** — dışa dönük bir eylem, kullanıcının kararı. ⏳ 4.8'in koşusu yazım anında bilinmiyor |
...
| **typecheck / lint / format / build / arch** | ✅ typecheck **10/10** ve bu kez **8 cache miss** (yani gerçekten baktı — önbellekten gelen bir onay değil, SAPMA-024) · lint 0 (ilk koşuda **5 hata** — `restrict-template-expressions`; `eslint-disable` ile susturulmadı, `String()` ile **yeniden yazıldı**, 4.6'nın `no-control-regex` kararının aynısı) · **format 0** (ilk koşuda 2 dosya, `prettier --write`) · **build 8/8 SOĞUK** (`.turbo/cache` silindi, `Cached: 0` **doğrulandı**, `pwd` = `/c/fms` önceden denetlendi — #31 dördüncü kez ısırmadı, turbo **9,64 s**) · **arch 9 kural** temiz (302 ms) |
...
| **Kapı kapsamı — ÖLÇÜLDÜ, varsayılmadı** | ⚠️ **4.8'de DE asıl doğrulayıcı kapı değildi — ALTINCI tekrar.** `typecheck` iki yeni indeksi ve yeni bir modülü hiçbir şey söylemeden geçirdi; **8 cache miss** olduğu için *"bakacak bir şey yoktu"* bahanesi de yok. Doğrulayan şeyler: **üretilen migration SQL'i** (iki `CREATE INDEX`, sütun sırası okundu), **katalog** (`pg_indexes.indexdef`, `pg_index.indisunique`), ve **gerçek PG'nin reddi** (yaş ifadesi `IMMUTABLE` hatası). 🆕 **Ve `pnpm test:db` yine tek başına İKİ bayat envanter yakaladı:** `search-index.itest.ts`in *"dört indeksin dördü de yaratıldı"* listesi (`LIKE '%_idx'` şemanın tamamını tarıyor) ve `world-seed.itest.ts`in zincir uzunluğu (11 → 12). Hiçbir statik kapı ikisini de göremezdi |
...
| **D5 — derlenmiş `dist`, düz `node`, AYRI gerçek veritabanı** | ✅ **23 / 23.** Vitest yok, testcontainers yok; `fms_d5_48` yaratıldı, zincirin **on iki adımı** uygulandı, sonunda düşürüldü. Ölçülenler: 22 tablo · 32 FK (**ikisi de sabit**) · **altı indeks** katalogdan (3.7'nin dördü + 4.8'in ikisi) · bileşik indeksin tanımı `btree (primary_position, current_ability)` — **sıra okundu** · ikisi de UNIQUE **değil**, **KARŞI KONTROL** `rivalries_pair_unique_idx` `true` · `now()` **`s`** · yaş ifadesi **reddedildi** (`IMMUTABLE`), **KARŞI ÖRNEK** `IMMUTABLE` ifade **kabul edildi** · derlenmiş `ageRangeToBirthDateRange` üç vaka (20–24 aralığı + iki artık yıl kelepçesi) · 🆕 **KRİTER 3'ÜN SORGUSU GERÇEK VERİYLE KOŞTU:** altı oyuncu yazıldı, sorgu **tam üçünü** döndürdü — iki **sınır** dahil (tam 20 ve tam 24), üç eleme doğru (tam 25 · yanlış mevki · CA=120). Bu bir **plan** iddiası değil bir **doğruluk** iddiası: fonksiyon doğru olup SQL'de yanlış kullanılabilirdi · `0011` geri alma **bayraksız** geçti (SIFIR kayıp), iki indeks düştü, 3.7'nin dördü **durdu**, 22 tablo ve 6 satır **durdu** · çevrim `identical`, `comparedFacts` **4209**. ⚠️ İki imza hatası gürültülü patladı (günlük #32, #33) |
...
| **install** | ✅ **4.8 DE YENİ BAĞIMLILIK EKLEMEDİ** — `pnpm-lock.yaml` ve `pnpm-workspace.yaml` **diff yok** (`git status` ile doğrulandı). 🆕 **VE BİR BAĞIMLILIK EKLENMESİ BİLİNÇLİ OLARAK REDDEDİLDİ:** `date-fns` kurulu **değil** (ölçüldü, 0 eşleşme) ve yaş→tarih çevrimi için kurulmadı — çevrim saf `Date.UTC` ile yazıldı, artık yıl kelepçesi **elle** ve üç testle sabitlendi (K12: 4.8'in kapsamı indeks, bağımlılık kararı değil) |
...
| **test** | ✅ **887 test / 64 dosya** (868/63'ten **+19 / +1**): tek yeni dosya `transfer-search.test.ts` — indeks envanteri (adlar şema tanımından `getTableConfig` ile okunuyor) + bileşik indeksin **sütun sırası** + çevrimin sınırları (iki uç **dahil**, tek yaşlık pencere, `minAge=0`) + **üç artık yıl vakası** + altı **REDDEDİLEN** girdi (bozuk biçim, negatif yaş, `min > max`, takvimde olmayan gün) + **KARŞI ÖRNEK** (gerçek artık gün kabul). 🆕 **KARŞI KONTROL yazıldı (D3):** `getTableConfig().indexes` okuma yolunun gerçekten çalıştığı `clubs`ın 3.7 indeksleriyle gösterildi — yoksa `config.name` her zaman `undefined` dönse bile iddialar *"boş listeye bakıyorum"* diye okunabilirdi. ⚠️ Ve `config.name` `string \| undefined` çıktı: `undefined`lar **filtrelenmedi**, `(adsız)` diye görünür kılındı · ✅ **`pnpm test:db` 251 test / 8 dosya** (241'den **+10**): `0011` çevrimi (saf `CREATE INDEX`, iki indeksin gerçekten düştüğü ayrıca ölçülüyor) · **SIFIR kayıp bayraksız** · bozulma tablosuna **iki `DROP INDEX`** · iki indeks tanımı katalogdan (**sıra** dahil) · **NEGATİF** yaş ifadesi reddi · **KARŞI ÖRNEK** `IMMUTABLE` ifade kabulü · UNIQUE olmama + **KARŞI KONTROL** `rivalries` |
...
| **kapsam** | ⬆️ **FONKSİYON DÜŞMEDİ, YÜKSELDİ — VE SEBEP İZOLE EDİLEREK ÖLÇÜLDÜ** — satır %85,86 → **%86,77** · ifade %85,76 → **%86,76** · fonksiyon %73,60 → **%75,81** · dal %88,64 → **%88,92**. Seri: 4.5 %77,68 → 4.6 %75,91 → 4.7 %73,60 → **4.8 %75,81** (dört turdur ilk yükseliş). Toplam 290/394 → **301/397**. 🆕 **Sebep tahmin edilmedi, İKİ YÖNLÜ ölçüldü** — yeni test dosyası geçici olarak devre dışı bırakılıp kapsam yeniden koşuldu: **test YOKken 290/397 = %73,05** (yani şema değişikliği tek başına paydayı +3 büyütüp payı sabit bırakıyor → **−0,55 düşüş**), **testle 301/397 = %75,81** (**+2,21 yükseliş**). Yükselişi getiren şey testin **`getTableConfig()` çağırması**: bu çağrı `players`/`people`/`clubs` dosyalarındaki `(table) => [...]` ve `.references(() => …)` ok fonksiyonlarını **çalıştırıyor** — 4.7'de `staff.ts` 0/5'ti çünkü hiçbir test onları çağırmıyordu. ⚠️ **Eşik %70 DÜŞÜRÜLMEDİ, dosya dışlanmadı, import testi yazılmadı.** Marj fonksiyonda **5,81 puan** (3,60'tan **genişledi**) |
| **kapsam (4.7 arşiv)** | ⬇️ **FONKSİYON YİNE DÜŞTÜ, SEBEBİ DOSYA BAZINDA ÖLÇÜLDÜ** — satır %86,49 → **%85,86** · ifade %86,47 → **%85,76** · fonksiyon %75,91 → **%73,60** · dal %88,64 → **%88,64** (değişmedi). Seri: 4.4 %77,56 → 4.5 %77,68 → 4.6 %75,91 → **4.7 %73,60**. **Sebep `coverage-summary.json`dan okundu:** dört yeni şema dosyası **12 kapsanmayan fonksiyon** getiriyor — `staff.ts` **0/5**, `managers.ts` **0/5**, `staff-attributes.ts` **0/1**, `manager-attributes.ts` **0/1**. Hepsi ok fonksiyonu: `.references(() => …)`, `(table) => [...]` ve yerel `literals`. Toplam **290/394**. ℹ️ Tahmin −1,5…−2 puandı, gerçek **−2,31** — `staff.ts` beklenenden bir fonksiyon fazla getirdi (`literals` yardımcısı sayılmamıştı). ⚠️ **Eşik %70 DÜŞÜRÜLMEDİ, dosya dışlanmadı, import testi yazılmadı.** Marj fonksiyonda **3,60 puan** (5,91'den daraldı) — 4.9'un seed'i geri kazandıracak |
...
| **Şema durumu (4.6 arşiv)** | **18 master tablo** + **4 indeks** + **26 FK** (15/21 → 18/26). Migration zinciri **10 dosya** (`0000`…`0009`), **onunun da elle yazılmış `down`u var**. 🆕 **Fazın ilk BİLEŞİK PK'leri** (`(player_id, position)` / `(player_id, trait_code)`) ve ilk **1:N** tabloları — 1:1 ayracı burada **soru bile değil** ve bu `spec/01`'den okunarak söylendi, devir notundan alınmadı (D7). `player_stats_history` `serial id` taşıyor çünkü doğal anahtar adayının bir bileşeni (`club_id`) **nullable**. 🆕 **§3.1.2 ⑤'in simetrik sonucu tekrar doğrulandı:** `0009` saf `CREATE TABLE` → çevriminde `identical: true` **beklenir** ve ölçüldü; zincirdeki `0006` bu testi **etkilemiyor** (kayma bir geri alma derinliği özelliği). `comparedFacts` alt sınırı **3.023 → 3.570** — **ÖLÇÜLDÜ** (sınır `9_999_999`a kondu, gerçek sayı testin reddettiği çıktıdan okundu). Artış **+547**, 4.5'in +780'inden **küçük**: 42 yeni sütun (63'e karşı), buna karşılık üç PK, beş FK, iki CHECK. ⏳ Faz 4'ün kalan **dört** master tablosu 4.7'de |
| **Veri durumu** | **2 / 22 tablo doludur** (`countries` 6, `competitions` 11); kalan **yirmisi boş** — seed **4.9**'da (kriter 1). ⚠️ **VE `players`IN BOŞ OLMASI 4.8'İN İDDİA SINIRINI BELİRLEDİ:** `ANALYZE` görmemiş bir tabloda planlayıcı indeksi seçer ve bu yanlış cevabın doğru görünmesidir (`reltuples = -1`, 3.9) — bu yüzden 4.8 **plan iddiası yazmadı**. ℹ️ 3.8'in seed'i `source` sütununa **`procedural`** yazdı; gerekçe `tools/data-cli/src/seed/world-seed-data.ts` başlığında ve G-14'te |
| **Veri durumu (4.6 arşiv)** | **2 / 18 tablo doluydu** (`countries` 6, `competitions` 11); `people`, `players`, iki nitelik tablosu ve 4.6'nın üç tablosu **boş** — seed **4.9**'da. ℹ️ 3.8'in seed'i `source` sütununa **`procedural`** yazdı; gerekçe `tools/data-cli/src/seed/world-seed-data.ts` başlığında ve G-14'te |
...
| **Açık sorun sayısı** | **0** |
| **G-17 — 4.8 MARUZİYETİ BÜYÜTMEDİ** | `0011` yeni bir kimlik sütunu getirmiyor ve `transfer-search.ts` hiçbir kimlikle çalışmıyor (girdisi yaş ve tarih). Maruziyet 4.7'nin bıraktığı yerde: **`people.id` · `players.id` · `staff.id` · `managers.id`**, dördü de `integer`. Tip seviyesinde kapanması **Faz 12** |
...
| **Teknik borç sayısı** | **8** — **4.8 hiçbir borç açmadı ve BORÇ-008'i ödemedi** (vadesi **4.11**, ve bu doğru: `0011` yeni bir CHECK getirmiyor, yani yeni bir kopya da doğmadı — dokuz yerde sabit). BORÇ-001, 002, 004 (Faz 16) · BORÇ-003, 005 (Faz 5) · **BORÇ-007 (Faz 12)** · BORÇ-006 (Faz 50) · **BORÇ-008 (Faz 4.11)** |
...
| **SAPMA sayısı** | **34** (SAPMA-001…034) — **4.8 de hiçbir SAPMA açmadı.** Aday değerlendirildi ve **sapma çıkmadı**, ve gerekçesi 4.7'nin adayıyla **aynı sınıf**: promptun *"CA `player_attributes`ta, üç tablo"* notu yanlıştı ama o bir **prompt** hatası, bir spesifikasyon sapması değil — `spec/01` §3.1 doğru yeri (satır 700–705, 4.1 kararı) zaten yazıyor ve şema ona **uyuyor**. ⚠️ **`spec/01`'in indeks satırının iki tabloyu karıştırması da yeni bir SAPMA DEĞİL** — SAPMA-031'in kapsamında ve satır 4.1'de bir *"niyet beyanı"* ilan edilerek zaten ele alınmıştı. Faz 4'ün payı **yedi** (028…034) |
...
| **Boşluk (4.6 arşiv)** | 🆕 **4.6 hiçbir boşluk açmadı veya kapatmadı.** İki aday değerlendirildi ve ikisi de **boşluk çıkmadı**: ① *"xG öncesi sezonlarda veri yok"* → ROADMAP Faz 9 istatistik ingestini adıyla sayıyor **ve** bir *"veri kalite raporu: eksik alan yüzdeleri"* istiyor, yani eksik alan orada ele alınıyor (**ölçüldü**, G-18'in dersi: hedef fazın o işi yapabildiği doğrulanır) ② *"`player_stats_history` doğal anahtarında teklik yok"* → `club_id` nullable olduğu için kısmi bir `UNIQUE` D3 yanılsaması üretirdi ve doğru biçimi bugün belirsiz; yeri **Faz 11** ve G-10/G-11/G-12 zaten o bloğu taşıyor. 🆕 4.6 ayrıca **`player_traits` için bir sözlük tablosu AÇMADI** (K12) — kümeyi üreten faz **Faz 10**, ve hiçbir şeyin yazmadığı bir tablo D3 sınıfı (`injury_types`in Faz 12'ye taşınma gerekçesi). **Önceki tur (4.5): birini KAPATTI, birini AÇTI.** ✅ **G-18 kapandı** (`0008`, `PERSON_TYPES` 4 → 5): seçenek ① uygulandı, ③ doğal sonucu olarak aynı alt görevde yapıldı, ② reddedildi. ⚠️ **Ve 4.4'ün ATAMASI GERİ ALINDI — dayanağı D7'ydi:** *"hakem verisi Faz 8'de geliyor"* gerekçesi **3.8'in kendi notuna** yaslanıyordu; Faz 8'in gerçek ingest listesi ölçüldü ve hakem **yok**, yani oraya yazılan kabul kriteri orada **karşılanamazdı**. Blok ve kriter Faz 8'den **kaldırıldı** (kütük satırı **silinmedi**, Kural 2). 🆕 **G-19 açıldı** — *"hiçbir faz hakem verisini ingest etmiyor"*; ROADMAP'in tüm hakem atıfları fazlarına göre çıkarıldı (23/26/29/45 **tüketici**, 46 **bakım**, 8 ve 9'da **yok**). **Sahip tahminle atanmadı** (K13): karar noktası **Faz 7**'ye yazıldı ve bir **kabul kriteri** getirdi |
...
| **Faz 4 kabul kriterleri** | **3 / 6 — 4.8 de hiçbirini kapatmadı ve bu BEKLENEN.** 4.8 **kriter 3'ün hazırlığı**; kriterin kendisi *"5.000 oyuncu hacminde < 50 ms"* diyor ve o hacim **4.9**'da geliyor, ölçüm **4.10**'da. ⚠️ **VE BUGÜN BİR PLAN İDDİASI YAZILMADI — bu bilinçli:** `players` boş, `ANALYZE` yapılmamış bir tabloda planlayıcı indeksi seçer ve bu **yanlış cevabın doğru görünmesidir** (`reltuples = -1`, 3.9). 4.8'in dürüst iddiası: *"indeksler yaratıldı, tanımları katalogda doğrulandı, ve çevrimin ürettiği aralık gerçek veride doğru satırları getiriyor."* Planlayıcı davranışı 4.10'un işi. `docs/schema/world.md` bu turda **yeniden üretilmedi** ve gerekmedi — ER indeks çizmiyor (3.10'un kararı), `er-diagram.itest.ts` **geçti**, blok bayatlamadı. Kriter 6 hâlâ `[ ]`. **Yapılmayan "yapıldı" yazılmaz** |
