...
| | |
|---|---|
| **Aktif faz / alt görev** | ▶️ **FAZ 6 — Tasarım Sistemi.** **6.0 ✅ · 6.1 ✅ · 6.2 ✅ · 6.3 ✅ · 6.3b ✅.** Sıradaki: **6.4 — shadcn/ui + temel bileşenler I**. §0.5 kontrol noktası **6.5 sonunda**; çizgi **6a = 6.0–6.5 / 6b = 6.6–6.12** |
| **Son tamamlanan** | ✅ **6.3b — BORÇ-011 + açık temanın 12 token'ı.** ⓪ **Sıra dışı iş:** 6.3'ün CI koşusu **kırmızıydı** — imaj `@fms/ui`yi çözemiyordu çünkü Dockerfile paketleri **elle sayıyordu**; `pnpm --filter @fms/web... build` ile bildirimsel hâle getirildi (aynısı `apps/api` için de). ① **BORÇ-011 ödendi** (`resolve.alias`), **iki yönlü** kanıtlandı. ② **12 token yazıldı: 0 türetildi, 12 yazıldı** |
| **Türetme neden çürüdü** | İki kural önerildi, ikisi de **spec'in kendi sekiz değerine** karşı sınandı ve düştü: *"oranı koru"* → `--text-secondary` **−2,409**; *"ΔL\* işaretini aileden al"* → `--bg-hover` **−9,68**. Sebep: koyuda yükselti ve etkileşim **aynı yönde**, açıkta **ters** — açık tema bir dönüşüm değil, **farklı yapıda bir palet**. İkisi bir **spec eşitliğini** koruyor, üçü gri rampayı sürdürüyor, **yedisi `deriveLightChromatic()`in artefaktı** (kural kodda, hexler onun çıktısı, test yeniden üretip karşılaştırıyor) |
| **Tarih / ilerleme / dallar** | 2026-09-05 · **5 / 50 faz (%10)** · `main` → `develop` (`2ec1548`) → **`feature/faz-06-design-system`** (taban `2ec1548`, **8 commit**) |
| **CI** | ⚠️ **6.3'ün koşusu (`33937132528`) KIRMIZIYDI ve sebebi 6.3b'de düzeltildi** — imaj derlemesi. ⚠️ Faz 4'ün kırmızı koşusu (`33419337117`) **hâlâ yeniden denenmedi**, kullanıcının kararı. ⏳ 6.3b'nin koşusu yazım anında bilinmiyor |
| **Kapılar** | ✅ typecheck **11/11 SOĞUK** · build **8/8 SOĞUK** · **D5: İMAJ derlendi ve KOŞTURULDU** — JS 200 · CSS 200 · kök 404 · `[data-theme=light]` **20 token** · 13 `@font-face` · lint **0** · format **0** · arch **9 kural** · **test 1290/85** (+15) · **test:db 301/10** · **gaps 20·3·17·0 ✗** · **i18n temiz, 335 dosya** |
| **Kapsam** | fonksiyon **%82,62 (409/495)** — yine yükseldi (6.3'te %82,17 / 401-488). Marj `409/0,70 = 584` → boşluk **84 → 89**. Eşik düşürülmedi, dosya dışlanmadı |
| **Mutasyon 3/3 — biri DEAD CODE buldu** | ① türetmenin ikinci kısıtı → **0 kırıldı**; üç ihtimalden **üçüncüsü**: `--text-inverse` = `--bg-base` = kapı yüzeyi olduğu için iki kısıt **birebir aynı hesap**. Kaldırıldı, garanti denetim testinde **ayrı vaka** olarak duruyor ② yazılan bir değer → **2** ③ koyulaştırma kuralı → **1** |
| **Paket / ortam** | Üretim paketi **383.654 → 383.976 bayt** (+322, **%0,08**); CSS 16.821 → **17.064**. §11.6 bütçesi taban × 1,10 — bol marj. ⚠️ İki kez **ölçüm aracı** yanılttı: MSYS yol dönüşümü ve `grep -c`nin satır sayması (günlük #10, #11) |
| **Kütükler** | Açık sorun **0** · teknik borç **11 satır, 5 ÖDENDİ → açık 6** (**BORÇ-011 kapandı**; **009 → 6.11**) · SAPMA **43** · boşluk **20**, açık **17** · günlük **11 satır** |
