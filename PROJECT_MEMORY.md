...
| | |
|---|---|
| **Aktif faz / alt görev** | ▶️ **FAZ 6 — Tasarım Sistemi ve Bileşen Kütüphanesi.** **6.0 ✅ · 6.1 ✅ · 6.2 ✅.** Sıradaki: **6.3 — tema + Tailwind 4 + fontlar** (ilk paket kurulumu, `tsconfig`e `jsx` + React tipleri). §0.5 kontrol noktası **6.5 sonunda**; bölünme çizgisi **6a = 6.0–6.5 / 6b = 6.6–6.12** |
| **Son tamamlanan** | ✅ **6.2 — tasarım token'ları.** `packages/ui` ilk içeriğini aldı: beş modül + altı test dosyası, **96 test**. Paket hâlâ **saf TypeScript** (JSX yok, React yok — 6.3'ün işi). 🆕 **SAPMA-043:** 6.1'in yazdığı kriter 5, spec'in **çözmediği** bir şeyi istiyordu (CVD simülasyonu); §7.2'nin mekanizması **ağırlık + desen**. Kriter **spec'e hizalandı**, spec genişletilmedi. 🆕 **Açık tema spec'te EKSİK: 20 token'ın yalnızca 8'i tanımlı** — 12'si uydurulmadı, **liste** olarak yaşıyor. 🆕 **Mutasyon 5/5** |
| **6.2'nin üç ölçümü** | ① Doğrusallaştırma eşiği (`0.03928` ↔ `0.04045`) 8 bitlik **hiçbir** renkte fark üretemiyor — **256 kanal değerinin hepsinde** kanıtlandı, karşı kontrolüyle ② Komşu bant kontrastları **1,04 – 1,71** → parlaklık tek başına ayırmıyor, spec'in yedekli kodlaması **mekanizmanın kendisi** ③ Rozet metni için **tek renk yetmiyor**; iki adaydan hesapla seçilince sekiz bandın sekizi de AA (**4,56 – 8,17**) |
| **Tarih / ilerleme / dallar** | 2026-09-04 · **5 / 50 faz (%10)** · `main` → `develop` (`2ec1548`) → **`feature/faz-06-design-system`** (taban `2ec1548`, **4 commit**) |
| **CI** | ✅ Dalda **2 push koşusu, 2'si de success** — sonuncusu `33888291301` (`df2245a`). ⚠️ **Faz 4'ün KIRMIZI koşusu (`33419337117`, İmaj/amd64, Docker Hub auth — kod dışı) HÂLÂ YENİDEN DENENMEDİ**, kullanıcının kararı. ⏳ 6.1 ve 6.2'nin koşuları yazım anında bilinmiyor |
| **Kapılar** | ✅ typecheck **10/10 SOĞUK** (`Cached: 0`) · build **8/8 SOĞUK** (`Cached: 0`, ayrı silme) · **D5: `dist` import edilip ÇALIŞTIRILDI**, test dosyası dist'e sızmadı (0) · lint **0** · format **0** ⚠️ `.md`ye bakmıyor · arch **9 kural** · **test 1223/81** (+96 / +6) · **test:db 301/10** · **gaps 20·3·17·0 ✗** · **i18n temiz, 322 dosya** |
| **Kapsam** | fonksiyon **%81,29 (378/465)** — **yükseldi** (6.1'de %80,70 / 364-451). Payda **+14**, pay **+14**: token'lar veri + saf fonksiyon. Marj `378/0,70 = 540` → boşluk **69 → 75**. Eşik düşürülmedi, dosya dışlanmadı. Tek kapsanmayan satır `attribute-scale.ts`in **erişilemez** savunma dalı (tip sistemi istiyor) |
| **Açık iki kusur (ölçüldü, gevşetilmedi)** | ⚠️ `--text-muted` **iki temada da AA altında** (koyu 3,27–3,83 · açık 2,86–3,07; 3:1 üstünde) ⚠️ koyu `--accent` beyaz zeminde **2,31** — açık tema eksikliğinin neden eksiklik olduğunun ölçümü. İkisi de teste **beklenen sonuç** olarak yazıldı; sahipleri **6.8** ve **Faz 49** |
| **Ortam** | PostgreSQL **18.6** · Node **24.19.0** · pnpm **11.23.0** · TS `~6.0.3` · jsdom **30.0.1** (`matchMedia` yok · `var()` çözülmüyor · rect 0×0) · Üretim paketi **380.908 bayt** (§11.6 tabanı, `perf:budget` 6.10'da) |
| **Kütükler** | Açık sorun **0** · teknik borç **10 satır, 4 ÖDENDİ → açık 6** (**009 → 6.11**) · **SAPMA 43** (043 bu alt görevde) · boşluk **20**, açık **17** · günlük **4 satır** (#3 lint, #4 **kendi testim kördü**) |
