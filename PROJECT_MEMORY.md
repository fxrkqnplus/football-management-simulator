...
| **4.3 (önceki)** | ✅ `people` + `players` + `0005` — envanter 11 → 13, FK 12 → 16; dört FK'nın dördü de kuraldan tahmin edildiği gibi. `SET NULL` dalının ilk canlı vakası (`players.club_id`), davranışı iki yönlü ölçüldü. `person_type` şemanın **ilk dizi sütunu** ve `introspect`te bir körlük açığa çıkardı (`udt_name` eklendi) |
| **4.5'in commit'leri** | `54e7fe7` (içerik: iki tablo, iki migration, iki kısıt, `'referee'`) · `5c17673` (kayıt: ANLIK DURUM, günlük #15–#19, rapor arşivi) · `f384402` (rapora hash'ler) · **`<bu commit>`** (ANLIK DURUM'a hash'ler). ⚠️ **PROTOKOL İHLALİ VE DÜZELTMESİ:** *"ANLIK DURUM'u yazan commit alt görevin SON commit'i olsun"* kuralı `f384402` ile kırıldı — rapora hash yazmak `5c17673`ten **sonra** geldi. Kuralın gerekçesi (*oturum koparsa `git log` son durumu göstersin*) burada zarar görmedi, çünkü `f384402` yalnızca iki hash dolduruyor. Yine de düzeltildi: hash'ler **ANLIK DURUM'a da** yazıldı ve son commit bu oldu. **Ders: bir raporu hash'leriyle tamamlamak, ANLIK DURUM'dan SONRA gelmek zorunda olan bir adımdır — o hâlde ANLIK DURUM da hash'leri taşımalı ve en sonda yazılmalı.** |
| **Tarih** | 2026-08-30 |
...
| **CI — liste sorgusuyla** | ✅ **4.4'ün koşusu İŞLENDİ: `33285942279`** (`a3caf2d`, `push`, `conclusion: success`). Dalda **6 push koşusu, altısı da yeşil**; `pull_request` olayı yok (PR faz sonunda), **iptal edilen koşu yok** (`status` alanı tek tek okundu — iptal ne yeşil ne kırmızıdır). **`1c93890`'dan beri 25 ardışık yeşil push koşusu, 0 kırmızı** — 18 (`feature/faz-03-database`) + 1 (`develop` merge) + **6** (bu dal). Bu daldaki sayı liste sorgusuyla ölçüldü; diğer iki bileşen 4.3'te ölçülmüştü ve bu turda yeniden ölçülmedi. ⏳ **4.5'in koşusu yazım anında bilinmiyor** |
| **typecheck / lint / format / build / arch** | ✅ typecheck **10/10** · lint 0 (ilk koşuda iki birim testinde **6 hata** — `?.` gereksiz zincirleme ve boolean karşılaştırma; düzeltildi) · **format 0** (ilk koşuda **3 dosya** reddedildi, `prettier --write` ile düzeltildi) · **build 8/8 SOĞUK** (`.turbo/cache` silindi, `Cached: 0` **doğrulandı**, turbo **5,79 s**) · **arch 9 kural** — `rule:` belirteçlerinden **sayıldı**, varsayılmadı |
| **Kapı kapsamı — ÖLÇÜLDÜ, varsayılmadı** | ⚠️ **4.5'te DE asıl doğrulayıcı kapı değildi — ÜÇÜNCÜ tekrar.** `typecheck` 57 yeni sütunu **hiçbir şey söylemeden** geçirdi (Drizzle sütun tanımı tip hatası üretmez). Sütun sayısını ve adlarını doğrulayan şey yine **üretilen migration SQL'i** oldu: `grep -c 'smallint NOT NULL'` → **47** ve **10**. 🆕 **Ama bu turda tip sistemi BİR KEZ ÖTTİ ve ücretsiz bir nöbetçiydi:** `CHAIN_TAGS` sabiti `stepsBackTo`'nun parametre tipini üretiyor, yani `'0007_player_attributes'` yazmak zinciri güncellemeden **derlenmedi** (TS2345, dört çağrı yerinde). 🆕 **Ve `pnpm test:db` yine bir TASARIM SONUCUNU açığa çıkardı:** on altı test `0008`in `down` sınırıyla kırıldı (günlük #17) — hiçbir statik kapı bunu göremezdi |
| **D5 — derlenmiş `dist`, düz `node`, AYRI gerçek veritabanı** | ✅ **17 / 17.** Vitest yok, testcontainers yok; `fms_d5_45` adında ayrı bir veritabanı yaratıldı ve zincirin **dokuz adımı** uygulandı. Ölçülenler: 15 tablo · `player_attributes` **50 sütun** · `player_hidden_attributes` **13** · nitelik sütunlarında **0 CHECK** · `players`ta **üç** CHECK · **NEGATİF** `CA>PA` reddedildi (`players_ca_le_pa_check`) · **NEGATİF** `min>max` reddedildi (`players_pa_range_check`) · **POZİTİF** `CA=PA` ve `min=max` kabul edildi (**sınır dahil** — `<` yazılsaydı ikisi de reddedilirdi) · 1:1 ikinci satır reddedildi · **CASCADE** oyuncu silinince nitelik gitti · `'referee'` kabul, `'coach'` **hâlâ** reddediliyor (kapalılık korundu) · `0008` `down`u `'referee'` varken **patladı** · daraltınca çevrim **birebir aynı** (`facts=3023 diff=0`) |
| **install** | ✅ **4.5 DE YENİ BAĞIMLILIK EKLEMEDİ** — `pnpm-lock.yaml` ve `pnpm-workspace.yaml` **diff yok** (`git status` ile doğrulandı). ℹ️ pnpm 11.24.0 çıkmış; kilit 11.23.0'da, karar verilmedi |
| **test** | ✅ **781 test / 55 dosya** (762/53'ten **+19 / +2**): iki **envanter** birim testi dosyası — `spec/02` §4.1'in 47 ve 10 kod adını kategori kategori iddia ediyor, sonra tablo alanlarıyla ve snake_case sütun adlarıyla karşılaştırıyor. ⚠️ CHECK **yokluğu** bilerek buraya yazılmadı (birim testi Postgres'e sormaz — 2.3b) · ✅ **`pnpm test:db` 216 test / 8 dosya** (190'dan **+26**): 47/10 katalog envanteri · **57 sütunda 0 CHECK** (negatif) · 1:1 teklik ×2 · ayrı `id` yokluğu · CASCADE davranışı · **1:0..1 ayrımı** · `CA>PA` ve `min>max` reddi · **sınır dahil karşı örnek** · aralıkların kısıtsızlığı (CA=250) · `'referee'` kabul + `'coach'` reddi + boş dizi reddi · **hakem kişileri `['player']` taşımıyor** (`referees ⋈ people` envanteri) · `0007` ve `0008` çevrimleri · iki `down` sınırı · **sıfır kayıp** vakası · `④ SESSİZ bozuk down (KISIT TANIMI)` |
| **kapsam** | ⬆️ **EĞİLİM İLK KEZ TERSİNE DÖNDÜ** — satır %85,72 → **%86,99** · ifade %85,83 → **%86,99** · fonksiyon %77,56 → **%77,68** · dal %88,78 → **%88,78 (ÜÇÜNCÜ KEZ DEĞİŞMEDİ)**. Seri: 4.2 %80,16 → 4.3 %78,20 → 4.4 %77,56 → **4.5 %77,68**. **Sebep ölçüldü:** iki yeni şema dosyası raporda **%66,66 satır · %0 fonksiyon** (3 satırın 2'si kapsanıyor; kapsanmayan tek satır `.references(() => players.id, …)` **ok fonksiyonu** — 4.4'ün tespitinin aynısı). Yani her yeni tablo hâlâ 1 kapsanmayan fonksiyon ekliyor, ama bu kez **birim testleri dosyanın üst düzey kodunu koşturuyor** ve satır/ifade tarafı kazanıyor. ⚠️ **Bu bir hedef değil, envanter testlerinin YAN SONUCU** — testler `getTableColumns` ile gerçek bir iddia taşıyor, import testi değil. **Eşik %70 DÜŞÜRÜLMEDİ, dosya dışlanmadı, import testi yazılmadı.** Marj fonksiyonda **7,68 puan** |
| **Şema durumu** | **15 master tablo** + **4 indeks** + **21 FK** (13/19 → 15/21). Migration zinciri **9 dosya** (`0000`…`0008`), **dokuzunun da elle yazılmış `down`u var**. ⚠️ **Tek alt görev İKİ migration yazdı** ve bu bir **iddia ayrımı** kararı (§3.1.2 ⑤'in kendi notu): `0007` iki tablo + `players`ın iki kısıtı, `0008` yalnızca `people`ın CHECK tanımı. 🆕 **§3.1.2 ⑤'in AYRACI AYRIŞTI: `ALTER` değil SÜTUN.** `0008` bir `ALTER` ama kayma **üretmiyor** → çevriminde `identical: true` **beklenir** ve ölçüldü; kayma bir **zincir** özelliği değil bir **geri alma derinliği** özelliği. `comparedFacts` alt sınırı **2.243 → 3.023** — **ÖLÇÜLDÜ** (sınır `9_999_999`a kondu, gerçek sayı testin reddettiği çıktıdan okundu). Artış **+780**: 63 yeni sütun × `ColumnFacts` alanları + iki tablo + iki PK + iki FK + iki CHECK; `0008`in katkısı **0**. ⏳ Faz 4'ün kalan **yedi** master tablosu 4.6–4.7'de |
| **Veri durumu** | **2 / 15 tablo doludur** (`countries` 6, `competitions` 11); `people`, `players` ve iki nitelik tablosu **boş** — seed **4.9**'da. ℹ️ 3.8'in seed'i `source` sütununa **`procedural`** yazdı; gerekçe `tools/data-cli/src/seed/world-seed-data.ts` başlığında ve G-14'te |
