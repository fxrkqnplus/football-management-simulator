...
| **SAPMA sayısı** | **29** (SAPMA-001…029) — 🆕 **4.0 iki sapma açtı:** SAPMA-028 (`karar`, nitelik aralıkları CHECK almaz) · SAPMA-029 (`düzeltme`, `source` kümesi dört → beş). Ayrıca SAPMA-023'ün **tamamlanmamış yayılımı** kendi satırına not olarak işlendi |
| **Boşluk sayısı** | **14** (G-01…G-14) — sayı değişmedi, ama **yedisinin `Durum`u ✅'e döndü**: G-07·G-09·G-10·G-12·G-13·G-14 ROADMAP'e işlendi, G-11 ROADMAP'e yazıldı ama **kapanmadı** (Faz 11'in doğrulayıcısında kapanır). Artık **⏳ satır yok** |
| **Faz 4 kabul kriterleri** | **0 / 6** — 4.0 kod yazmadı. ⚠️ Kriter listesi 4.0'da **değişti**: *"Tüm nitelikler 1–20 CHECK"* → *"aralıklar CHECK ALMAZ, denetim Faz 11"* + *"İLİŞKİ değişmezleri (`CA <= PA`, `pa_range_min <= pa_range_max`) CHECK ALIR"* (SAPMA-028) |
