...
| | |
|---|---|
| **Aktif faz / alt görev** | **FAZ 4 — Veritabanı Şeması II.** Liste ROADMAP'te (`8203ac1`, K11). **4.0 · 4.0b · 4.1 · 4.2 · 4.3 · 4.4 kapandı.** ⏸️ **SIRADAKİ: 4.5** — `player_attributes` (47) + `player_hidden_attributes` (10) (`0007`); nitelik CHECK'i **YOK** (SAPMA-028), `players`'a `CA <= PA` ve `pa_range_min <= pa_range_max` CHECK'leri. 47+10 envanteri `spec/02` §4.1'den **sayılarak** doğrulanır |
| **Son tamamlanan** | ✅ **4.4 — üç ileri FK + migration `0006`. FAZIN İLK `ALTER TABLE`'I.** FK **16 → 19**, tablo **13'te sabit**. ⚠️ **ÜÇÜ RESTRICT DEĞİL — 4.3'ün tahmini yanlıştı ve sebebi YÖNTEMSELDİ:** tahmin *hedefin* sınıfına bakıyordu (`people` = `independent`), oysa kural ① dışında **kaynağın** sınıfına bakar. Kural **koşturuldu**, sonra ölçüldü, üçü de tuttu: `federations.president_person_id` **SET NULL** · `clubs.chairman_person_id` **RESTRICT** (nullable olmasına rağmen) · `referees.person_id` **RESTRICT** (üçün tek `NOT NULL`u). `federations` artık **bir CASCADE + bir SET NULL** taşıyor — kuralın sahipliği referanstan **aynı tablo içinde** ayırdığının ilk canlı kanıtı. 🆕 §3.1.2 ④ ve ⑤ ilk kez birlikte devrede: `attnum` deliği **her kısmi geri almada** görünüyor (günlük #12) |
...
| **CI — liste sorgusuyla** | ✅ **4.3'ün koşusu İŞLENDİ: `33275696638`** (`26843c4`, `push`, `conclusion: success`, **6/6 iş yeşil** — `Kalite kapıları` ve `Entegrasyon` amd64+arm64 ile iki `İmaj`). Dalda **5 push koşusu, beşi de yeşil**; `pull_request` olayı yok (PR faz sonunda), **iptal edilen koşu yok** (`status` alanı tek tek okundu — iptal ne yeşil ne kırmızıdır). **`1c93890`'dan beri 24 ardışık yeşil push koşusu, 0 kırmızı** — 18 (`feature/faz-03-database`) + 1 (`develop` merge) + **5** (bu dal). Bu daldaki sayı liste sorgusuyla ölçüldü; diğer iki bileşen 4.3'te ölçülmüştü ve bu turda yeniden ölçülmedi. ⏳ **4.4'ün koşusu yazım anında bilinmiyor** |
| **typecheck / lint / format / build / arch** | ✅ typecheck **10/10** · lint 0 · **format 0 — ilk koşuda `round-trip.itest.ts`i İKİ KEZ reddetti** (düzeltildi) · **build 8/8 SOĞUK** (`.turbo/cache` silindi, `Cached: 0` **doğrulandı**, turbo **6,64 s** / duvar saati **7,43 s**) · **arch 9 kural** — `rule:` belirteçlerinden **sayıldı** (`grep -c` **14 satır** veriyor; ayrı kural adı **9**), varsayılmadı |
| **Kapı kapsamı — ÖLÇÜLDÜ, varsayılmadı** | ⚠️ **4.4'te DE asıl doğrulayıcı kapı değildi ve bu ikinci tekrar.** `typecheck` üç yeni sütunu **hiçbir şey söylemeden** geçirdi (Drizzle sütun tanımı tip hatası üretmez); `ON DELETE` doğruluğunu doğrulayan şey yine **üretilen migration SQL'i** oldu — üç `ON DELETE` satırı tek tek okundu (`restrict` · `set null` · `restrict`). 🆕 **Ve `pnpm test:db` bu kez BİR TASARIM SONUCUNU açığa çıkardı, bir yazım hatasını değil:** dört var olan çevrim testi kırıldı çünkü `0006` `ALTER`-only (günlük #12) — hiçbir statik kapı bunu göremezdi |
...
| **test** | ✅ **762 test / 53 dosya** (değişmedi — 4.4 birim testi eklemedi, çünkü eklenen şey şema; kanıt entegrasyonda) · ✅ **`pnpm test:db` 190 test / 8 dosya** (178'den **+12**): üç ileri FK'nın **davranışı** (SET NULL · iki RESTRICT) · `NOT NULL` negatif testi · başkansız kulüp/federasyon karşı örneği · iki nullability iddiası · `0006` çevrimi (**tam fark listesi**) · **dolu tabloda gürültülü patlama** · **saf sütun kayıp ölçümü** · `federations` fiziksel sütun sırası (3.4'ten beri eksik olan nöbetçi) |
| **kapsam** | ⬇️ **YİNE DÜŞTÜ VE SEBEBİ ÖLÇÜLDÜ** — satır %85,96 → **%85,72** · ifade %86,05 → **%85,83** · fonksiyon %78,20 → **%77,56** · dal %88,78 → **%88,78 (YİNE DEĞİŞMEDİ)**. **Sebep:** üç değiştirilen şema dosyası raporda **%0 satır · %0 fonksiyon · %100 dal** (`clubs.ts` · `federations.ts` · `referees.ts`), yani eklenen her satır doğrudan paydaya giriyor. **Fonksiyon düşüşünün kaynağı adıyla belli:** her `.references(() => people.id, …)` bir **ok fonksiyonu** ve üçü de kapsanmıyor. Dal metriği yine etkilenmedi — Drizzle tablo tanımları dallanma içermiyor. **Eşik %70 DÜŞÜRÜLMEDİ, dosya dışlanmadı, import testi yazılmadı.** Marj fonksiyonda **7,56 puan** |
| **Şema durumu** | **13 master tablo (SABİT)** + **4 indeks** + **19 FK** (16 → 19). Migration zinciri **7 dosya** (`0000`…`0006`), **yedisinin de elle yazılmış `down`u var**. ⚠️ **`0006` zincirin 0001'den beri ilk `ALTER`-only migration'ı** ve bunun zincir çapında bir sonucu var: `attnum` deliği (§3.1.2 ⑤) artık **her kısmi geri almada** görünüyor, çevrim testleri `identical: true` yerine **farkların tam listesini** iddia ediyor. `comparedFacts` alt sınırı **2.204 → 2.243** — **ÖLÇÜLDÜ** (sınır `9_999_999`a kondu, gerçek sayı testin reddettiği çıktıdan okundu). Artış yalnızca **+39** ve bu beklenendi: üç sütun + üç kısıt, yeni tablo yok. ⏳ Faz 4'ün kalan **dokuz** master tablosu 4.5–4.7'de |
...
| **Boşluk sayısı** | **18** (G-01…G-18) — 🆕 **4.4 ikisini açtı:** **G-17** (markalı kimlik tipleri; `people.id` ↔ `players.id`) → **Faz 12** · **G-18** (hakemin `person_type`ı — kapalı küme hakemi ifade etmiyor, ama `referees.person_id` artık `NOT NULL`) → **Faz 8**. **İkisi de aynı alt görevde ROADMAP'e de yazıldı** (Kural 5); G-18 ayrıca Faz 8'e bir **kabul kriteri** getirdi. ⏳ satır **yok**. ℹ️ **G-18 yeni bir desen:** G-01…G-16 hep *"spec istiyor, kimse yapmıyor"*du; bu, spec'in **kendi içinde** tutarsız olduğu ve tutarsızlığın ancak **ilk gerçek uygulamada** göründüğü ilk vaka |
| **Faz 4 kabul kriterleri** | **1 / 6** — 🆕 **kriter 2 KAPANDI (4.4):** *"Üç ileri FK eklendi ve `ON DELETE` davranışı tanımlı"*. Yalnızca *tanımlı* değil **ölçülü**: üçünün de davranışı gerçek PG 18.6'ya karşı koşturuldu, katalogdan okumakla yetinilmedi. Kalanlar: kriter 1 ve 3 **4.9/4.10**'a (5.000 oyuncu seed + sorgu süresi), kriter 4 ve 5 **4.5**'e (nitelik CHECK'i yok + ilişki değişmezleri), kriter 6 **4.11**'e. `docs/schema/world.md` bugün güncel (4.4'te yeniden üretildi) ama faz sürerken yine bayatlayacak — o yüzden kriter 6 hâlâ `[ ]`. Yapılmayan **"yapıldı"** yazılmaz |
