...
| | |
|---|---|
| **Aktif faz / alt görev** | ▶️ **FAZ 6 — Tasarım Sistemi.** **6.0 ✅ · 6.1 ✅ · 6.2 ✅ · 6.3 ✅.** Sıradaki: **6.4 — shadcn/ui uyarlaması + temel bileşenler I** (Button…Tabs). §0.5 kontrol noktası **6.5 sonunda**; çizgi **6a = 6.0–6.5 / 6b = 6.6–6.12** |
| **Son tamamlanan** | ✅ **6.3 — tema + Tailwind 4 + fontlar.** Lockfile **647 → 682 (+35)**: Tailwind +33, fontlar +2, React +0. 🆕 **CSS TÜRETİLİYOR** (elle yazılmıyor); tazelik nöbetçisi **ilk gününde gerçek bir sapma yakaladı** (Prettier). 🆕 **§7.3'ün cümlesi yarı yanlış çıktı:** `ı` (U+0131) `latin-ext`te **değil**, `latin`te — iki alt küme de gerekli. 🆕 **React kuruldu, sonra KALDIRILDI** — tema katmanı onsuz yazılabildi ve jsdom'da tam test edilebiliyor; `types: []` korundu |
| **⚠️ KARAR BEKLİYOR** | **Açık temanın 12 tanımsız token'ı YAZILMADI.** İki ölçüm engelledi: ① önerilen türetme kuralı spec'in **kendi 8 değerine** karşı sınandı ve **çürüdü** (`--text-secondary` **−2,41** sapma) ② *"tema-bağımsız"* okuması da çöktü — beş anlamsal rengin **beşi de** açık zeminde AA altında (`--warning` **2,04** … `--danger` **3,91**). Yani on ikisi de bir **tasarım kararı**, türetme değil (SAPMA-026) |
| **Tarih / ilerleme / dallar** | 2026-09-05 · **5 / 50 faz (%10)** · `main` → `develop` (`2ec1548`) → **`feature/faz-06-design-system`** (taban `2ec1548`, **6 commit**) |
| **CI** | ✅ Dalda **3 push koşusu, 3'ü de success**; sonuncusu `a3d5bff`in koşusu. ⚠️ **Faz 4'ün KIRMIZI koşusu (`33419337117`) HÂLÂ YENİDEN DENENMEDİ** — kullanıcının kararı. ⏳ 6.3'ün koşusu yazım anında bilinmiyor |
| **Kapılar** | ✅ typecheck **11/11 SOĞUK** (`Cached: 0`) · build **8/8 SOĞUK** (ayrı silme) · **D5: `dist` import edilip ÇALIŞTIRILDI**, test dosyası sızmadı (0) · lint **0** (borusuz) · format **0** · arch **9 kural** · **test 1275/85** (+52/+4) · **test:db 301/10** · **gaps 20·3·17·0 ✗** · **i18n temiz, 334 dosya** |
| **Kapsam** | fonksiyon **%82,17 (401/488)** — yine **yükseldi** (6.2'de %81,29 / 378-465). Payda **+23**, pay **+23**. Marj `401/0,70 = 572` → boşluk **75 → 84**. Eşik düşürülmedi, dosya dışlanmadı |
| **Paket / ortam** | Üretim paketi **380.908 → 383.654 bayt (+2.746, %0,72)** — §11.6'nın yeni satırı **taban × 1,10**, bol marj. İlk **CSS varlığı**: 16.821 bayt. jsdom **30.0.1** (`matchMedia` yok) · Tailwind **4.3.3** · fontlar **5.3.0** |
| **Kütükler** | Açık sorun **0** · teknik borç **11 satır, 4 ÖDENDİ → açık 7** (🆕 **BORÇ-011**: `pnpm test` hiç build etmiyor, paketler arası testler **bayat `dist`** ölçebiliyor → **6.4**) · SAPMA **43** · boşluk **20**, açık **17** · günlük **7 satır** |
| **Bloke eden var mı?** | ⚠️ **6.4 başlayabilir** ama iki şey duruyor: ① **12 token kararı** (açık tema tamamlanmamış, üretilmiş CSS bunu kendi içinde yazıyor) ② **BORÇ-011 6.4'ün ilk işi** olmalı — yoksa bileşen testleri bayat `dist` ölçer. ℹ️ `main.test.tsx` jsdom yarışı **6.4'ten itibaren** izlenecek |
