...
| | |
|---|---|
| **Aktif faz / alt görev** | **FAZ 4 — Veritabanı Şeması II.** Liste ROADMAP'te (`8203ac1`, K11). 🆕 **FAZ 4a (4.0 → 4.7) KAPANDI** — §0.5 kontrol noktası 4.7'de ateşledi ve bölünme uygulandı. ⏸️ **SIRADAKİ: 4.8** (Faz 4b'nin ilki) — **transfer arama indeksleri** (`0011`); kapsam `spec/01`'in indeks satırından **değil** kriter 3'ün sorgusundan türetilir (o satır iki tabloyu karıştırıyor, 4.0'da ölçüldü). ✅ **`0008`in sınırı artık MİRAS DEĞİL:** `migrateDownPastRefereeCheck` sarmalayıcısı daraltmayı kendisi yapıyor — yeni bir test hiçbir şey hatırlamak zorunda değil. ⚠️ **Ama sınır KAYBOLMADI:** kendi testi ham `migrateDown` çağırıyor, ve `runner.itest.ts` de gerçek zinciri kullanıyor (bugün hakem yazmadığı için engeli görmüyor — uyarı o dosyanın başlığında) |
| **Son tamamlanan** | ✅ **4.7 — `staff` + `staff_attributes` + `managers` + `manager_attributes` (`0010`) + `migrateDownPastRefereeCheck` sarmalayıcısı (ayrı commit, kod işinden ÖNCE).** Envanter **18 → 22 tablo**, FK **26 → 32**. **FAZ 4'ÜN ON BİR MASTER TABLOSU KAPANDI** (11 Faz 3 + 11 Faz 4). FK kuralı **KOŞTURULDU** → **6/6** (CASCADE ×4 + SET NULL ×2). 🆕 **KARŞI-ÖLÇÜM kararı destekledi:** `staff`/`managers` §3.1.0 sütunlarını taşısaydı altı FK'nın **dördü** RESTRICT'e dönerdi — en keskin sonuç `club_id`, çünkü o zaman bir kulüp silindiğinde personeli serbest bırakmak yerine silme **reddedilirdi**. 🆕 **§3.1.2 ②'nin ayracı BEŞ kez koştu, İKİ cevap verdi:** `role` (12) · `coaching_badge` (5) · `experience_level` (5) **CHECK aldı**; `philosophy` **almadı** (küme `...` ile bitiyor → **sayılamıyor**, 4.6'nın `trait_code` gerekçesinin aynısı) ve `reputation` (0-200) almadı (kalibrasyon). 🆕 **`staff.person_id` UNIQUE DEĞİL** ve bu `spec/01`'in yazımından okunan ölçülmüş bir fark (`players` `FK UNIQUE` diyor, `staff` demiyor) — fixture aynı kişiye iki rol yazarak bunu **kullanıyor**, yoksa varsayım hiç sınanmazdı |
| **⚠️ 4.7'NİN EN PAHALI BULGUSU — KENDİ ÖLÇÜMÜMÜZ BAYATLADI (günlük #27)** | Devir notu `narrow…` için **15** çağrı yeri diyordu; kaynaktan sayınca **17** çıktı (`seedAllTables` 16 → **18**). Sayı 4.6'da alt görevin **ortasında** ölçülmüş, sonra iki test daha eklenmişti. **Yapısal iddia doğruydu ve hâlâ doğru** — bozulan yalnızca sayılardı. ⚠️ **D7'nin yeni bir biçimi:** bugüne kadar *"kendi planımız kaynak değildir"* diye okunuyordu; bu vaka **doğru yapılmış bir ölçümün de** yazıldığı andan sonra yanlışa dönebileceğini gösteriyor. **Kural: bir devir notundaki sayı, kullanılacağı gün YENİDEN sayılır** — özellikle *"kaç yer güncellenecek"* sayısıysa |
...
| **4.7'nin commit'leri** | **`420115f`** (sarmalayıcı: `migrateDownPastRefereeCheck`, 22 çağrı yeri, `runner.itest.ts` uyarısı, ADIM 0/ANALİZ arşivi) · **`0f9557e`** (içerik: dört tablo, `0010`, elle `down`, testler, `spec/01` §3.0 + §3.1.0, `spec/09` §11.5, `OUTPUT-FORMAT`, ER, ROADMAP + §0.5 sonucu) · **`<bu commit>`** (kayıt: ANLIK DURUM, günlük #26–#31, BORÇ-008, rapor arşivi) |
...
| **CI — liste sorgusuyla** | ✅ **4.6'nın koşusu İŞLENDİ: `33311685875`** (`8bca789`, `push`, `conclusion: success`). Dal üzerindeki **10 push koşusu** liste sorgusuyla tek tek okundu: **8 yeşil, 2 iptal, 0 kırmızı**. **İptal ne yeşil ne kırmızıdır** — ardışıklığa girmiyor, seriyi de kırmıyor. Ardışık yeşil: 18 (`feature/faz-03-database`) + 1 (`develop` merge) + **8** (bu dal) = **27**. Bu daldaki bileşen bu turda ölçüldü; diğer ikisi 4.3'te ölçülmüştü ve yeniden ölçülmedi. ⏳ **4.7'nin iki koşusu yazım anında bilinmiyor** |
...
| **typecheck / lint / format / build / arch** | ✅ typecheck **10/10** · lint 0 (ilk koşuda **2 hata** — `simple-import-sort`, `pnpm lint:fix` ile düzeltildi) · **format 0** · **build 8/8 SOĞUK** (`.turbo/cache` silindi, `Cached: 0` **doğrulandı**, turbo **11,16 s**) · **arch 9 kural** temiz (277 ms). ⚠️ **Soğuk build ilk denemede YANLIŞ KAPSAMDA koştu** (2/2) — `Bash` çalışma dizini `packages/db/drizzle`de kalmıştı; zemin doğrulanıp yeniden koşuldu (günlük #31) |
...
| **Kapı kapsamı — ÖLÇÜLDÜ, varsayılmadı** | ⚠️ **4.7'de DE asıl doğrulayıcı kapı değildi — BEŞİNCİ tekrar.** `typecheck` 48 yeni sütunu ve **altı** yeni FK'yı hiçbir şey söylemeden geçirdi. Doğrulayan şey yine **üretilen migration SQL'i** oldu: `ON DELETE` satırları koşturulan kuralla karşılaştırıldı (**6/6**), CHECK değerleri (12 · 5 · 5) ve `smallint NOT NULL` sayıları (16 · 8) SQL'den **programatik** sayıldı. 🆕 **Ve `pnpm test:db` yine tek başına 23 bayat envanter yakaladı** — hiçbir statik kapı göremezdi: ER bloğu, `comparedFacts` sınırı, FK envanteri, tablo sınıfları, sequence listesi, üç kayıp ölçümü, seed hacmi, zincir uzunluğu |
...
| **install** | ✅ **4.7 DE YENİ BAĞIMLILIK EKLEMEDİ** — `pnpm-lock.yaml` ve `pnpm-workspace.yaml` **diff yok** (`git status` ile doğrulandı). ℹ️ pnpm 11.24.0 çıkmış; kilit 11.23.0'da, karar verilmedi (`DEPENDENCY-WATCH` Faz 16) |
...
| **Veri durumu** | **2 / 22 tablo doludur** (`countries` 6, `competitions` 11); kalan **yirmisi boş** — seed **4.9**'da (kriter 1). ℹ️ 3.8'in seed'i `source` sütununa **`procedural`** yazdı; gerekçe `tools/data-cli/src/seed/world-seed-data.ts` başlığında ve G-14'te |
...
| **Açık sorun sayısı** | **0** |
| **⚠️ G-17'NİN MARUZİYETİ 4.7'DE BİR KADEME DERİNLEŞTİ** | Şemada artık **`people.id` · `players.id` · `staff.id` · `managers.id`** var ve dördü de `integer`. `fixtures.ts`e `staffIdOfPerson(personKey, role)` yardımcısı yazıldı ve **`playerIdOfPerson`den bir bileşen daha taşıyor**: `players.person_id` UNIQUE olduğu için kişi tek başına yeterliydi, `staff.person_id` UNIQUE **değil** — rol olmadan alt sorgu ya *"birden çok satır"* ile patlar ya da **yanlış personel satırını** seçer. Bir `people.id`yi `staff_id` sütununa yazmak FK tarafından **yakalanmaz** (o kimlikte bir personel büyük olasılıkla vardır). Bugünkü savunma yine bir **isimlendirme disiplini**; tip seviyesinde kapanması **Faz 12** (G-17) |
...
| **SAPMA sayısı** | **34** (SAPMA-001…034) — **4.7 hiçbir SAPMA açmadı.** Aday değerlendirildi ve **sapma çıkmadı**: promptun *"`spec/02`'den say"* notu yanlıştı ama o bir **prompt** hatası, bir spesifikasyon sapması değil — `spec/01` ve `spec/02` birbiriyle çelişmiyor, personel nitelikleri yalnızca `spec/01`'de tanımlı ve bu tutarlı. Faz 4'ün payı **yedi** (028…034) |
...
| **Faz 4 kabul kriterleri** | **3 / 6 — 4.7 de hiçbirini kapatmadı ve bu BEKLENEN.** 4.7 kriter **1** ve **6**'ya çalışıyor; ikisi de ancak sonraki alt görevlerde kapanabilir (kriter 1 → seed, **4.9**; kriter 6 → ER + kapanış, **4.11**). `docs/schema/world.md` bu turda **programatik olarak** yeniden üretildi (22 tablo / 32 FK, blok elle düzenlenmedi) ve mermaid **dışı** prose sayıları da güncellendi — ama faz sürerken yine bayatlayacak, kriter 6 hâlâ `[ ]`. **Yapılmayan "yapıldı" yazılmaz.** 🆕 **Kriter 3'ün zemini büyüdü:** `staff` ve `managers` bir gün personel aramasını besleyecek (ROADMAP Faz 17 *"oyuncu + kulüp + **personel** + lig + turnuva"*), ama indeks kararı **4.8**'in ve o alt görev kapsamı `spec/01`'in indeks satırından değil **kriter 3'ün sorgusundan** türetiyor |
