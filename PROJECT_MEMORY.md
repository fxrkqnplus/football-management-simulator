...
| | |
|---|---|
| **Aktif faz / alt görev** | **FAZ 4 — Veritabanı Şeması II.** Liste ROADMAP'te (`8203ac1`, K11). **FAZ 4a (4.0 → 4.7) KAPANDI**, **FAZ 4b açık**. ⏸️ **SIRADAKİ: 4.11** — ER diyagramı + `docs/schema/world.md` + **faz kaydı** + PR (→ kriter 6, fazın **son** kriteri). 4.11'in taşıdığı üç ek iş ROADMAP'e yazılı: ① **BORÇ-008** (CHECK literal ifadesinin **dokuz** kopyası) burada ödenir ② **`referees.ts:23`ün karşılıksız cümlesi** düzeltilir (4.9 günlük #36 → 4.10'da sahiplendirildi) ③ **`SPEC-COVERAGE-GAPS` ↔ ROADMAP tutarlılık kontrolü** koşturulur (**yirmi** G-satırı; G-20 4.9'da eklendi). ⚠️ **CI SAYIMI DA 4.11'DE işlenir** — 4.7'nin kırmızısı seriyi kırmıştı |
| **Son tamamlanan** | ✅ **4.10 — KABUL KRİTERİ 3 ÖLÇÜLDÜ VE KAPANDI. Faz 4 artık 5/6.** Migration **YOK** (zincir 12 · tablo 22 · FK 32 · indeks 6 · sequence 14 · `COMPARED_FACTS_FLOOR` 4.209 — altısı da sabit). İki yeni entegrasyon dosyası, **iki ayrı konteyner**. **A** (`transfer-search-criterion`): medyan **0,425 ms** / en kötü **0,461 ms** / ısıtmasız ilk koşu **0,426 ms**, bütçe **50 ms** → **~%1'i**; 27 satır; plan **kararlı** (5 koşu, tek plan). ⚠️ **SÜRE TARAFI ÖNEMSİZ ve SAKLANMADI** — aynı sorgu **indeksler kapalıyken de** bütçenin altında ve bu **koşan bir testle** iddia ediliyor, yani A *"indeks çalışıyor"* **demiyor**. **B** (`transfer-search-index`): 50.000 sentetik satır (hacim **uydurulmadı**, ROADMAP Faz 32'nin *"50.000 oyuncuda"* kriterinden **alındı**); indeksli **2,20 ms** vs indekssiz **6,08 ms** = **2,76×**; 3.9'un **KONTROL** ve **SIZINTI** nöbetçileri **uyarlandı** (kopyalanmadı — fark tabloyla dosya başlığında). 🆕 **`ANALYZE`IN ŞART OLDUĞU KOŞAN BİR TEST OLDU:** istatistiksiz planlayıcı **iki indeksi de** seçiyor; ölçüm `ANALYZE`sız alınsaydı *"her iki indeks de kullanılıyor"* yazılırdı ve **yanlış** olurdu. *"Öncesi"* durumu **yarıştan çekildi** (`autovacuum_enabled = false`) ve tarif ayrı bir sondayla, **aynı koşuda karşı kontrolle** doğrulandı (D2). 🆕 **Mimari etiketi ölçüldü:** Docker Engine **`linux/amd64`**; üretim **ARM64** (K14) — milisaniyeler oraya **taşınmaz** |
| **⚠️ 4.10'UN EN PAHALI BULGUSU — AYNI SORGUDA İKİ İNDEKS, ZIT KARAR** | 4.8 bir soru bırakmıştı: *"JOIN'in hangi taraftan başlayacağı seçiciliğe bağlı … **hangisi olduğu 4.10'un ölçümüdür**."* **Cevap: hiçbiri.** Planlayıcı bir **Hash Join** kuruyor ve iki tarafa **zıt** karar veriyor — aynı sorgu, aynı hacim (5.000): `players` tarafı **%1,5** seçici → **Bitmap Index Scan** (bileşik indeks); `people` tarafı **%35,5** → **Seq Scan**, `people_birth_date_idx` **hiç kullanılmıyor**. 3.9'un *"ayraç hacim değil seçicilik"* dersinin **en dar** biçimi: burada hacim bir **değişken bile değil**. 🆕 **VE AYRAÇ İKİ BOYUTLU ÖLÇÜLDÜ:** yedi kademeli merdiven (1.000 → 200.000, **200×**) planlayıcının kararını **hiç** çevirmedi — aranan *"çevrilme noktası"* **bulunamadı ve yokluğu bir bulgu**; çeviren tek şey seçicilik oldu (%0,36 → %94'te Seq Scan). ⚠️ **Bu, 4.8'in indeksinin bir kısmını KARŞILIKSIZ bırakıyor** — `people_birth_date_idx` kriter 3 tarafından kullanılmıyor. Kaldırılmadı (K12: 4.10 şemaya dokunmaz) ve **yeniden değerlendirme ROADMAP Faz 32 kapsamına** plan tablosu + üç somut soruyla yazıldı |
| **⚠️ 4.9'un en pahalı bulgusu (arşiv) — `reltuples` BİR DURUM DEĞİL, BİR YARIŞ (günlük #35)** | 3.9'un `ANALYZE` dersi *"`reltuples = -1` → planlayıcı kör"* diye bir **DURUM** olarak öğrenilmişti. Buraya kopyalanan iddia testcontainers'ta **geçti**; D5 aynı iddiayı gerçek ve uzun ömürlü bir PostgreSQL'e karşı koşturdu ve **`5000`** döndü. Mekanizma **tahmin edilmedi**, ayrı bir sonda betiğiyle dört anda ölçüldü: `autovacuum=on`/`naptime=60s` · seed'den hemen sonra `-1` · **+15 sn `5000`** ve `last_autoanalyze` damgalı · +60 sn'de `last_analyze` **hâlâ YOK**. Yani değer, ölçümün seed'den **kaç saniye sonra** alındığına bağlı ve testcontainers yarışın **kısa tarafında** bitiyor. **İki sonuç:** ① yazdığım test kırılgandı, iddia **değişmez** olana çevrildi (`last_analyze IS NULL` = *"seed `ANALYZE` çağırmıyor"*) + karşı kontrol (`n_live_tup = 5000`) ② **4.10'un yöntemi değişti** — `reltuples != -1` denetimi `ANALYZE` hiç çağrılmadan da yeşil verir, yani **baktığını göstermeden** onaylar (D3'ün ölçüm tarafındaki biçimi). **Genel kural: zamana bağlı bir değeri iddia eden test, o değerin YARIŞ olup olmadığı ölçülmeden yazılmaz.** ℹ️ D5 ikinci koşusunda `-1` verdi — aynı betik, farklı değer: yarışın en doğrudan kanıtı |
| **4.7 (önceki)** | ✅ **`staff` + `staff_attributes` + `managers` + `manager_attributes` (`0010`) + `migrateDownPastRefereeCheck` sarmalayıcısı (ayrı commit, kod işinden ÖNCE).** Envanter **18 → 22 tablo**, FK **26 → 32**. **FAZ 4'ÜN ON BİR MASTER TABLOSU KAPANDI** (11 Faz 3 + 11 Faz 4). FK kuralı **KOŞTURULDU** → **6/6** (CASCADE ×4 + SET NULL ×2). 🆕 **KARŞI-ÖLÇÜM kararı destekledi:** `staff`/`managers` §3.1.0 sütunlarını taşısaydı altı FK'nın **dördü** RESTRICT'e dönerdi. 🆕 **§3.1.2 ②'nin ayracı BEŞ kez koştu, İKİ cevap verdi:** `role` (12) · `coaching_badge` (5) · `experience_level` (5) **CHECK aldı**; `philosophy` **almadı** (küme `...` ile bitiyor → **sayılamıyor**) ve `reputation` (0-200) almadı (kalibrasyon). 🆕 **`staff.person_id` UNIQUE DEĞİL** ve bu `spec/01`'in yazımından okunan ölçülmüş bir fark. ✅ **`0008`in sınırı artık MİRAS DEĞİL** — sarmalayıcı daraltmayı kendisi yapıyor; **ama sınır KAYBOLMADI** (kendi testi ham `migrateDown` çağırıyor) |
| **⚠️ 4.7'NİN EN PAHALI BULGUSU (arşiv) — KENDİ ÖLÇÜMÜMÜZ BAYATLADI (günlük #27)** | Devir notu `narrow…` için **15** çağrı yeri diyordu; kaynaktan sayınca **17** çıktı (`seedAllTables` 16 → **18**). Sayı 4.6'da alt görevin **ortasında** ölçülmüş, sonra iki test daha eklenmişti. **Yapısal iddia doğruydu ve hâlâ doğru** — bozulan yalnızca sayılardı. ⚠️ **D7'nin yeni bir biçimi:** bugüne kadar *"kendi planımız kaynak değildir"* diye okunuyordu; bu vaka **doğru yapılmış bir ölçümün de** yazıldığı andan sonra yanlışa dönebileceğini gösteriyor. **Kural: bir devir notundaki sayı, kullanılacağı gün YENİDEN sayılır** — özellikle *"kaç yer güncellenecek"* sayısıysa |
| **4.6 (önceki)** | ✅ **`player_positions` + `player_traits` + `player_stats_history` (`0009`) + `bash-text-guard` kancası (ayrı commit, kod işinden ÖNCE).** Envanter **15 → 18 tablo**, FK **21 → 26**. **Fazın ilk 1:N tabloları ve ilk BİLEŞİK PK'leri.** FK kuralı **KOŞTURULDU** → **5/5** (CASCADE ×4 + SET NULL), üretilen SQL'in `ON DELETE` satırlarıyla tek tek karşılaştırıldı. 🆕 **§3.1.2 ②'nin ayracı AYNI ALT GÖREVDE İKİ FARKLI CEVAP ÜRETTİ:** `position` (12 kod, `players.ts`ten **ithal** — iki kopya ayrışamasın) ve `level` (5 derece) **CHECK aldı**; `trait_code` **almadı**, çünkü küme `spec/02`'de **hiç tanımlı değil** (0 eşleşme, ölçüldü) ve ROADMAP *"~30"* diyor — sayılamayan bir küme kapalı iddia edilemez. 🆕 **`competition_id`in CASCADE alması sezgiye aykırı** ve 4.3'ün 4.4 için yaptığı yanlış tahminin sınıfı: kural ② **hedefin değil KAYNAĞIN** sınıfına bakıyor |
| **4.5 (önceki)** | ✅ **`player_attributes` (47) + `player_hidden_attributes` (10) + `players`ın iki ilişki değişmezi (`0007`) + `'referee'` (`0008`). FAZIN İLK ÇİFT-MIGRATION ALT GÖREVİ.** Envanter **13 → 15 tablo**, FK **19 → 21**. **47 ve 10 `spec/02` §4.1'den SAYILDI** (14+14+8+11, benzersizlik ayrıca ölçüldü), ROADMAP'ten alınmadı — SAPMA-001 tam bu sınıftı ve `player_hidden_attributes` onun **kendi vakası**. Envanter bir **sayı** değil bir **liste** olarak yaşıyor, üç katmanlı iddia: sabit → TS alanı → katalog sütunu. **1:1 ayracı KOŞTURULDU** (*"tabloya gelen FK"*: iki tablo için de **0**) → 3.5 deseni, `players`ınki kopyalanmadı. FK kuralı da koşturuldu → **CASCADE · CASCADE**, üretilen SQL'le 2/2. 🆕 **G-18 KAPANDI** ve 4.4'ün ataması **D7 gerekçesiyle** geri alındı: Faz 8'in gerçek ingest listesinde hakem **yok**, blok ve kriter oradan **kaldırıldı**. 🆕 **G-19 açıldı** — hiçbir faz hakem verisini ingest etmiyor; sahip **tahminle atanmadı**, karar noktası **Faz 7**'ye kabul kriteriyle yazıldı |
| **⚠️ 4.5'İN SINIRI (arşiv) — zincir çapında** | `0008`in `down`u `people_person_type_check`i **daraltıyor** ve `ADD CONSTRAINT … CHECK` var olan satırları **doğruluyor**. Sonuç: dolu bir `people` tablosunda `'referee'` varsa geri alma **gürültülü patlıyor** — ve `down` **LIFO** çalıştığı için, kısıt zincirin en üstünde olduğundan **hiçbir** geri alma başlayamıyor. **0006'nın sınırından yapısal olarak farklı** (orada patlayan `up`tı, etkisi kendi testiyle sınırlıydı). On altı test birden kırıldı, çoğu 4.5'in **hiç dokunmadığı** testler. Sınır **kabul edildi ve gizlenmedi** (kendi testi var); üç alternatif tek tek elendi ve elenme sebepleri `drizzle/down/0008_person_type_referee.sql` başlığında. **Ders: bir kapalı kümeyi genişletmek, o kümeyi daraltan geri almayı VERİYE BAĞIMLI hâle getirir** (günlük #17) |
| **4.4 (önceki)** | ✅ Üç ileri FK + `0006` — FK 16 → 19, fazın ilk `ALTER TABLE`'ı. Üçü **üç farklı** `ON DELETE` aldı (SET NULL · RESTRICT · RESTRICT) çünkü kural ① dışında **kaynağın** sınıfına bakıyor; 4.3'ün hafızadan yapılmış tahmini yanlıştı, 4.4 kuralı **koşturdu** ve 3/3 tuttu. `attnum` deliği artık her kısmi geri almada görünüyor (günlük #12) |
| **🆕 KANCA — `bash-text-guard` (4.6, AYRI COMMIT, kod işinden ÖNCE)** | ORTAM TUZAKLARI ⑤ üç ardışık alt görevde ihlal edilmişti (#3 · #14 · #19; sonuncusu iki kez, biri **commit mesajında**). Teşhis #19'da yazılıydı: *"kural doğru, eksik olan kuralın ATEŞLENDİĞİ ANDA görünür olması"* — SAPMA-033'ün sınıfı. `PreToolUse`/`Bash` kancası **DAR**: bir **taşıyıcı** (heredoc · `node -e` · `python -c`) taşıdığı yük **ters tırnak** VEYA **ASCII olmayan bayt** içeriyorsa reddediyor. **İki koşul birden** — saf ASCII SQL heredoc'ları ve tek argümanlı Türkçe `git commit -m` etkilenmiyor; üç ihlalin üçü de bu imzayı taşıyor (ölçüldü, üçü de testte). **Kancanın kendi kaçış yolu YOK.** ⚠️ **KABLOLAMA BU OTURUMDA ÖLÇÜLEMEDİ** ve `✅` yazılmadı (SAPMA-024): `.claude/settings.json` oturum **başında** okunuyor, bu oturum onsuz başladı; gerçek bir heredoc çağrısı denendi ve **ateşlenmedi**. Kancanın kendi sözleşmesi **iki yönlü** ölçüldü (alt süreç, gerçek `stdin`: engellenmesi gereken → çıkış **2** + stderr, engellenmemesi gereken → çıkış **0** + BOŞ stderr). **Kablolamanın doğrulanması bir sonraki oturumun ilk işi** |
| **4.10'un commit'leri** | **`9fd09b0`** (içerik: `transfer-search-criterion.itest.ts` + `transfer-search-index.itest.ts`, ROADMAP — 4.10 `[x]` + kriter 3 `[x]` + **Faz 32 kapsamına plan bulgusu** + **4.11'e `referees.ts` maddesi**, `spec/09` §11.5) · **`<bu commit>`** (kayıt: ANLIK DURUM, günlük #37–#38, rapor arşivi) |
| **4.7'nin commit'leri (arşiv)** | **`420115f`** (sarmalayıcı) · **`0f9557e`** (içerik: dört tablo, `0010`) · **`0459fa5`** (kayıt) |
| **4.6'nın commit'leri (arşiv)** | **`4a1c40f`** (kanca) · **`6a08776`** (içerik) · **`8bca789`** (kayıt) |
| **Tarih** | 2026-09-02 |
| **Genel ilerleme** | **3 / 50 faz (%6)** — Faz 3 kapandı ve `develop`'a merge edildi (PR #4). 🆕 **Faz 4 ikiye bölündü: 4a (4.0→4.7) KAPANDI, 4b (4.8→4.11) açık.** Faz sayısı değişmedi — bölünme bir **kayıt** ayrımı, kapsam değişikliği değil |
| **⚠️ §0.5 KONTROL NOKTASI ATEŞLENDİ VE BİR KAYDI DEĞİŞTİRDİ (4.7)** | İki ölçüm: 4.7'nin **başında 1,941 gün** (eşik 2 — aşılmadı), **kapanışında 2,633 gün** (**AŞILDI**). Kaynak `git show -s`: Faz 4'ün ilk commit'i `0682c5f` (2026-08-29 05:00:19 +0300). Kural uygulandı ve ROADMAP'e yazıldı: **4a = 4.0→4.7 · 4b = 4.8→4.11**. Çizgi zaten hazırdı (ROADMAP Faz 4 risk notu). ⚠️ **Faz 3'te aynı kural ateşlemişti (4 gün, sınır 3) ve HİÇBİR ŞEY OLMAMIŞTI** çünkü süreyi ölçen bir adım yoktu; adım 4.1'de `SESSION-TEMPLATE`e eklendi (SAPMA-033) ve **bu ilk kez gerçekten bir sonuç üretti**. Tahmin listesi bu fazı işaretlememişti — yakalayan şey **ölçüm** oldu. ℹ️ **Tek PR ve tek faz kaydı 4.11'de**; bölünme faz kaydının §1'inde görünecek |
...
| **⚠️ CI — 4.9 YEŞİL, SAYAÇ 2; 4.7'NİN KIRMIZISI HÂLÂ AÇIK** | ✅ **4.9'un koşusu: `33559303179`** (`c53e691`, `push`, `completed`, **`success`**) — liste sorgusuyla işlendi. ℹ️ İki commit birlikte push edildiği için `cd415eb`in **ayrı koşusu yok**; koşu tepe commit'e açıldı. ⚠️ **`33419337117`** (`0459fa5`, 4.7) **`failure`** olarak **duruyor ve YENİDEN DENENMEDİ** — dışa dönük bir eylem, kullanıcının kararı. Sebebi kod dışıydı (İmaj/amd64, Docker Hub auth zaman aşımı) **ama bir iptal değil bir başarısızlık** ve `1c93890`'dan beri süren **27 ardışık yeşil** seri orada **sonlandı**. **Bugünkü sayaç: 2 ardışık yeşil** (`33450312143`, `33559303179`). **Tam sayım 4.11'de işlenir.** ⏳ 4.10'un koşusu yazım anında bilinmiyor |
| **CI (arşiv)** | ✅ **4.6'nın koşusu: `33311685875`** (`8bca789`, `push`, `success`). Dal üzerindeki **10 push koşusu**: **8 yeşil, 2 iptal, 0 kırmızı**. Ardışık yeşil o gün: 18 (`feature/faz-03-database`) + 1 (`develop` merge) + **8** (bu dal) = **27** |
| **CI (arşiv)** | ✅ **4.5'in koşusu: `33289958620`** (`82d62d1`, `push`, `conclusion: success`). 🆕 **VE BU TURDA İLK KEZ İPTAL EDİLEN KOŞU GÖRÜLDÜ:** `33289938530` (`f384402`) ve `33289927984` (`5c17673`) `conclusion: cancelled` — üç commit peş peşe push edildiği için GitHub önceki iki koşuyu iptal etti. **İptal ne yeşil ne kırmızıdır**: ardışıklık sayımına **girmiyor**, ama seriyi de **kırmıyor**. Dalda **9 push koşusu: 7 yeşil, 2 iptal, 0 kırmızı**. **`1c93890`'dan beri 26 ardışık yeşil push koşusu, 0 kırmızı** — 18 (`feature/faz-03-database`) + 1 (`develop` merge) + **7** (bu dal). Bu daldaki sayı liste sorgusuyla ölçüldü (`event`, `status` ve `conclusion` alanları tek tek okundu); diğer iki bileşen 4.3'te ölçülmüştü ve bu turda yeniden ölçülmedi. ⏳ **4.6'nın koşusu yazım anında bilinmiyor** |
| **typecheck / lint / format / build / arch** | ✅ typecheck **10/10**, **8 cache miss** · lint 0 (ilk koşuda **1 hata** — `require-await`; susturulmadı, test `async` olmaktan **çıkarıldı**) · **format 0** (ilk koşuda **2 dosya**, `prettier --write`) · **build 8/8 SOĞUK** (`.turbo/cache` silindi, `Cached: 0` **doğrulandı**, `pwd` = `/c/fms` önceden denetlendi — #31 altıncı kez ısırmadı, turbo **10,37 s**) · **arch 9 kural** temiz (284 ms). ⚠️ **`format` bu commit'in `.md` dosyalarına BAKMADI** (`.prettierignore` `*.md`, SAPMA-024); denetlenen küme **TypeScript**. ⚠️ **VE `pnpm test` HİÇ DEĞİŞMEDİ (967/66)** — 4.10 `src` altına kod yazmadı, yani birim test kapısı bu alt görevin ürettiği **hiçbir dosyaya bakmadı**; doğrulayan kapı **`test:db`** oldu (**+24**) |
| **typecheck / lint / format / build / arch (4.6 arşiv)** | ✅ typecheck **10/10** · lint 0 (ilk koşuda **2 hata** — `no-control-regex` (`[^\x00-\x7F]`) ve import sıralaması; birincisi `eslint-disable` ile susturulmadı, `codePointAt` ile **yeniden yazıldı**) · **format 0** (ilk koşuda **6 dosya** reddedildi, `prettier --write`) · **build 8/8 SOĞUK** (`.turbo/cache` silindi, `Cached: 0` **doğrulandı**, turbo **6,06 s**) · **arch 9 kural** temiz (272 ms) |
| **Kapı kapsamı — ÖLÇÜLDÜ, varsayılmadı** | ⚠️ **4.10'da DA asıl doğrulayıcı kapı değildi — SEKİZİNCİ tekrar.** `typecheck` iki yeni entegrasyon dosyasını, iki `EXPLAIN` sarmalayıcısını ve 50.000 satırlık bir üretim SQL'ini hiçbir şey söylemeden geçirdi; soğuk koştu (8 cache miss). Doğrulayan şeyler: **gerçek PostgreSQL'in planlayıcısı** (iki indeksin zıt kararı), **katalog** (`pg_stat_all_tables.last_analyze`/`n_live_tup`), ve **iki sonda betiği** (hacim merdiveni · `autovacuum_enabled` doğrulaması). 🆕 **Ve bu turda `pnpm test` bile bakacak bir şey bulamadı** — 967/66 sabit, çünkü 4.10 `src` altına hiç kod yazmadı. *"Bakacak bir şey bulamayan kapıya ✅ yazılmaz"* burada **birim test kapısı** için geçerli |
| **Kapı kapsamı (4.9 arşiv)** | ⚠️ **4.9'da DA asıl doğrulayıcı kapı değildi — YEDİNCİ tekrar.** `typecheck` 5.000 satırlık bir üreteci, iki tabloya yazan **28 sütun bağını** ve `ageRangeToBirthDateRange`in yeni tüketicisini hiçbir şey söylemeden geçirdi; **8 cache miss** olduğu için *"bakacak bir şey yoktu"* bahanesi yok. Doğrulayan şeyler: **gerçek PostgreSQL'in reddi** (`integer out of range` → günlük #34), **katalog** (`pg_stat_all_tables`, `information_schema.columns`, `pg_class.reltuples`), ve **D5** (üç uyuşmazlık buldu, üçü de **benim beklentimde**: sütun sayısı 16 sanılmıştı, katalog **17** dedi — #30/#32'nin sınıfı ama bu kez bir sütun **adında** değil bir **beklenti sayısında**). 🆕 **Ve `pnpm test:db` yine tek başına İKİ bayat envanter yakaladı:** `seed-query-performance.itest.ts`in hacim iddiası ve `world-seed.itest.ts`in ifade sayısı. Hiçbir statik kapı ikisini de göremezdi |
| **Kapı kapsamı (4.7 arşiv)** | ⚠️ **4.7'de DE asıl doğrulayıcı kapı değildi — BEŞİNCİ tekrar.** `typecheck` 48 yeni sütunu ve **altı** yeni FK'yı hiçbir şey söylemeden geçirdi. Doğrulayan şey yine **üretilen migration SQL'i** oldu: `ON DELETE` satırları koşturulan kuralla karşılaştırıldı (**6/6**), CHECK değerleri (12 · 5 · 5) ve `smallint NOT NULL` sayıları (16 · 8) SQL'den **programatik** sayıldı. 🆕 **Ve `pnpm test:db` yine tek başına 23 bayat envanter yakaladı** — hiçbir statik kapı göremezdi: ER bloğu, `comparedFacts` sınırı, FK envanteri, tablo sınıfları, sequence listesi, üç kayıp ölçümü, seed hacmi, zincir uzunluğu |
| **D5 — 4.10'da KONUSUZ, ve bu bir atlama DEĞİL** | ⚠️ 4.10 **`src` altına hiç kod yazmadı** — yani derlenip çalıştırılacak **yeni bir yüzey yok**; D5'in sorduğu soru (*"derlenmiş hâl gerçekten çalışıyor mu"*) burada konusuz. Ölçülen şey zaten **gerçek bir PostgreSQL'e karşı koşan sorgular**. ℹ️ Üstelik iki sonda betiği de **derlenmiş `dist`e karşı düz `node`** ile koştu (`packages/db/dist` + `tools/data-cli/dist`, ayrı gerçek veritabanları `fms_probe_410` · `fms_probe_a410` · `fms_probe_av410`) — yani D5'in **ruhu karşılandı**, sözleşmesi ise bu turda uygulanamazdı |
| **D5 (4.9 arşiv)** | ✅ **54 / 54.** Vitest yok, testcontainers yok; `fms_d5_49` yaratıldı, zincirin **on iki adımı** uygulandı, sonunda düşürüldü. Ölçülenler: zincir 12 · tablo 22 · FK 32 · indeks 6 (**dördü de sabit**) · **sütun adları KATALOGDAN** (`people` 17, `players` 17) ve `players.key` katalogda **YOK** · `RETURNING` 5.000 + 5.000 · tablo sayımı 5.000 + 5.000 · `club_id` NULL **5.000/5.000** + **KARŞI KONTROL** dolu **0** · kriter 3 aralığı `2001-07-02 … 2006-07-01` · **KRİTER 3 SATIR SAYISI 27** · DR **389** · CA>120 **1.106** · 20–24 **1.776** · sekiz tablo **BOŞ** (beş nitelik + `staff` + `managers` + `clubs`) · iki koşu zaman damgaları dışında **birebir aynı** · `players.id` **sabit** · `updated_at` ikisinde de ilerledi · **üç NEGATİF kısıt** (`ca_le_pa` · `pa_range` · `primary_position`, hepsi **adıyla**) + **KARŞI ÖRNEK** geçerli satır kabul · silinen satır üçüncü koşuda **geri yazıldı** · seed `ANALYZE` **çağırmadı**. **Süre:** ilk koşu **2.795 ms**, ikinci **2.439 ms**. ⚠️ Üç uyuşmazlık çıktı, üçü de **beklentide** (günlük #34, #35) |
| **D5 (4.7 arşiv)** | ✅ **25 / 25.** Vitest yok, testcontainers yok; `fms_d5_47` adında ayrı bir veritabanı yaratıldı ve zincirin **on bir adımı** uygulandı. Ölçülenler: **22 tablo** · **32 FK** · sütun sayıları katalogdan (`staff` **6**, `staff_attributes` **19**, `managers` **12**, `manager_attributes` **11**) · derlenmiş sabitler (12 · 16 · 8 · 5 · 5) · **`managers.user_id` YOK** · **POZİTİF** aynı kişiye iki personel satırı (`person_id` UNIQUE değil) · **NEGATİF** 13. rol ve 6. rozet reddedildi · **POZİTİF** `philosophy` kısıtsız (uydurma değer kabul) · `spoken_languages` dizi olarak döndü (`{tr,en}`) · **CASCADE** kişi → personel/menajer/nitelikler (**iki kademe**) · **SET NULL** kulüp silinince personel **durdu**, `club_id` **boşaldı** · `0010` çevrimi **birebir aynı** (`comparedFacts=4205`, bağımsız doğrulama). ⚠️ D5 fixture kullanmıyor, ham SQL yazıyor — `clubs` sütun adları **hafızadan** yazılınca gürültülü patladı ve katalogdan okunarak düzeltildi (günlük #30) |
| **install** | ✅ **4.10 DA YENİ BAĞIMLILIK EKLEMEDİ** — `pnpm-lock.yaml` ve `pnpm-workspace.yaml` **diff yok** (`git status` ile doğrulandı). Ölçüm bir kıyaslama kütüphanesi (`benchmark`, `tinybench` vb.) **kurmadı**: süre dağılımı `EXPLAIN (ANALYZE)`ın kendi `Execution Time` alanından, dokuz örneklemin medyanı olarak okundu — veritabanının **kendi** ölçümü, JS tarafındaki bir sarmalayıcının değil. ℹ️ `DEPENDENCY-WATCH`'ta Faz 4'e atanmış satır **yok** (yeniden ölçüldü) |
| **install (4.7 arşiv)** | ✅ **4.7 DE YENİ BAĞIMLILIK EKLEMEDİ.** ℹ️ pnpm 11.24.0 çıkmış; kilit 11.23.0'da, karar verilmedi (`DEPENDENCY-WATCH` Faz 16) |
| **install (4.6 arşiv)** | ✅ **4.6 DA YENİ BAĞIMLILIK EKLEMEDİ** — `pnpm-lock.yaml` ve `pnpm-workspace.yaml` **diff yok** (`git status` ile doğrulandı). Kanca yalnızca Node yerleşiklerini (`node:url`, `node:child_process`, `node:path`) kullanıyor. ℹ️ pnpm 11.24.0 çıkmış; kilit 11.23.0'da, karar verilmedi |
| **test** | ✅ **`pnpm test` 967 / 66 — DEĞİŞMEDİ** (4.10 `src` altına kod yazmadı; bu bir eksiklik değil, alt görevin şekli) · ✅ **`pnpm test:db` 301 test / 10 dosya** (277/8'den **+24 / +2**): **A** — hacim (önce hacim sonra süre, SAPMA-024) · `last_analyze` denetimi + **karşı kontrol** `n_live_tup` · 27 satır · medyan+en kötü < 50 ms · ısıtmasız ilk koşu · `TIMING OFF` (D2) · **plan: bileşik indeks VAR, `people_birth_date_idx` YOK** · `ANALYZE` **öncesi/sonrası** planın farklı olduğu · plan kararlılığı · ⚠️ **indeksler kapalıyken de bütçenin altında** (A'nın kendi sınırı) · `SET LOCAL` sızıntısı. **B** — 50.000 hacim · istatistik + karşı kontrol · seçicilikler (**%0,36** ve **%94**) · seçici → iki indeks, **seçici olmayan → Seq Scan** · indeksli < indekssiz · **KONTROL** · **SIZINTI** · seed hattının bu veritabanına **hiç dokunmadığı** |
| **test (4.9 arşiv)** | ✅ **967 test / 66 dosya** (887/64'ten **+80 / +2**): iki yeni dosya — `player-seed-data.test.ts` (ağırlık tablolarının **toplamı 1.000**, mevki kümesi `PLAYER_POSITIONS`la **birebir**, yaşlar **kesintisiz**, 20–24 dilimi **tam 355**, bantlar **çakışmıyor** ve **120 eşiği bir bandın İÇİNDE**, uyruk anahtarları `SEED_COUNTRIES`te **var** + karşı kontrol, `spec/02`de ayak ölçeğinin **yokluğu**, anahtar **sözlük sırası** + karşı örnek) ve `player-generator.test.ts` (determinizm · **bağımlılık** · saflık · inşa değişmezleri · pencere sınırları · **27 satırlık kesişim** + `age` alanıyla **karşı kontrol**). 🆕 **Saflık `grep`le değil KOŞARAK kanıtlandı:** `Math.random` ve `Date.now` patlayacak şekilde değiştirildi, üretim yine tamamlandı — dolaylı bir çağrı (`new Date()`) aramaya takılmazdı · ✅ **`pnpm test:db` 277 test / 8 dosya** (251'den **+26**): 5.000+5.000 satır · `club_id` **iki yönlü** · **çakışma programatik** (var olan küme önce **doldurularak**) · determinizm · `created_at`/`updated_at` **iki tablo için ayrı** · **üç NEGATİF kısıt** + karşı örnek · beş nitelik tablosunun **boş** kaldığı · zincir **12** ve tablo **22** dokunulmadan |
| **test (4.7 arşiv)** | ✅ **868 test / 63 dosya** (822/59'dan **+46 / +4**): dört **envanter** birim testi dosyası (`staff.test.ts` 12 rolü listeyle **ve** `spec/01` metninde arayarak · `staff-attributes.test.ts` 16 nitelik + **`spec/02`'de yokluk** iddiası · `managers.test.ts` `user_id` yokluğu + üç kapalı küme + `philosophy`nin `...`i · `manager-attributes.test.ts` 8 nitelik + iki tablonun **kesişimi**). 🆕 **İKİ KARŞI KONTROL yazıldı (D3):** `isUnique` ve `hasDefault` var olmayan alanlar olsaydı negatif iddialar **kör geçerdi** — `players.person_id` UNIQUE ve `created_at` DEFAULT taşıyor, ikisi de ayrıca iddia ediliyor · ✅ **`pnpm test:db` 241 test / 8 dosya** (230'dan **+11**): `person_id` UNIQUE olmama pozitifi · **NEGATİF** 13. rol · **NEGATİF** 6. rozet · **NEGATİF** geçersiz deneyim seviyesi · karşı örnek (geçerli değerler kabul) · `philosophy` CHECK yokluğu **iki yönlü** · nitelik tablolarının CHECK yokluğu **iki yönlü** (250 kabul ediliyor) · **CASCADE** kişi→personel→nitelik (iki kademe) · **SET NULL** personel ve menajer için iki yönlü · `0010` çevrimi · dört tablo kaybı (bayraksız **reddediliyor** + bayraklı rapor) |
| **kapsam** | ➡️ **DEĞİŞMEDİ — ve bu BEKLENENDİ, ölçülerek doğrulandı.** satır **%87,99** · ifade **%87,94** · fonksiyon **%78,80** (357/453) · dal **%88,94** — dördü de 4.9'la **birebir aynı**. Sebep yapısal: 4.10 `src` altına hiç kod yazmadı; iki yeni dosya `integration/` altında ve `vitest.config.ts`in `coverage.include` deseni yalnızca `src` ağaçlarını topluyor, yani **ne pay ne payda** kımıldadı. Beklenti *"marj daralmaz"*dı ve **tuttu**; tutmasaydı bu bir bulgu olurdu. ⚠️ **Eşik %70 DÜŞÜRÜLMEDİ, dosya dışlanmadı, import testi yazılmadı.** Marj **yeniden hesaplandı** (kopyalanmadı): `357 / 0,70 = 510` → payda en fazla **510**, bugün **453** → boşluk **57 fonksiyon**. ℹ️ Artefakt bu turda **bayat değildi** — 4.9'un raporuyla uyuşuyordu (4.8'de uyuşmamıştı) |
| **kapsam (4.7 arşiv)** | ⬇️ **FONKSİYON YİNE DÜŞTÜ, SEBEBİ DOSYA BAZINDA ÖLÇÜLDÜ** — satır %86,49 → **%85,86** · ifade %86,47 → **%85,76** · fonksiyon %75,91 → **%73,60** · dal %88,64 → **%88,64** (değişmedi). Seri: 4.4 %77,56 → 4.5 %77,68 → 4.6 %75,91 → **4.7 %73,60**. **Sebep `coverage-summary.json`dan okundu:** dört yeni şema dosyası **12 kapsanmayan fonksiyon** getiriyor — `staff.ts` **0/5**, `managers.ts` **0/5**, `staff-attributes.ts` **0/1**, `manager-attributes.ts` **0/1**. Hepsi ok fonksiyonu: `.references(() => …)`, `(table) => [...]` ve yerel `literals`. Toplam **290/394**. ℹ️ Tahmin −1,5…−2 puandı, gerçek **−2,31** — `staff.ts` beklenenden bir fonksiyon fazla getirdi (`literals` yardımcısı sayılmamıştı). ⚠️ **Eşik %70 DÜŞÜRÜLMEDİ, dosya dışlanmadı, import testi yazılmadı.** Marj fonksiyonda **3,60 puan** (5,91'den daraldı) — 4.9'un seed'i geri kazandırdı (**ölçüldü**: %78,80) |
| **kapsam (4.6 arşiv)** | ⬇️ satır %86,99 → **%86,49** · ifade %86,99 → **%86,47** · fonksiyon %77,68 → **%75,91** · dal %88,78 → **%88,64**. Seri: 4.2 %80,16 → 4.3 %78,20 → 4.4 %77,56 → 4.5 %77,68 → **4.6 %75,91**. **Sebep dosya bazında ölçüldü, tahmin edilmedi:** üç yeni şema dosyası **9 kapsanmayan fonksiyon** getiriyor — `player-positions.ts` **0/4**, `player-stats-history.ts` **0/3**, `player-traits.ts` **0/2**. Hepsi ok fonksiyonu: `.references(() => …)`, `(table) => [...]` ve `inList`. **4.4/4.5'in tespitinin büyümüş hâli:** o gün tablo başına **1** kapsanmayan fonksiyon vardı; 4.6'da ortalama **3**, çünkü bileşik PK ve CHECK'ler ek ok fonksiyonu getiriyor. ℹ️ Karşı yönde bir kazanç da var: `drizzle-snapshot.ts` **%100 satır / %100 fonksiyon** (13/13) — üç yeni birim testi onu tam kapsıyor. ⚠️ **Eşik %70 DÜŞÜRÜLMEDİ, dosya dışlanmadı, import testi yazılmadı.** Marj fonksiyonda **5,91 puan** |
| **Şema durumu** | **22 master tablo** + **6 indeks** + **32 FK** + **14 sequence**. 🆕 **4.10 DA ŞEMAYA HİÇ DOKUNMADI — migration YAZILMADI** (KARAR 6): zincir **12 dosya** sabit, `COMPARED_FACTS_FLOOR` **4.209** sabit, `EXPECTED_TABLE_COUNT` **22** / `EXPECTED_FOREIGN_KEY_COUNT` **32** dokunulmadan; altı envanter testi de değiştirilmeden yeşil kaldı. ⚠️ **AMA 4.10 BİR İNDEKSİ SORGUYA SOKTU:** `people_birth_date_idx` kriter 3'ün sorgusu tarafından **kullanılmıyor** (planlayıcı `people`ı Seq Scan'liyor, %35,5 seçicilik). İndeks **kaldırılmadı** — bu bir şema değişikliği olurdu ve kararın sahibi tüketici faz; **yeniden değerlendirme ROADMAP Faz 32 kapsamına** yazıldı. ℹ️ Aşağıdaki arşiv satırı `0011`in ayrıntısını taşıyor |
| **Şema durumu (4.8 arşiv)** | **22 master tablo** + **6 indeks** + **32 FK**. `0011` yalnızca iki `CREATE INDEX`, sıfır sütun. 🆕 **`0011`in `down`unda SIRA bir ZORUNLULUK DEĞİL** ve bu `0010`dan farkı: orada iki katmanlı iki zincir vardı ve CASCADE'siz `DROP TABLE` yanlış sırada reddedilirdi; burada iki indeks **birbirine bakmıyor**. `0004`ün sırasıyla da karıştırılmamalı — orada zincir gerçekti (indeks → fonksiyon → uzantı). `CASCADE` **yok**. `comparedFacts` alt sınırı **4.205 → 4.209** — **ÖLÇÜLDÜ** (sınır `9_999_999`a kondu, gerçek sayı testin reddettiği çıktıdan okundu). Artış **+4** ve 3.7'nin emsaliyle **birebir**: *"dört indeks olgusu (ad + tanım) → +8"*, yani indeks başına **2** olgu; mekanizma kodda da görünür (`diffMaps` 1 varlık + 1 alan). Sequence **14'te sabit** |
| **Şema durumu (4.7 arşiv)** | **22 master tablo** + **4 indeks** + **32 FK** (18/26 → 22/32). 🆕 **FAZ 4'ÜN ENVANTERİ KAPANDI: 11 (Faz 3) + 11 (Faz 4) = 22.** Migration zinciri **11 dosya** (`0000`…`0010`), **on birinin de elle yazılmış `down`u var**. 🆕 **`0010`un `down`unda SIRA bir tercih DEĞİL, ZORUNLU** — iki katmanlı iki zincir (`staff_attributes` → `staff`, `manager_attributes` → `managers`) ve CASCADE'siz `DROP TABLE` yanlış sırada reddedilir; `0009`dan farkı tam olarak bu. `comparedFacts` alt sınırı **3.570 → 4.205** — **ÖLÇÜLDÜ** (sınır `9_999_999`a kondu, gerçek sayı testin reddettiği çıktıdan okundu). Artış **+635**, 4.6'nın +547'sinden büyük: 48 yeni sütun, 6 FK, 3 CHECK, 2 sequence. 🆕 **Sequence 12 → 14** ve oran yine sabit değil: dört tablodan **ikisi** `serial id` aldı (`staff`, `managers`), nitelik tabloları PK=FK olduğu için almadı |
| **Veri durumu** | **4 / 22 tablo doludur** ve **4.10 bunu değiştirmedi** — B'nin 50.000 sentetik satırı **ayrı bir konteynerde** yaşıyor, seed veritabanına hiç girmedi (KARAR 3; koşan bir testle de iddia ediliyor: B'nin veritabanında `seed-player-%` anahtarlı satır **0**). ⚠️ **4.9'un tam envanter nöbetçisi GEVŞETİLMEDİ** — D6 sorusu soruldu (*"hangisi yanlış?"*) ve cevap **nöbetçi doğru** çıktı; sayı büyütülmedi, `toHaveLength(18)` gevşetilmedi. Ayrıntı aşağıdaki arşiv satırında |
| **Veri durumu (4.9 arşiv)** | 🆕 **4 / 22 tablo doludur** (`countries` 6 · `competitions` 11 · **`people` 5.000** · **`players` 5.000**); kalan **on sekizi boş**. ⚠️ **Ve bu ayrım artık TAM BİR ENVANTERLE iddia ediliyor:** `seed-query-performance.itest.ts`in eski testi *"2 tablo dolu, 20 tablo BOŞ"* diyordu ama sayımı yalnızca **dört** tabloyu soruyordu — dördü de 4.9'dan sonra aynı kalıyordu, yani **başlık yalana dönecek, iddia geçmeye devam edecekti** (kırılmaktan kötü). Sayım artık `pg_class`tan bütün tabloları dolaşıyor: yeni bir tablo dolduğu gün **hangisi olursa olsun** öter. ⚠️ **Nitelik tabloları BİLEREK boş** (`player_attributes` · `player_hidden_attributes` · `player_positions` · `player_traits` · `player_stats_history`) — dağılımın sahibi **Faz 10**, ve beşinin de boş kaldığı **koşan testlerle** iddia ediliyor. ⚠️ **5.000'in 5.000'i SERBEST OYUNCU** (`club_id` NULL, karşı kontrol dolu = 0) — `clubs` boş ve kulüp verisi Faz 8'in işi. ℹ️ Seed `source` sütununa **`procedural`** yazıyor (G-14) ve `key` öneki **`seed-player-`** — paket namespace'inden (`player-12847`, `spec/12` §17.4) **ayrı**, çünkü aynı namespace'te biri diğerini `ON CONFLICT (key)` üzerinde **sessizce** ezerdi. Bu ayrımın bedeli **G-20** |
...
| **Açık sorun sayısı** | **0** |
| **G-17 — 4.10 MARUZİYETİ BÜYÜTMEDİ** | Yeni kimlik sütunu yok; iki yeni test dosyası da kimlikleri yalnızca **JOIN koşulunda** kullanıyor (`pe."id" = p."person_id"`), yani bir `people.id`yi `player_id` sanma yolu **açılmadı**. B'nin sentetik üretimi `person_id`yi `people.key` üzerinden çözüyor (`JOIN "people" p ON p."key" = …`) — 4.9'un `scalarIdByKey` deseninin SQL tarafındaki karşılığı. Maruziyet 4.7'nin bıraktığı yerde: **`people.id` · `players.id` · `staff.id` · `managers.id`**, dördü de `integer`. Tip seviyesinde kapanması **Faz 12** |
| **G-17 (4.9 arşiv) — maruziyet İLK KEZ 5.000 SATIRA YAYILDI** | Yeni bir kimlik sütunu yok; maruziyet 4.7'nin bıraktığı yerde (**`people.id` · `players.id` · `staff.id` · `managers.id`**, dördü de `integer`). ⚠️ **Ama 4.9 hizalamayı ilk kez HACİMLE sınadı:** 5.000 `players` satırının `person_id`si 5.000 `people` satırına bağlanıyor ve **hizasızlık FK tarafından yakalanmazdı** — yanlış kişinin oyuncusu da geçerli bir satırdır. Savunma yine bir isimlendirme disiplini, ama bu kez **koşan bir iddia** eklendi: `players[i].personKey === people[i].key === seedPlayerKey(i)`, 5.000 satırın 5.000'inde. Tip seviyesinde kapanması **Faz 12** |
| **⚠️ G-17'NİN MARUZİYETİ 4.7'DE BİR KADEME DERİNLEŞTİ (arşiv)** | Şemada artık **`people.id` · `players.id` · `staff.id` · `managers.id`** var ve dördü de `integer`. `fixtures.ts`e `staffIdOfPerson(personKey, role)` yardımcısı yazıldı ve **`playerIdOfPerson`den bir bileşen daha taşıyor**: `players.person_id` UNIQUE olduğu için kişi tek başına yeterliydi, `staff.person_id` UNIQUE **değil** — rol olmadan alt sorgu ya *"birden çok satır"* ile patlar ya da **yanlış personel satırını** seçer. Bir `people.id`yi `staff_id` sütununa yazmak FK tarafından **yakalanmaz** (o kimlikte bir personel büyük olasılıkla vardır). Bugünkü savunma yine bir **isimlendirme disiplini**; tip seviyesinde kapanması **Faz 12** (G-17) |
| **⚠️ G-17 (4.6 arşiv)** | Üç yeni tablonun üçü de `player_id` ile anahtarlı, yani `playerIdOfPerson()` yardımcısı artık **beş** fixture fonksiyonunda kullanılıyor. 🆕 **VE 4.6 YENİ BİR MARUZİYET BİÇİMİ GETİRDİ:** `player_stats_history` aynı satırda hem `player_id` hem `club_id` hem `competition_id` taşıyor — **üç farklı varlığın kimliği, üçü de `integer`**. Bir `club_id`yi `competition_id` sütununa yazmak FK tarafından **yakalanır** (farklı hedef tablolar), ama bir `players.id`yi `player_id` sanmak yakalanmaz. Bugünkü savunma yine bir **isimlendirme disiplini**; tip seviyesinde kapanması **Faz 12** (G-17) |
| **✅ 4.3'ün RİSKİ ARTIK ATANDI — G-17** | `people.id` ↔ `players.id` karışması (ikisi de `integer`, FK yakalayamaz) **G-17 olarak açıldı ve Faz 12'ye atandı**; kütüğe **ve** ROADMAP Faz 12 kapsamına yazıldı. ⚠️ **4.3'ün önerisi (*"şimdilik yazma"*) yanlıştı ve kullanıcı itiraz etti:** K12 **uygulamayı** yasaklıyor, **kaydetmeyi** değil — ve *"ANLIK DURUM'da kayıtlı, kaybolmuyor"* argümanı bu bloğun kendi başlığıyla çelişiyordu (alt görev başına yeniden yazılır, SAPMA-004). **Ders: bir riskin kalıcı yeri kütüktür; ANLIK DURUM bir kayıt yeri değil, bir durum fotoğrafıdır.** |
| **Teknik borç sayısı** | **8** — **4.10 da hiçbir borç açmadı ve BORÇ-008'i ödemedi** (vadesi **4.11**; 4.10 hiç CHECK yazmadı, kopya sayısı **dokuzda sabit**). BORÇ-001, 002, 004 (Faz 16) · BORÇ-003, 005 (Faz 5) · **BORÇ-007 (Faz 12)** · BORÇ-006 (Faz 50) · **BORÇ-008 (Faz 4.11)**. 🆕 **Ve 4.11'e İKİNCİ bir iş sahiplendirildi** (borç değil, düzeltme): `referees.ts:23`ün karşılıksız `SeededRng` cümlesi — günlük #36'da bulunmuştu, 4.10'da **ROADMAP 4.11 satırına** yazıldı. Gerekçe: *"çalışma günlüğü bir yapılacaklar listesi değildir"* — faz kapanışında boşaltılıp kayda işleniyor, yani orada bir **iş** olarak yaşamazdı |
| **Teknik borç (4.7 arşiv)** | **8** — **4.7 birini açtı:** CHECK değer listesini SQL literaline çeviren ifade **dokuz** şema dosyasında tekrarlanıyor (beşi adlandırılmış yerel yardımcı, dördü satır içi) ve 4.7 bunu 7'den 9'a çıkardı. Ortaklaştırma altı dosyaya dokunuyor → K12 gereği ertelendi, **ama rapora değil KÜTÜĞE yazıldı**: raporda kalan bir gözlemin sahibi yoktur (G-17'nin dersi) |
| **Teknik borç (4.6 arşiv)** | **7** — BORÇ-001, 002, 004 (Faz 16) · BORÇ-003, 005 (Faz 5) · **BORÇ-007 (Faz 12)** · BORÇ-006 (Faz 50). 🆕 **4.0: üçüne "gerekçe Faz 16'da yeniden türetilir, kopyalanmaz" notu düşüldü** — *"6.0.0 kurulum anında 3 haftalıktı"* argümanı Faz 16'da geçersiz olacak. Ölçüldü: `ioredis`/`bullmq` **hiçbir `package.json`'da, katalogda veya `pnpm-lock.yaml`'da yok** (0 eşleşme), yani kayıt bir kilit değil bir **sürüm kararı** |
| **SAPMA sayısı** | **34** (SAPMA-001…034) — **4.10 da hiçbir SAPMA açmadı.** Aday değerlendirildi ve **sapma çıkmadı**: ROADMAP 4.10'un *"`reltuples != -1` denetlenir"* cümlesi yanlıştı **ama düzeltmesi 4.9'da zaten yapılmıştı** (madde ROADMAP'te tabloyla birlikte duruyor), yani bu bir **açık kayıt**, yeni bir sapma değil. ⚠️ Ve `people_birth_date_idx`in kullanılmaması da **SAPMA DEĞİL**: `spec/01` §3.1'in indeks satırı bir **niyet beyanı** ilan edilmişti (4.1 kararı, SAPMA-031 kapsamında) ve 4.8 zaten *"planlayıcının seçtiği iddia edilmiyor"* diye yazmıştı — yani spec ile yapılan iş **çelişmiyor**, ölçüm bir açık soruyu **kapatıyor**. Faz 4'ün payı **yedi** (028…034) |
| **SAPMA (4.9 arşiv)** | **4.9 da hiçbir SAPMA açmadı.** Aday değerlendirildi ve **sapma çıkmadı**: `world-seed-data.ts:19`un *"`SeededRng`i `packages/engine`den `packages/shared`a taşımak"* cümlesi var olmayan bir modülü ima ediyordu ve ölçümle çürütüldü — **ama bu bir spesifikasyon sapması değil, bir BAŞLIK METNİ hatası.** SAPMA, `spec` ile yapılan iş arasındaki farkı kaydeder; burada `spec`in bir dediği yok, yalnızca bir **gerekçenin metni** yanlıştı ve savunduğu karar (bir RNG yazmama) **değişmedi**. BORÇ-001'in *"5.11.1'de tutuldu"* vakasının aynı sınıfı. Cümle düzeltildi ve **düzeltmenin kendisi görünür bırakıldı** — alıntılanmış eski hâli sonraki taramalarda ihlal gibi görünürdü. ℹ️ Aynı sınıfın `referees.ts:23`teki kardeşi **düzeltilmedi** (K12, kapsam dışı) ve günlük **#36**'ya yazıldı. Faz 4'ün payı **yedi** (028…034) |
| **Boşluk sayısı** | **20** (G-01…G-20) — **4.10 hiçbir boşluk açmadı veya kapatmadı.** ⚠️ **Bir aday CİDDİ OLARAK değerlendirildi ve BOŞLUK ÇIKMADI:** 4.8'in 4.10'a bıraktığı *"`people.birth_date` kısmi indeks (`WHERE person_type @> ARRAY['player']`) gerekebilir"* sorusu. **Bugün ÖLÇÜLEMEDİ ve sebebi yazıldı (D1):** `people`ın **5.000/5.000'i oyuncu**, yani o yüklem **hiçbir satırı elemiyor** ve kısmi indeksin kazancı bu veride **tanımsız**. **Sahip aranırken üç faz kapsamı okundu** — Faz 8 (`chairman` yazıyor) ve Faz 37 (personel popülasyonu) **indeks kararının yeri değil**; **Faz 32** ise kabul kriterine *"Faz 4'ün kriterinin **tam hâli** ölçüldü … bileşik indeksin gerçek transfer aramasını taşıdığı **burada** kanıtlanır"* + *"50.000 oyuncuda tüm filtreler < 300 ms"* yazmış. **Yani işin sahibi ZATEN VARDI ve bir G-satırı bunu tekrarlamaktan başka bir şey yapmazdı.** Emsal de aynı fazı gösteriyor (`spec/01` §3.1: tekil indeksler *"tüketicisi olan fazda (Faz 32) değerlendirilir"*). ⚠️ **Ama "sahibi var" demek YETMEZ** — ölçüm (plan tablosu + üç somut soru) **ROADMAP Faz 32 KAPSAMINA** yazıldı; rapora yazmak hiç yetmezdi. ⚠️ **G-16 · G-17 · G-20 4.10'da da KORUNDU** |
| **Boşluk (4.9 arşiv)** | 🆕 **20** (G-01…G-20) — **4.9 BİRİNİ AÇTI: G-20.** *"4.9'un 5.000 prosedürel oyuncusunun **ömrü tanımsız**."* Ülke/yarışma seed'inden **yapısal olarak farklı** ve fark ölçüldü: o satırlar gerçek veriyle **aynı `key`i** taşıyor, yani Faz 8 ingesti `DO UPDATE` ile üzerlerine yazıyor; oyuncu satırları ise ayrı namespace'te, yani Faz 9'un `player-*` anahtarları onları **ezmez, yanlarına ekler**. İki sayım birden bozuluyor: *"serbest oyuncu havuzu ~300"* → **5.300**, *"3.500+ oyuncu"* → **8.500** üzerinde ölçülür. ⚠️ **SAHİP TAHMİNLE ATANMADI (G-18'in dersi) — üç aday kapsamı OKUNDU:** **Faz 10 yapamaz** (girdisi `player_stats_history`, bu satırların istatistik geçmişi yok) · **Faz 11 yapamaz** (40+ kuralda *"her oyuncunun niteliği var"* yok, ve doğrulayıcı **rapor eder, silmez**) · **Faz 9 yapabilir** (`players`a yazan faz, ve iki sayı **kendi kriterleri**). ROADMAP Faz 9 kapsamına **üç seçenekle** ve bir **kabul kriteriyle** yazıldı; karar bugün verilmedi (K13). ⚠️ **G-16 ve G-17 4.9'da da KORUNDU.** Önceki üç tur (4.6, 4.7, 4.8) hiçbir boşluk açmamıştı |
| **Boşluk (4.8 arşiv)** | **4.8 hiçbir boşluk açmadı veya kapatmadı.** İki aday değerlendirildi ve ikisi de **boşluk çıkmadı**: ① *"`people.birth_date` indeksi seçici olmayabilir — `people` oyuncu dışı kişileri de taşıyor, kısmi indeks (`WHERE person_type @> ARRAY['player']`) gerekebilir"* → bu bir **boşluk değil bir ölçüm sorusu** ve sahibi **4.10** (kriter 3'ün ölçümü, `ANALYZE` + seçicilik). Bugün kısmi indeks yazmak, ölçülmemiş bir tahmini şemaya çakmak olurdu; 3.9'un kendi dersi *"ayraç hacim değil seçicilik"* ve o ayraç henüz koşturulamıyor ② *"`player_positions` üzerinden mevki araması indekssiz"* → kriter 3'ün sorgusu `players.primary_position` diyor (`spec/01`'in 4.1 kararı) ve mevki matrisi üzerinden arama **Faz 32**'nin işi — `spec/01` `(finishing)`/`(passing)`/`(pace)` için birebir aynı gerekçeyi zaten yazmış (*"tüketicisi olan fazda değerlendirilir"*). ⚠️ **G-16 ve G-17 4.8'de de KORUNDU:** `managers.user_id` ve markalı kimlik yazılmadı. ⚠️ **`SPEC-COVERAGE-GAPS`'ta Faz 4'e atanmış hiçbir G-satırı YOK** (ölçüldü: atamalar Faz 2·3·5·6·7·11·12·13·17·47·49·50) — yani bu faz başında okunacak satır yoktu. Önceki iki tur (4.6, 4.7) da hiçbir boşluk açmamıştı |
| **⚠️ 4.5'İN İKİNCİ DERSİ — yanlış bir atamanın altında İKİ boşluk yatar** | G-18 bir **anlam** boşluğuydu (*"hakemin `person_type`ı ne?"*), G-19 bir **sahiplik** boşluğu (*"hakem satırlarını kim yazacak?"*). 4.4'te ikisi tek bir boşluk gibi görünüyordu çünkü ikisi de *"hakem verisi Faz 8'de gelir"* varsayımına yaslanıyordu — **o varsayım ölçülünce ikisi birden ayrıştı**. Genel biçim: bir atamayı **doğrulamak**, atamanın konusundan ayrı ikinci bir boşluğu görünür kılar. Ve o ikinci boşluk ancak *"hedef faz bu işi YAPABİLİYOR mu?"* diye sorulduğunda ortaya çıkıyor — **kapsam taşıması kütüğe yazmakla bitmez** |
| **Faz 4 kabul kriterleri** | 🆕 **5 / 6 — 4.10 KRİTER 3'Ü KAPATTI.** *"«20–24 yaş, sağ bek, CA>120» sorgusu 5.000 oyuncu hacminde < 50 ms"* → `[x]`. Ölçüm **istatistikle** alındı, istatistiğin varlığı **`last_analyze` ile** denetlendi (+ karşı kontrol `n_live_tup`), süre bir **dağılım** olarak raporlandı (medyan **0,425 ms** · en kötü **0,461 ms** · ısıtmasız ilk koşu **0,426 ms**), ölçüm aracı `TIMING OFF` ile doğrulandı ve plan **kararlılığı** ayrıca iddia edildi. ⚠️ **Kriter sağlandı ama süre tarafının ÖNEMSİZ olduğu SAKLANMADI** — koşan bir test aynı sorgunun **indeksler kapalıyken de** bütçenin altında kaldığını gösteriyor. **KALAN TEK KRİTER: 6** (şema dokümanı → **4.11**). *"Yapılmayan «yapıldı» yazılmaz"* |
| **Faz 4 kabul kriterleri (4.9 arşiv)** | **4 / 6 — 4.9 KRİTER 1'İ KAPATTI.** *"5.000 sahte oyuncu seed → şema tutarlı"* → `[x]`. Tutarlılık **üç katmanda** ölçüldü: ① `0007`nin iki CHECK'i **inşa gereği** sağlanıyor (`clamp(CA, 200, …)`) **ve** kısıtların gerçekten **ısırdığı** ihlal eden **tek** satırla ayrıca gösterildi — *"5.000 satır girdi, patlamadı"* tek başına **kör bir kontroldü** ② FK'ler anahtarla çözüldü (`nationality_country_id` `NOT NULL`, çözülemeyen anahtar **gürültülü** patlardı; `competitions.country_id` nullable olduğu için orada aynı hata **sessiz** geçerdi — fark yazılı) ③ D5 **54/54**. ⏳ Kalan: **kriter 3** (4.10) ve **kriter 6** (4.11). `docs/schema/world.md` bu turda **yeniden üretilmedi** ve gerekmedi — 4.9 şemaya dokunmadı, `er-diagram.itest.ts` **geçti**, blok bayatlamadı. **Yapılmayan "yapıldı" yazılmaz** |
| **Faz 4 kabul kriterleri (4.7 arşiv)** | **3 / 6 — 4.7 de hiçbirini kapatmadı ve bu BEKLENEN.** `docs/schema/world.md` o turda **programatik olarak** yeniden üretildi (22 tablo / 32 FK, blok elle düzenlenmedi) ve mermaid **dışı** prose sayıları da güncellendi — ama faz sürerken yine bayatlayacak, kriter 6 hâlâ `[ ]` |
...
|---|---|---|---|---|---|
| 38 | 4.10 | (hata değil, **ölçüm bulgusu**) 4.8'in iki indeksinden **biri** kriter 3'ün sorgusu tarafından **kullanılmıyor** — `people_birth_date_idx` yerine `Seq Scan on people` | Yaş penceresi `people`ın **%35,5**'ini tutuyor (1.776 / 5.000); planlayıcı o kadar satırı indeks üzerinden okumaktansa taramayı seçiyor — **ve haklı**. `players` tarafı **%1,5** seçici olduğu için bileşik indeksi **kullanıyor**. Yani `0011`in iki indeksi **aynı sorguda, aynı hacimde zıt** davranıyor | İndeks **kaldırılmadı** (K12 + KARAR 6: 4.10 şemaya dokunmaz) ve karar **uydurulmadı**. Plan **koşan iki testle** iddia edildi (biri *"bileşik indeks kullanılıyor"*, biri *"`people_birth_date_idx` KULLANILMIYOR ve bu DOĞRU karar"*) — ikincisi olmadan gelecekte biri planı görüp *"indeks çalışmıyor"* diye bir regresyon sanabilirdi. Yeniden değerlendirme **ROADMAP Faz 32 kapsamına** plan tablosu + üç somut soruyla yazıldı | ⚠️ **VE MUTASYON GÖSTERDİ Kİ BÜTÇE TESTİ BUNU GÖREMEZDİ:** `ANALYZE` kaldırılınca **2 / 78** kırılıyor ama `< 50 ms` testleri **kırılmıyor** — istatistiksiz, dolayısıyla **yanlış** alınmış bir ölçüm bütçeyi kusursuz geçiyor. **Genel kural: geçen bir bütçe testi ölçümün DOĞRU ALINDIĞINI göstermez; onu ancak ölçümün ÖN KOŞULUNU iddia eden ayrı bir nöbetçi gösterir** (4.9'un *"determinizm testi tek başına nöbetçi değildir"* kuralının ölçüm tarafındaki kardeşi). `spec/09` §11.5'e işlendi |
| 37 | 4.10 | **ADIM 0 ④ İLK KEZ ISIRDI: Docker çalışmıyordu.** `docker version` çıktısında **`Server:` bölümü yok** (`failed to connect to the docker API at npipe:…`), `wsl -l -v` → `docker-desktop **Stopped**` | Ortam durumu; kod dışı. ⚠️ **Ama 4.10 için sıradan bir eksiklik değildi:** alt görevin **tamamı** bir veritabanı ölçümü (A · B · `test:db` · sonda betikleri — hepsi Docker'a bağlı), yani kapı olmasa iş **yanlış yerden** başlayacaktı | Önce Docker'a bağlı **olmayan** her şey koşturuldu (statik kapılar · birim testleri · kapsam artefaktı · CI liste sorgusu · okuma listesi), sonra Docker Desktop başlatıldı ve hazır olduğu **kanıtlandı** — bekleme uykuyla değil bir **kontrol komutuyla** yapıldı (`docker info` döngüsü). Sonuç: `Server: Docker Desktop 4.88.1`, Engine **`linux/amd64`**, `docker-desktop Running` | **Kapı yazıldığı gibi çalıştı ve bedeli sıfır oldu.** ℹ️ Bir yan kazanç: aynı kontrol NOT ⑩'un girdisini de **ölçmüş** oldu (mimari **amd64**), yani *"üretim ARM64"* etiketi hafızadan değil ölçümden geldi. **Ders: bir ön koşul kontrolü, işin ilk adımı olduğunda ücretsizdir; ortasında olduğunda pahalıdır** |
| 36 | 4.9 | (hata değil, **kaydedilen bir tutarsızlık**) `packages/db/src/schema/referees.ts:23` *"`SeededRng` deterministik bir anahtar **veriyor**"* diyor — **şimdiki zamanda**, var olmayan bir sınıf için | `SeededRng` repoda **hiçbir yerde yok** (ölçüldü: `packages/engine/src/index.ts` gövdesi `export {};`, sınıf yalnızca beş yorum satırında geçiyor); `DebugPanel.tsx:73` doğrudan *"Faz 22'de gelecek"* diyor. Cümle 3.6'da yazıldığında da karşılıksızdı | **Düzeltilmedi** — `referees.ts` 4.9'un kapsamı değil (K12). Aynı sınıfın `world-seed-data.ts:19`daki kardeşi **düzeltildi**, çünkü o dosya bu alt görevin kapsamındaydı ve düzeltme **görünür** bırakıldı (*"DÜZELTME (Faz 4.9) — bu paragraf var olmayan bir modülü ima ediyordu"*) — alıntılanmış eski hâli sonraki taramalarda ihlal gibi görünürdü | **Kaydedilmemiş bir risk, kaydedilmiş bir borçtan tehlikelidir.** Kapsam dışı bir tutarsızlığın yeri rapor değil **kütük/günlük**. ⚠️ Ve genel biçim: *"gelecekte gelecek"* bir şeyi **şimdiki zamanda** anlatan bir yorum, o şey gelene kadar **sessizce yanlıştır** — hiçbir kapı yorumları denetlemiyor |
| 35 | 4.9 | D5, entegrasyon testinde **yeşil** olan bir iddiayı gerçek veritabanında **kırdı**: `players.reltuples` `-1` bekleniyordu, **`5000`** döndü | (hata değil, **bir kuralın SINIRININ yanlış bilinmesi**) 3.9'un `ANALYZE` dersi *"`reltuples = -1` → planlayıcı kör"* diye bir **DURUM** olarak öğrenilmişti; ölçüm onun bir **YARIŞ** olduğunu gösterdi. `autovacuum=on`, `naptime=60s`: 5.000 satırlık INSERT autoanalyze eşiğini aşıyor ve istatistik **kimse `ANALYZE` çağırmadan** ~15 sn içinde doluyor. Testcontainers'ta test yarışın **kısa tarafında** bittiği için `-1` görüyordu | Mekanizma **tahmin edilmedi**, ayrı bir sonda betiğiyle dört anda ölçüldü (migration sonrası / seed sonrası / +15 sn / +60 sn). İddia **değişmez** olana çevrildi: `last_analyze IS NULL` (*"seed `ANALYZE` çağırmıyor"*) — o alan yalnızca **elle** `ANALYZE` ile doluyor, +60 sn'de bile `YOK`. Karşı kontrol de eklendi (`n_live_tup = 5000`) | ⚠️ **VE BULGU 4.10'UN YÖNTEMİNİ DEĞİŞTİRDİ:** ROADMAP 4.10 *"`ANALYZE` şart (`reltuples != -1` denetlenir)"* diyordu; o denetim **`ANALYZE` hiç çağrılmadan da yeşil verir** — D3'ün ölçüm tarafındaki biçimi. Not tabloyla birlikte **ROADMAP 4.10 kapsamına** yazıldı (rapora yazmak yetmez — G-17'nin dersi). **Genel kural: zamana bağlı bir değeri iddia eden test, o değerin YARIŞ olup olmadığı ölçülmeden yazılmaz.** D5'in değeri üçüncü kez buradan geldi (#30, #32, #35) |
| 34 | 4.9 | Seed gerçek PostgreSQL'e yazarken **`integer out of range`** ile patladı; `pnpm test:db` 13 test birden kırıldı | Üreteç `mix32` sonunda `>>> 0` ile **işaretsiz** 32 bit döndürüyordu (tavan 4.294.967.295); `people.portrait_seed` ise `integer`, yani **işaretli** 32 bit (tavan 2.147.483.647). Aradaki fark yalnızca üst bit | `>>> 1` — üst bit düşürülüyor, değer her zaman `[0, 2^31−1]`. Nöbetçi **birim testine** kondu ve **karşı kontrolüyle** birlikte: tavana yakın değerlerin gerçekten üretildiği ayrıca iddia ediliyor, yoksa *"hepsi küçük sayı"* diye bedavaya geçerdi | **Ders *"tavanı kontrol et"* DEĞİL:** bir üretecin çıktı aralığı **hedef sütunun TİPİNDEN** okunur — jeneratörün *"32 bit"* demesi `integer`ın ne dediğini söylemiyordu. Şemanın iki temsili var ve burada doğru olan **şema dosyası** (`integer('portrait_seed')`). ℹ️ Bedel düşük kaldı çünkü hata **sessiz olamazdı** |
| 33 | 4.8 | D5 betiği iki kez **imza hatasıyla** patladı: `executor.run is not a function`, sonra `Cannot read properties of undefined (reading 'steps')` | (hata değil, **derlenmiş yüzeye karşı yazmanın bedeli**) İki çağrı da **hafızadan** yazılmıştı: `createPostgresExecutor` bir `sql` nesnesi değil bir **URL** alıyor ve `{ executor, close }` döndürüyor; `migrateDown` bağlamı ve seçenekleri **ayrı argüman** olarak istiyor (`migrateDown({executor,source,logger}, {steps})`) | İkisi de kaynaktan okundu — `dist/migrate/postgres-executor.js` gövdesi ve `runner.itest.ts`teki gerçek çağrı yerleri → D5 **23/23** | ⚠️ **D5'in değeri ve bedeli aynı yerden geliyor:** betik bilerek derlenmiş `dist`e ve düz `node`a karşı yazılıyor, yani TS tiplerinin koruması **yok** — bir imza hatası ancak çalışma anında görülüyor. İkisi de **gürültülü** patladı ve bu iyi; sessiz olsalardı D5'in ölçtüğü şey anlamsızlaşırdı. **Kural: `dist`e karşı yazılan her çağrının imzası kaynaktan okunur** — bir entegrasyon testindeki çağrı yeri en ucuz kaynaktır |
| 32 | 4.8 | D5 betiği `column "work_permit_rule" of relation "countries" does not exist` ile patladı | (hata değil, **#30'un TEKRARI — ikinci vaka**) `countries` INSERT'ünün sütun adları **hafızadan** yazıldı: `work_permit_rule` · `league_reputation` · `is_active`. Gerçek şema `work_permit_rule_key` · `football_level` · `currency_code` diyor | Zorunlu sütun kümesi `information_schema.columns`tan **okundu** (9 sütun, `is_nullable='NO' AND column_default IS NULL`), CHECK değerleri `pg_constraint`ten alındı (`tr_quota`) → D5 **23/23** | ⚠️ **#30 BUNU ÖNGÖRMÜŞTÜ ve yine oldu:** *"D5 fixture kullanmıyor, ham SQL yazıyor — fixture'ın koruması orada yok, bedeli bu."* Bedel **düşük** çünkü hata sessiz olamaz: eksik bir sütun `INSERT`i durdurur. **Ama tekrarın kendisi bir sinyal:** D5 betiği her turda sıfırdan yazıldığı için `fixtures.ts`in fazlar boyunca biriktirdiği bilgiyi **hiç görmüyor** ve aynı tuzağa her turda yeniden açık. ℹ️ Çözüm *"fixture'ı kullan"* **değil** — D5'in bütün anlamı fixture'ı atlamak; çözüm sütun listesini her turda **katalogdan okumak**, hatırlamaya çalışmamak |
| 31 | 4.7 | **`pnpm build` sekiz yerine İKİ görev koştu ve `packages/db/dist/schema/staff.js` bulunamadı** — bir an *"build kırık"* diye okundu | (hata değil, **ölçüm aracının kendi hatası — D2, bu fazda DÖRDÜNCÜ kez**) `Bash` aracının çalışma dizini **çağrılar arasında kalıcı**; üç komut önce `cd /c/fms/packages/db/drizzle` yapılmıştı (migration SQL'inin tip ailelerini saymak için) ve geri dönülmemişti. `pnpm build` oradan koşunca **`packages/db` çalışma alanını** çözdü, yani 8/8 değil 2/2. `rm -rf .turbo/cache` de kök yerine o dizinde koştu (orada böyle bir klasör yok — zararsız) | Kök dizine dönüldü, ağacın sağlamlığı **ölçüldü** (11 migration + 11 `down` + `0010_snapshot.json` + `git status`), soğuk build yeniden koşuldu → **8/8, `Cached: 0`, 11,16 s** ve dört yeni `dist` dosyası adıyla doğrulandı | ⚠️ **Bir ölçümün ÇIKTISI kadar ZEMİNİ de doğrulanır.** `Tasks: 2` satırı aslında doğru bir cevaptı — yanlış olan **soru**ydu. Ve tehlikeli tarafı: `rm -rf` gibi bir komut yanlış dizinde koşarsa sessizce başka bir şey silebilirdi. **Kural: kalıcı bir çalışma dizini değiştirildiğinde, ondan sonraki ilk komut ya mutlak yol kullanır ya da `cd` ile geri döner** — ve `Tasks:` gibi bir sayı beklenenden farklıysa önce **nerede koştuğu** sorulur |
| 30 | 4.7 | D5 betiği `column "tla" of relation "clubs" does not exist` ile patladı | (hata değil, **doğru mekanizmanın çalışması**) `clubs`a satır yazmak için sütun adları **hafızadan** yazılmıştı (`tla`, `color1`, `color2`); gerçek şema `abbreviation`, `color_primary`, `color_secondary` diyor ve ayrıca `city` · `crest_seed` · `supporter_count` · `supporter_expectation` **`NOT NULL`** | Sütun listesi `information_schema.columns`tan **okundu** (24 sütun, sıra ve nullability ile) ve `INSERT` ona göre yazıldı → D5 **25/25** | ℹ️ **Entegrasyon testleri bu tuzağa DÜŞMÜYOR çünkü `clubInsertSql` fixture'ını kullanıyorlar** (günlük #28'in kararı: *"`clubs` 24 sütunlu ve `NOT NULL` listesi fazlar boyunca büyüdü, fixture tek yer"*). D5 bilerek `dist`i ve **ham SQL**'i kullanıyor, yani fixture'ın koruması orada yok — bedeli bu. **Gürültülü patladı ve bu iyi:** sessizce yanlış satır yazsaydı CASCADE ölçümü anlamsız olurdu |
| 29 | 4.7 | (hata değil, **ölçüm bulgusu**) `spec/09` §11.5'in mutasyon serisi **4.5'te bitiyordu**; 4.6'nın ölçümü (**27 / 230**) tabloda **yoktu** | Ölçüm 4.6'da yapıldı, ANLIK DURUM'a ve alt görev raporuna yazıldı, ama serinin **kalıcı yerine** işlenmedi. Dosyanın tamamında `4.6` → **0 eşleşme** (ölçüldü) | Eksik **4.6** ve yeni **4.7** satırları birlikte eklendi; ayrıca `OUTPUT-FORMAT`ın kurallarına *"şemaya dokunan bir alt görevde mutasyon serisi §11.5'e işlendi mi?"* maddesi yazıldı | ⚠️ **F1'in kaydın kendisine uygulanmış hâli:** *"envanterler kör kalmaz, ama GÜNCELLENMEZSE bayatlar."* Ve çare bir kez daha yazmak değil, **kontrol eden bir adım** (SAPMA-033). ℹ️ Kaydın bayatlaması **sessizdi**: seri 4.5'te donmuş görünüyordu ve hiçbir kapı bunu göremezdi — bir belgeye yazılmamış bir ölçüm, yapılmamış bir ölçümden ayırt edilemez |
| 28 | 4.7 | **`stepsBackTo('0009…')` artık İKİ adım** ve üç testin iddiası tek satır değişmeden yanlış hâle geldi (üç tablo → yedi, beş → dokuz, on yedi → yirmi bir) | (hata değil, **tasarım sonucu — #16'nın DÖRDÜNCÜ vakası**) `stepsBackTo` zincirden **türetiliyor** (4.3'ün kazancı) ve `0010` üstüne eklenince aynı çağrı derinleşti. Türetilmiş sayı doğru kaldı, **iddia ve başlık** sayıyla birlikte kaymadı | Üç testin beklentisi ve **başlığı** birlikte güncellendi (*"yalnızca 0009"* → *"0009'a kadar"*), ve her birine sınıfının 4.7'de değiştiği yazıldı | ⚠️ **Desen artık dördüncü kez tekrarlıyor ve maliyeti öngörülebilir:** zincire her yeni migration, **ondan daha eski** her kısmi geri almanın iddiasını genişletiyor. Yeni bir migration yazarken sorulacak soru üçlü: *"hangi testler kırıldı?"* · *"hangi testlerin SINIFI değişti?"* · *"hangi sınıfın son örneği kayboldu?"* (#16 · #23) |
| 27 | 4.7 | (hata değil, **ölçüm bulgusu**) Devir notunun iki sayısı **bayattı**: `narrow…` çağrı yeri **15** deniyordu, gerçek **17**; `seedAllTables()` **16** deniyordu, gerçek **18** | 4.6'nın raporu sayıyı alt görevin **ortasında** ölçmüş, sonra `0009`un iki testi daha eklenmişti. Yapısal iddia (*"biri bilerek çağırmıyor"*) doğruydu ve **hâlâ doğru** — bozulan yalnızca sayılardı | Kaynaktan yeniden sayıldı (`grep -c '^\s*await narrow…'`) ve sarmalayıcı **17** çağrı yerine göre yazıldı; fark commit mesajına ve dosya başlığına yazıldı | ⚠️ **D7'nin taze bir vakası — ve bu kez kendi ÖLÇÜMÜMÜZ bayatladı, tahminimiz değil.** Bir ölçüm doğru yapılmış olabilir ve yine de **yazıldığı andan sonra** yanlışa dönebilir. **Kural: bir devir notundaki sayı, kullanılacağı gün YENİDEN sayılır** — özellikle o sayı bir *"kaç yer güncellenecek"* sayısıysa |
| 26 | 4.7 | (hata değil, **kablolama doğrulaması**) `bash-text-guard` kancasının kablolaması 4.6'da **ölçülememişti** (`.claude/settings.json` oturum başında okunuyor, o oturum onsuz başlamıştı) | — | Bu oturum kancayla başladı ve **üç yönlü** sınandı: ① heredoc + Türkçe + ters tırnak → **engellendi** (çıkış 2, dosya **oluşmadı**) ② `node -e` + Türkçe → **engellendi** ③ saf ASCII heredoc → **geçti** | ✅ **4.6'nın açık bıraktığı tek kalem kapandı.** Engelleme mesajı taşıyıcıyı ve yükü **adıyla** bastı; ③ olmasaydı ①② hiçbir şey kanıtlamazdı (kanca her şeyi kilitliyor olabilirdi — D3). ⚠️ Ve kanca bu alt görevde **gerçek işe yaradı**: `schema-constraints.itest.ts`e blok eklerken saf ASCII bir heredoc bilerek kullanıldı ve **geçti**, Türkçe içerik ise `Edit` ile yazıldı |
| 25 | 4.6 | (hata değil, **kural ihlali — DÖRDÜNCÜ vaka, ve kanca YAZILDIKTAN SONRA**) Bir test bloğunu dosyaya eklemek için refleks olarak `cat >> dosya << 'SQLEOF'` yazıldı; ayrıca mutasyonu uygulamak için `python3 - << 'PYEOF'` kullanıldı | ⚠️ **Kanca aynı oturumda yazılmıştı ve ATEŞLENMEDİ** — `.claude/settings.json` oturum **başında** okunuyor, bu oturum onsuz başladı. Yani kuralın *"ateşlendiği anda görünür olması"* hedefi doğru kurgulandı ama **bu oturumda hâlâ görünmüyor** | ① `cat >>` vakası: yük **boştu**, `git diff --stat` + `od -c` → **0 bayt** eklendi, içerik `Edit` ile yazıldı ② `python3` vakası: yük **saf ASCII kod**, Türkçe metin yok — kancanın kapsamı bunu **bilerek** geçiriyor (iki koşul: taşıyıcı **ve** yük) | ⚠️ **İKİ AYRI DERS. ① Kanca DOĞRU KALİBRE:** ikinci vaka gerçekten zararsızdı ve kural onu ayırıyor; kapsamı genişletmek meşru ASCII kullanımlarını da engellerdi. ② **Ama refleksin kendisi dördüncü kez tekrarladı** ve bu, kancanın **kablolanmasının** neden asıl mesele olduğunu gösteriyor: kural yazılı, kanca yazılı, kanarya yeşil — ve refleks yine ortaya çıktı çünkü **hiçbir şey ötmedi**. ⏳ Kablolamanın doğrulanması **bir sonraki oturumun ilk işi** |
| 24 | 4.6 | **Mutasyon serisi yine ALARM verdi: 26 / 229 — pay 4.5'in değerinde SABİT** (`0009` yazıldı, çevrim testi eklendi, pay hiç artmadı) | **4.5'in dersinin birebir tekrarı ve bu sefer ÖNGÖRÜLMÜŞTÜ.** `0009`un çevrim testi `differences: []` iddia ediyor — **boş** bir envanter; körelen karşılaştırıcı tam olarak onu üretiyor. Diğer yeni testler (`compareSchemas` çağırmayan davranış/CHECK/kayıp testleri) mutasyonun ölçtüğü yola hiç dokunmuyor | Bir **negatif** test yazıldı: `⑤ SESSİZ bozuk down (BİLEŞİK PK)` — bir `down`un bileşik PK'yi **tek sütunlu** geri koyduğu sessiz vaka. `0009` zincire ilk kez bileşik PK'yi getirdi, yani `constraint.definition` yeni bir **yapı** taşıyor. Pay **26 → 27** (payda 230) | ⚠️ **Kural üçüncü kez doğrulandı: payı artıran şey testin SAYISI değil, bir FARK BEKLEMESİDİR.** 4.3 (`udtName`), 4.5 (`constraint.definition`), 4.6 (bileşik PK) — üçü de aynı biçim: şemaya yeni bir **olgu türü ya da yapı** girdiğinde, onu ölçen negatif test yazılmazsa karşılaştırıcının o alanı okuduğu yalnızca **varsayılır** (D3). ℹ️ Alarmın bu kez **beklenmesi** ölçümü ucuzlattı: 4.5'in notu okunmuştu, alarm sürpriz değildi |
| 23 | 4.6 | **`SIFIR kayıp` testi kırıldı — ve sebebi bir testin bozulması değil, bir SINIFIN ZİNCİRDEN KAYBOLMASI** | (hata değil, **tasarım sonucu — #16'nın en keskin biçimi**) 4.5 `0008`i *"`loss.ts`in dördüncü sınıfı: sıfır kayıp, zincirde ilk"* diye ölçmüştü ve `stepsBackTo('0008')` o gün **tek** adımdı. `0009` zincirin üstüne eklenince aynı çağrı **iki** adım oldu ve `0009` üç tablo düşürüyor — yani gerçek zincirde **sıfır kayıp üreten hiçbir geri alma kalmadı**. Sınıf bozulmadı, **erişilemez** oldu | Vaka bir **`fixtureChain`** ile korundu (dosyanın kendi deseni: gerçek zincire test migration'ı eklemek yerine geçici bir zincir yazılır) — kısıt-only bir `down`, `allowDataLoss` verilmeden geçiyor. Ayrıca `0008`in **kendi katkısının sıfır olduğu** ayrı bir testle iddia edildi: üç tablo düşüyor, **sütun kalemi yok** | ⚠️ **#16 bir testin SINIFININ değişebileceğini söylüyordu; bu vaka bir sınıfın TEK ÖRNEĞİNİN ortadan kalkabileceğini gösteriyor.** İkisi arasındaki fark önemli: birincisinde iddia güncellenir, ikincisinde **nöbetçi kaybolur** ve kaybı hiçbir şey söylemez — test yeşile dönerdi eğer beklenti gevşetilseydi. **Kural: bir migration zincire eklenirken *"hangi testlerin sınıfı değişti?"* yanına *"hangi sınıfın son örneği kayboldu?"* de sorulur** |
| 22 | 4.6 | **`drizzle snapshot’ı gerçek şemayı doğru anlatıyor` testi kırıldı:** `missingInSnapshot` dört kalem — `player_positions.primaryKeyColumn` ve `player_traits.primaryKeyColumn` | **İKİ AYRI KÖRLÜK, ve ikincisi birincisini düzeltirken bulundu.** ① `drizzle-kit` tek sütunlu PK'yi sütunun `primaryKey: true` alanında, **bileşik** olanı ayrı bir `compositePrimaryKeys` nesnesinde yazıyor — ve o durumda her sütunun `primaryKey`i **`false`** (`0009_snapshot.json`dan okundu). `snapshotToFacts` ikincisine hiç bakmıyordu. ② Alan eklendikten sonra test **yine** kırıldı: `compareSnapshotToReal` olguları `new Map(facts.map(f => [f.path, f]))` ile indeksliyor, yani **aynı yolda tekrarlanan olgularda sonuncusu kazanıyor** ve öncekiler sessizce kayboluyor — çok sütunlu PK zaten `realSchemaToFacts` tarafında da böyle üretiliyordu | PK artık **tek bir olgu**: `primaryKeyColumns` (çoğul), değeri sıralı virgüllü liste — `columnNames` deseninin aynısı. İki taraf da aynı dili konuşuyor. **Üç birim testiyle kanıtlandı**, biri **negatif** (bileşik PK'nin bir sütunu snapshot'ta eksikse **uyuşmazlık** görünüyor) | ⚠️ **4.3'ün `udtName` kuralının GENİŞLEMİŞ hâli.** O gün kural *"şemaya yeni bir TİP AİLESİ girdiğinde `introspect`in okuduğu sütun listesi gözden geçirilir"* diye yazılmıştı; burada giren şey bir tip değil bir **YAPI TÜRÜ** (bileşik anahtar). **Kural: şemaya yeni bir tip ailesi YA DA YAPI TÜRÜ girdiğinde, her karşılaştırıcının okuduğu alan listesi gözden geçirilir ve genişletme NEGATİF bir testle kanıtlanır.** ⚠️ Ve ikinci katman daha genel: **bir karşılaştırıcı olguları anahtara göre indeksliyorsa, aynı anahtarın tekrarlanabildiği ilk gün sessizce körelir** — tekrarlanan yol yerine birleşik tek değer kullanılır |
| 21 | 4.6 | İki test iddiası **ölçülmeden yazıldı ve test onları anında reddetti**: ① sayaç sütunu sayısı `22` yazıldı, gerçek **23** ② `spec/01`'deki mevki kümesinin biçimi bir **tip birleşimi** sanıldı, gerçekte `text` sütununun yanındaki bir **yorum** | (hata değil, **doğru mekanizmanın çalışması**) İkisi de bir **tahmindi** ve ikisi de bir **teste** yazıldı | Gerçek değerler testin reddettiği çıktıdan okundu ve yorumla birlikte yazıldı (*"önce 22 yazıldı ve test onu anında reddetti"*) | ℹ️ **3.5'in dersinin canlı kanıtı:** *"bir tahmin, belgeye değil bir teste yazılırsa anında reddedilir."* Aynı iki sayı bir rapora yazılsaydı hiçbir şey ötmezdi — D1'in Faz 2'de üç kez ödenen bedeli tam olarak buydu. **Tahminin YERİ vardır** ve bu turda doğru yerdeydi |
| 20 | 4.6 | (hata değil, **ölçüm bulgusu**) ROADMAP Faz 4'ün migration numaraları **bir kaymış**: 4.6 `(0008)` diyordu ama `0008` 4.5'te kullanılmıştı | Liste (`8203ac1`) her alt göreve **bir** migration varsayarak numaralandırılmıştı; 4.5 bir iddia ayrımı gerekçesiyle **iki** yazdı ve numaralandırma güncellenmedi | 4.6 → `0009`, 4.7 → `0010`, 4.8 → `0011` düzeltildi; 4.6'nın eskisi `~~0008~~` biçiminde **görünür** bırakıldı (**SAPMA-034**) | ⚠️ Hata **sessiz kalırdı**: `drizzle-kit generate` sıradaki numarayı kendisi veriyor, yani yanlış numara yalnızca ROADMAP'i okuyan **insanı** yanıltırdı. **Kural: bir plandaki türetilmiş numaralar, planın bir adımı beklenenden fazla üretince sessizce bayatlar** — *"bir alt görev birden fazla migration yazabilir"* kuralı 4.5'te kondu ama **numaralandırmaya yayılmadı** |
| 19 | 4.5 | (hata değil, **kural ihlali — AYNI ALT GÖREVDE İKİ KEZ, #14 ile birlikte ÜÇÜNCÜ tekrar**) ① bir test dosyasının sonuna içerik eklemek için `cat >> dosya << 'XEOF'` ② **commit mesajı** `git commit -F - << 'GITEOF'` ile stdin'den verildi (Türkçe metin, ters tırnak, `→`, `§`) | #14 ile **birebir aynı sınıf** ve #14'ün kendi cümlesi bunu öngörüyordu: *"bir kuralı ihlal edip yakalanmamak, kuralın gereksiz olduğunu değil o turda şanslı olunduğunu gösterir."* ⚠️ **Ve ikinci vaka birincisinden ağır:** bir dosya `Edit` ile onarılabilir, **commit mesajı geçmişe yazılır**. Bozulsaydı düzeltmesi `--amend` (yani commit hash'inin değişmesi) olurdu | İkisi de **hemen ölçüldü**: ① `git diff --stat` + `od -c` → **0 bayt** eklendi, içerik `Edit` ile yazıldı ② `git log -1 --format=%B \| od -c` → **31 satırda çok baytlı UTF-8 dizisi**, Türkçe karakterler / ters tırnaklar / `→` / `§` tek tek okundu, **bozulma yok**. Doğru yol biliniyordu ve kullanılmadı: mesajı `Write` ile bir dosyaya yazıp `git commit -F <dosya>` | ⚠️ **BİR KURAL ÜÇ KEZ ÜST ÜSTE İHLAL EDİLİYORSA SORUN KURALDA DEĞİL, KURALIN YAŞADIĞI YERDEDİR.** #14 onu *"aynen duruyor"* diye kapatmıştı; bir sonraki alt görevde **iki kez** tekrarladı. Kural doğru ve bedeli #3'te ödendi (üç ANLIK DURUM satırı sessizce boşaldı). Eksik olan şey, kuralın **ateşlendiği anda** görünür olması: bugün yalnızca ORTAM TUZAKLARI ⑤'te ve günlükte yaşıyor, yani `cat >> … <<` yazıldığı anda hiçbir yerde görünmüyor — **SAPMA-033'ün sınıfı** (*"kontrol eden adımı olmayan kural ateşlendiğinde hiçbir şey olmaz"*). ⏳ Kapatma yolu belli: `heredoc` / `-e` / `-c` içeren `Bash` komutlarını reddeden bir kanca. **Faz 4 kapsamında değil** (K12) ve bir özellik değil bir **süreç** kararı olduğu için `V2-BACKLOG`'a değil buraya yazıldı; kararı kullanıcı verir |
| 18 | 4.5 | **Mutasyon serisi ALARM verdi: 25/215 — pay 4.4'ün değerinde SABİT.** İki yeni migration, iki yeni çevrim testi ve pay hiç artmadı | **4.4'ün dersinin sınırı.** 4.4 *"envanter iddiaları körlükten çıkarır"* diye ölçmüştü ve doğruydu — ama o gün envanter **dolu** bir farklar listesiydi. 4.5'in iki yeni testi `differences: []` iddia ediyor, yani **boş** bir envanter; mutasyonun ürettiği değer tam olarak bu (`{ differences: [], identical: true, … }`). Yani iki pozitif test kör bir karşılaştırıcıyla da geçiyor — §11.5'in başındaki 3.2b ölçümünün (16'da 15) birebir tekrarı | Bir **negatif** test yazıldı: `④ SESSİZ bozuk down (KISIT TANIMI)` — bir `down`un CHECK'i **dar** geri koyduğu sessiz vaka. `0007` zincire ilk kez *başka bir tablonun kısıtını*, `0008` ilk kez bir kısıtın **tanımını** getirdi, yani `constraint.definition` yeni bir **olgu türü**ydü ve onu ölçen hiçbir test yoktu. Pay **25 → 26** | ⚠️ **Kural: bir envanter iddiası körlükten ancak FARK BEKLEDİĞİNDE çıkarır.** *"Boş bir liste tek başına 'yok' diye okunur"* kuralının mutasyon tarafındaki biçimi. Ve ikinci katman: **şemaya yeni bir OLGU TÜRÜ girdiğinde** (dizi tipi → 4.3'ün `udtName`'i; kısıt tanımı → burası) karşılaştırıcının o alanı gerçekten okuduğu **negatif bir testle gösterilmezse yalnızca varsayılmış olur** (D3) |
| 17 | 4.5 | **`0008`in `down`u dolu bir veritabanında ZİNCİRİN TAMAMINI bloke etti** — on altı test birden `check constraint "people_person_type_check" … is violated by some row` ile patladı, ve bunların çoğu 4.5'in hiç dokunmadığı testlerdi | (hata değil, **`'referee'` eklemenin ölçülmemiş bedeli**) `ADD CONSTRAINT … CHECK` var olan satırları **doğruluyor**; `seedAllTables()` iki hakem kişisi yazıyor ve ikisi de artık `['referee']` taşıyor. ⚠️ **Etki 0006'nınkinden YAPISAL olarak farklı:** 0006'nın sınırı yalnızca kendi yeniden `up`ını engelliyordu; `0008` zincirin **en üstünde** ve `down` **LIFO** çalışıyor — yani dolu bir veritabanında **hiçbir** geri alma başlayamıyor, kısmi olanlar da tam zincir de | Sınır **kabul edildi ve gizlenmedi**: kendi testi var (*"0008 DOWN`u `referee` taşıyan bir satır varken GÜRÜLTÜLÜ patlıyor"*), ve `migrateDown` çağıran testler bir `narrowRefereePersonTypesForDown()` yardımcısı kullanıyor (`clearRefereesForReup` deseninin kardeşi). ⚠️ Yardımcı satırları **silmiyor**, `person_type`ı daraltıyor — silme `SEEDED_ROWS`u değiştirir ve kayıp ölçümü testlerinin zeminini kaydırırdı. Üç alternatif tek tek elendi: `NOT VALID` (geri alınmış şemada geçersiz veri sessizce kalırdı), `down`un satır silmesi (`loss.ts` **satır** kaybını ölçmüyor → sessiz kayıp, 3.2b'nin *"fazla giden down"* sınıfı), `'referee'`yi `0007`ye koymak (aynı sonuç — sorun migration'ın yerinden değil kümenin daralmasından) | ⚠️ **Kural: bir KAPALI KÜMEYİ genişletmek, o kümeyi DARALTAN geri almanın veriye bağımlı hâle gelmesi demektir** — ve kısıt zincirin üstündeyse bağımlılık **zincir çapına** yayılır. Bir küme genişletilirken sorulacak soru: *"bu değeri taşıyan bir satır varken `down` ne yapar?"* Ayrıca: **bir alt görevin dokunmadığı testleri kırması, o alt görevin kapsamının sandığından geniş olduğunun kanıtıdır** — #12'nin (*"bir `ALTER`ın etkisi kendi testiyle sınırlı değildir"*) kısıt tarafındaki biçimi |
| 16 | 4.5 | *"SAF SÜTUN kaybı — 0006 tek başına geri alınınca hiçbir TABLO düşmüyor"* testi, **tek bir satırı değişmeden** yanlış bir şey iddia eder hâle geldi: artık iki tablo düşüyor | `stepsBackTo('0006')` zincirden **türetiliyor** (4.3'ün kazancı) ve `0007` eklenince aynı çağrı 1 adımdan **3 adıma** çıktı. Sayı doğru kaldı, ama testin **adı ve iddiası** sayıyla birlikte kaymadı — vaka *saf sütun*tan *karışık*a döndü | Test yeniden adlandırıldı (`KARIŞIK kayıp — 0006'ya kadar geri alınınca 0007'nin iki tablosu da düşüyor`) ve beklentisi genişletildi. Saf vaka **kaybolmadı, yeri değişti**: `0008` artık ondan da saf bir vakayı ölçüyor — **sıfır kayıp** (`loss.ts`in dördüncü sınıfı, zincirde ilk) | ⚠️ **Kural: bir testin ölçtüğü SINIF, kendi kodundan değil ZİNCİRDEKİ YERİNDEN gelebilir.** Türetilmiş bir sayı (`stepsBackTo`) elle yazılmış bir sayıdan iyidir ama **iddiayı** güncellemez; yeni bir migration eklenirken *"hangi testlerin SINIFI değişti?"* de sorulur, yalnızca *"hangi testler kırıldı?"* değil. #12'nin akrabası |
| 15 | 4.5 | `player-hidden-attributes.test.ts`teki *"gizli nitelikler görünür nitelik tablosunda DEĞİL"* testi kırıldı: `expected [ 'consistency', …(9) ] to not include 'consistency'` | (yazım hatası) `attributeFields` bu dosyada **gizli** tablonun alanlarıydı; test onu görünür tablonun alanları sanıyordu. İki dosya arasında kopyalanan bir yardımcı adının, kopyalandığı yerde **başka bir şeye** işaret etmesi | `playerAttributes` açıkça import edildi ve karşılaştırma iki yönlü yapıldı (görünür ⊄ gizli **ve** gizli ⊄ görünür) — tek yönlü iddia zaten yarım bir korumaydı | ℹ️ Testin kendisi doğru şeyi ölçmek istiyordu ve **kırılması onu düzeltti**; yanlış yazılmış bir iddia sessiz kalsaydı iki tablonun ayrımı hiç sınanmamış olurdu. *"Kısmi koruma D3 yanılsaması üretir"*in test tarafındaki biçimi |
| 14 | 4.4 | (hata değil, **kural ihlali — bu kez bedelsiz**) İki test başlığı Türkçe metinle `python` **heredoc**'undan geçirilerek değiştirildi | ORTAM TUZAKLARI ⑤ ve günlük #3 bunu açıkça yasaklıyor: *"Markdown/Türkçe metin hiçbir kabuk argümanından geçmez — `node -e`, `python -c`, **heredoc**, tırnaklı dize aynı sınıf."* Tırnaklı heredoc (`<<'PYEOF'`) kabuk ikamesini engelliyor, yani #3'ün somut mekanizması (çift tırnak içinde komut ikamesi) burada yoktu — **ama kural mekanizma hakkında değil, SINIF hakkında** | Sonuç **hemen doğrulandı**: `file` → UTF-8, `\r` sayısı 0, üç başlık tek tek okundu, Türkçe karakterler yerinde. Bozulma **yok**; metin geri yazılmadı, yalnızca kayda geçirildi | ⚠️ **Bir kuralın "bu sefer işe yaradı" ile doğrulanması, kuralı zayıflatır.** #3'ün bedeli üç ANLIK DURUM satırının **sessizce boşalmasıydı** ve o gün de betik *"başarı"* raporlamıştı. **Kural aynen duruyor: metin değişiklikleri `Edit`/`Write` ile yapılır.** Bir kuralı ihlal edip yakalanmamak, kuralın gereksiz olduğunu değil, **o turda şanslı olunduğunu** gösterir |
| 13 | 4.4 | Yeni yazılan *"KARŞI ÖRNEK — federasyon ülkesi silinince GİDİYOR (CASCADE)"* testi **alakasız bir kısıtla** patladı: `people_nationality_country_id_countries_id_fk` | Fixture federasyon başkanını **aynı ülkenin** vatandaşı yapmıştı. `DELETE FROM countries` o ülkeye bakan **bütün** FK'ları tetikliyor ve `people` RESTRICT'i CASCADE'den önce ötüyor — yani test ölçmek istediği şeyi (`federations.country_id` CASCADE'i) **hiç göremedi** | Başkanın kişisi **ikinci bir ülkeye** taşındı; aynı düzeltme `RESTRICT: hakemi olan bir ÜLKE silinemiyor` testine de uygulandı (orada da hakemin kişisi aynı ülkedeydi ve iddia `referees_country_id`i adıyla arıyordu) | ⚠️ **`fixtures.ts` başlığındaki #17 kuralının SİLME tarafındaki biçimi:** *"bir negatif testte reddin sebebi TEK olmalı"* bugüne kadar `INSERT` için okunuyordu. **Genel biçim: bir `DELETE` testi, silinecek satıra bakan BÜTÜN yabancı anahtarları hesaba katar** — şema büyüdükçe aynı satıra bakan FK sayısı artıyor ve dünkü tek sebepli bir test bugün çok sebepli oluyor. `people` 4.3'te geldi ve bu testleri **yazıldıkları gün** değil, bugün çok sebepli hâle getirdi |
| 12 | 4.4 | Dört **var olan** çevrim testi kırıldı (`0005`, `0004`, `0003`, `0002`) — hepsi *"şema BİREBİR aynı"* diyordu ve artık değil | (hata değil, **tasarım sonucu**) `0006` zincirin 0001'den beri ilk `ALTER`-only migration'ı. §3.1.2 ⑤: `DROP COLUMN` `attnum`ı geri kazanmaz. Sonuç **yalnızca 0006'nın kendi testini değil**, 0006'nın **içinden geçen her kısmi geri almayı** etkiliyor — hangi kaymanın göründüğü, o derinlikte tablonun **ayakta kalıp kalmadığına** bağlı (0005/0004 → 3 kayma, 0003 → 2, 0002 → 1, 0001 ve tam zincir → 0) | Dört test `identical: true` iddiasından **farkların TAM listesi** iddiasına geçirildi (3.4'ün 0001 için verdiği kararın aynısı) ve kaymalar tek bir yerde (`ALTER_0006_SHIFTS`) tanımlandı; hangi tablonun düştüğü tablosuyla birlikte yazıldı | ⚠️ **İki ders. ① Bir `ALTER` migration'ın etkisi kendi testiyle sınırlı değildir** — zincire eklenen her `ALTER`, ondan **daha eski** her kısmi geri almanın beklentisini değiştirir; yeni bir `ALTER` yazarken *"hangi testler bu migration'ın içinden geçiyor?"* sorulur. ② **Ve bu bir GÜÇLENDİRME oldu:** mutasyon ölçümü gösterdi ki `identical: true` iddiası körelen bir karşılaştırıcıyla **geçiyordu**, tam liste iddiası **kırılıyor** — pay 20 → 25. *Özetler körlenebilir, envanterler kör kalmaz* |
| 11 | 4.4 | **4.3'ün raporu 4.4 için yanlış tahmin bıraktı:** *"üçü de `people`'a bakacak ve `people` `independent` olduğu için üçü de RESTRICT alacak"*. Doğrusu **SET NULL · RESTRICT · RESTRICT** | Tahmin kuralı **yanlış taraftan** okuyordu. `expectedDeleteAction` yalnızca ① adımında **hedefin** sınıfına bakıyor (*"hedef sözlük mü"*); ②③④ **kaynağın** sınıfına bakıyor. `people`'ın `independent` olması bu üç FK hakkında **hiçbir şey** söylemiyor. ⚠️ Yöntemsel fark net: 4.3'te dört tahminin dördü tuttu çünkü **kural koşturuldu**; 4.4 tahmini **hafızadan** yapıldı | 4.4'te önce kural derlenmiş `fk-policy.js` üzerinden **koşturuldu** (üç FK'nın olguları verilerek), tahmin yazıldı, sonra `drizzle-kit generate` çıktısının `ON DELETE` satırları **tek tek** okunup karşılaştırıldı → **3/3**. Ardından davranış gerçek PG18.6'ya karşı ölçüldü | ⚠️ **Kural: bir kuralı hafızadan uygulama — KOŞTUR.** Ve ikinci bir katman: **bir alt görevin sonraki alt göreve bıraktığı tahmin, bir ÖLÇÜM gibi okunuyor.** 4.3'ün raporu §12'de tahmini *"(tahmin; 4.4 ölçecek)"* diye işaretlemişti ve bu doğru davranıştı — ama işaret, tahminin **yanlış üretilmiş** olmasını engellemiyor. **Devir teslim notundaki bir tahmin, üretildiği yöntemle birlikte yazılır** (*"kural koşturularak"* / *"hafızadan"*), yoksa sonraki oturum onu bir zemin sanır |
