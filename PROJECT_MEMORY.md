...
| | |
|---|---|
| **Aktif faz / alt görev** | **FAZ 4 — Veritabanı Şeması II.** Alt görev listesi **onaylandı ve ROADMAP'e yazıldı** (`8203ac1`, K11). **4.0 · 4.0b · 4.1 kapandı** — üçü de kod yazmadı. ⏸️ **SIRADAKİ: 4.2** — `fk-policy.ts` V3'e genişletilir (üçüncü olgu `is_nullable`, `SET NULL` üretimi). İlk **kod** alt görevi; migration yok |
| **Son tamamlanan** | ✅ **4.1 — kararlar ve envanter mutabakatı. KOD YAZILMADI.** Envanter **19 → 11 master** (SAPMA-030): beş tablo **Faz 12**'ye, biri **Faz 13**'e (SAPMA-032), `değer<15M` yüklemi **Faz 30/32**'ye (SAPMA-031) — hepsi **hedef fazın ROADMAP kapsamına yazıldı**, kütüğe kayıtla yetinilmedi. Üç tablo **açılmıyor**, iki ad çakışması mevcut tablolara katlandı. `spec/01`'e `player_stats_history.club_id` + `injury_types`/`manager_career` **tanımları** + üç düzeltme notu. **SAPMA-033: §0.5'in bölünme kuralı ateşlendi ama işlemedi** (Faz 3 = 4 gün) → `SESSION-TEMPLATE`'e **süre ölçüm adımı**. G-15, G-16 açıldı. **Doğrulama: 38 iddia, 38'i geçti**, mutasyonla iki yönlü sınandı. ℹ️ Önceki: **4.0b** |
| **4.0b (önceki)** | ✅ **CI işlendi, süreç boşluğu kapatıldı, plan yeniden üretildi.** ① 4.0'ın CI koşusu **liste sorgusuyla** çıkarıldı ② `docs/OUTPUT-FORMAT.md`'ye yeni kural: **onay bekleyen içerik raporun `DETAY` bölümünde yaşar** — 4.0'ın planı yalnızca terminalde yaşadığı için kaybolmuştu ③ yedi tablonun **tüketici araması** ve dört karar yeniden üretildi (ölçüm ①/② tekrarlanmadı, arşivden okundu). ℹ️ Önceki: **4.0** — üç süreç boşluğu (`0682c5f`), kayıt (`6063544`) |
| **Tarih** | 2026-08-29 |
...
| **test** | ✅ **744 test / 52 dosya** (değişmedi) · ✅ **`pnpm test:db` 163 test / 8 dosya**, gerçek PG18 konteyneriyle, **31,20 s**. Dağılım ölçüldü: `packages/db/integration` **6 dosya**, `tools/data-cli/integration` **2 dosya** |
| **kapsam** | ✅ satır **%86,99** · ifade **%87,04** · dal **%88,68** · fonksiyon **%80,00** — dördü de Faz 3 kapanışıyla **birebir aynı** (4.0 kod yazmadı). Eşik %70, DÜŞÜRÜLMEDİ. ⚠️ Fonksiyon marjı **10 puan**; Faz 4 şema dosyalarıyla payda büyüyecek |
