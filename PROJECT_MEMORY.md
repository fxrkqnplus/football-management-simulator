...
| | |
|---|---|
| **Aktif faz / alt görev** | ▶️ **FAZ 6 — Tasarım Sistemi.** **6.0 ✅ · 6.1 ✅ · 6.2 ✅ · 6.3 ✅ · 6.3b ✅ · 6.4-ön ✅.** Sıradaki: **6.4 — shadcn/ui + temel bileşenler I**. §0.5 kontrol noktası **6.5 sonunda**; çizgi **6a = 6.0–6.5 / 6b = 6.6–6.12** |
| **Son tamamlanan** | ✅ **6.4-ön — geriye dönük denetim (A–H) ve kapı genişletmesi.** Kod var, özellik yok. 🆕 **`pnpm debt:check`** kuruldu ve **ilk koşusunda kırmızı verdi**: açık 6 BORÇ satırının **5'i** (001·002·004·006·007) hedef fazının ROADMAP kapsamında geçmiyordu → beşi de **Faz 16/50/12**'ye **adıyla** taşındı. Ayrıca `BORÇ-010`un satırı **4 hücreliydi** (başlık 5 diyor) — 5.6'dan beri görünmeyen bozukluğu ayrıştırıcı buldu |
| **Uygulama kararı** | **ORTAK ÇEKİRDEK** (`scripts/lib/ledger-coverage.mjs`), iki uygulama değil — ayrışacak yarı ortak olan yarı. `gaps:check` çekirdeğe taşındı, çıktısı **bayt bayt aynı** (md5 doğrulandı). ⚠️ **SAPMA kütüğü kapının DIŞINDA ve sebebi ölçüldü:** satırlarında **vade fazı sütunu yok**, denetlenebilir hedefi yok — uydurulmadı (SAPMA-026) |
| **İki yönlü + mutasyon** | Kapı: sahipsiz satırda **ötüyor** · bugün **susuyor** · **kaç satır taradığını basıyor** (12·5·7). **Mutasyon 3/3 kırdı** — ① CI adımı sökülünce ② `CLAUDE.md` ağacından betik adı bozulunca ③ çekirdeğin sütun kontrolü körelince. Yerine oturma **md5**, geri alma **dosya yedeğinden** |
| **Yeni koşan nöbetçiler** | `scripts/inventory-guards.test.mjs` ① kök `package.json`daki her **`:check`** kapısı `ci.yml`de mi + maskeleme yok mu ② `CLAUDE.md` §2.2 ağacı ve belge haritası **diskle** örtüşüyor mu (`tools/` ağacı 4.11 ve 5.9'da bayatlamıştı, bugün **üçüncü kez** bayat çıktı: **`scripts/` dizininin tamamı yoktu**). ⚠️ **`perf:budget` `:check` desenine GİRMİYOR** — ROADMAP 6.10'da adıyla yazılı |
| **En sert C bulgusu** | `CLAUDE.md` §2.3 `.env.example` bloğu **`NODE_ENV=development` ile başlıyordu**; `scripts/check-env-file.mjs` tam o değişkeni görürse **exit 1**. Anayasa, koşan bir kapının **yasakladığı** şeyi tarif ediyordu — olduğu gibi uygulansaydı `pnpm build` kırılırdı |
| **Tarih / ilerleme / dallar** | 2026-09-05 · **5 / 50 faz (%10)** · `main` → `develop` (`2ec1548`) → **`feature/faz-06-design-system`** (taban `2ec1548`, **9 commit**) |
| **CI** | ✅ **6.3b'nin koşusu `33949199057` YEŞİL — altı işin altısı, adım listesi tek tek okundu** ("success" etiketine güvenilmedi): imaj **derlendi ve koşturuldu**, amd64+arm64. ⚠️ Faz 4'ün kırmızı koşusu (`33419337117`) **hâlâ yeniden denenmedi**, kullanıcının kararı. ⏳ 6.4-ön'ün koşusu yazım anında bilinmiyor |
| **Kapılar** | ✅ typecheck **11/11** · build **8/8 SOĞUK** (`Cached: 0 cached, 8 total`) · lint **0** · format **0** · arch **9 kural** · **test 1315/87** (+25) · **test:db 301/10** · **gaps 20·3·17·0 ✗** · 🆕 **debt 12·5·7·0 ✗** · **i18n temiz, 341 dosya** |
| **Kapsam / paket** | fonksiyon **%82,62 (409/495)** — pay **ve** payda sabit: `coverage.include` `scripts/`i saymıyor (`*/src/**` deseni), yani 25 yeni test paydaya dosya eklemiyor. Marj `409/0,70 = 584` → boşluk **89**. Eşik düşürülmedi, dosya dışlanmadı. Üretim paketi **383.976 / 17.064** — 6.3b ile **bayt bayt aynı** |
| **Kütükler** | Açık sorun **0** · teknik borç **12 satır, 5 ÖDENDİ → açık 7** (🆕 **BORÇ-012** `arch:check` çıktısı kapsamını söylemiyor → **6.10**) · SAPMA **44** (🆕 **044** `spec/09` §11.5 kapı listesi eksikti) · boşluk **20**, açık **17** · günlük **13 satır** |
| **⚠️ Bekleyen karar** | `docs/spec/09` §11.5'e iki satır **sorulmadan** eklendi (SAPMA-044). `SESSION-TEMPLATE` *"spec'te eksik varsa **sor**"* diyor. Önerim kalması; *"geri al"* denirse iki satır + SAPMA-044 tek commit'te sökülür |
