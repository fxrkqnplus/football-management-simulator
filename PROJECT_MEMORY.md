...
| | |
|---|---|
| **Aktif faz / alt görev** | ▶️ **FAZ 6 — Tasarım Sistemi.** **6.0–6.5 ✅ · 6.6-ön ✅ · 6.6 ✅ · 6.6b ✅ · 6.6c ✅ (bu commit).** Sıradaki: **6.7 — DataTable motoru** (§0.5: 8 birim, sığıyor; etiketli sınıf idiyomu + `local/no-untyped-arbitrary-value` altında başlar). Onay bekleyen karar: **6.7'ye geçiş (K11)**. 6.8 = 13 birim (sınırda; 6.8a/6.8b çizgisi ROADMAP'te hazır, bölünme önerilmedi) |
| **Son tamamlanan** | ✅ **6.6c — SORUN-002 kapandı + yedi belge maddesi.** Nöbetçi **önce**: `local/no-untyped-arbitrary-value` (45 test: RuleTester 19+14 · türetme 7 · kanarya 5) gerçek depoda **34 hata / 12 dosya** verdi (6.6b sayımıyla dosya dosya birebir), sonra 34 geçiş etiketlendi (`family-name:` 15 · `length:` 19) → `lint` **0 / 303**. Token türü **`tokens.generated.css`ten türetilir** — ikinci liste yok (DZ-07), yükleyici dosya yoksa fırlatır (kapalı-güvenli). Derlenmiş CSS (soğuk ×2): yanlış çift **0**, `font-family:` 2+2 · `font-size:` 5 token; CSS 33.844 → **33.778**. twMerge 3.6.0: etiketli boyut + renk **ikisi de kalıyor** (eski biçimde boyut/aile düşüyordu — ölçüldü). Mutasyon **4/4** ısırdı (M3: kural `off` → 3 kanarya kırmızı, `eslint` ürün dosyasında EXIT 0 kaldı) |
| **⚠️ BULGU — Tailwind yorumları da tarıyor** | Kod satırlarında 0 etiketsiz sınıf kalınca derlenmiş CSS'te yanlış çiftler **hâlâ vardı**: `@source '…/packages/ui/src'` düz metin tarar, 6.6'nın 5 bileşeni + 3 testinin **JSDoc örnekleri** ölü yardımcı üretiyordu. 15 yorum geçişi `[…var(` biçimine çevrildi → 0. **Yorumdaki örnek sınıf da derlenir** — `no-bare-jsx-text`in *"yorumlar görülmez"*i Tailwind için geçerli değil (günlük #43). *"Derlenmiş CSS'te 0"* iddiası yorumlar dâhil ölçülür |
| **Belge maddeleri (②–⑧)** | DZ-21 metni CLAUDE.md §18.2'de yeni ifadeyle; `denetci.md` + `kapici.md`de aynı *"verilmez/okumazsın"* kalıbı **vardı**, hizalandı · SKILL.md kapanış listesine **CI-sayıları kuralı** (ilk uygulaması: #121 sayıları bu kayda girdi) · CLAUDE.md §16.2 **6. madde** (moral eşikleri kalibrasyon; Faz 38 · 44) · 6.8 kapsamına bant 1 notu — ⚠️ girdiler ölçüldü: payı `--text-muted`/kenarlık değil **CVD alfası** yer · **§0.5: 6.7 = 8 birim · 6.8 = 13 birim** (listeler ROADMAP'te) · *"Bilinen kayıt düzeltmeleri"*: CI #121 6/6 · 1702/1702 · atlanan 0 · %85,71 · **SORUN-003** (beş test: web ×2 pay · ESLint ×9 · i18n ×10) sahibi 6.12, 6.12 kapsamında adıyla · SORUN-002 ✅ · V2-BACKLOG: diğer belirsiz önekler |
| **Tarih / ilerleme / dallar** | 2026-09-13 · **5 / 50 faz (%10)** · `main` → `develop` (`2ec1548`) → **`feature/faz-06-design-system`** (taban `2ec1548`; `git rev-list --count 2ec1548..HEAD` = **20** at `ea970b5`, bu commit'le **21**) · son tag `faz-06-5-son`; faz kapanışında `git tag -a faz-06-son` |
| **Kapılar (bu commit'in ağacı, yüksüz)** | gaps **20·3·17·0✗** · debt **16·5·11·0✗** · format **0** (bu commit'te bakılan: `eslint.config.js` + kural `.js` + test `.mjs` + 21 `.tsx/.ts`; `*.md` bakılmadı) · i18n **117/92/3** · 58 dosya · 2 kök · görünmez **425** (rapor dâhil) · arch temiz · typecheck **11/11** · lint **0 (303)** · **test 1746/118 + 1 skipped** (taban 1702/117, +45 kural; skipped = §③ kirli ağaç) · **test:db 301/10** · fonksiyon **%85,71 (522/609)** (kural `coverage.include`ın `tools/*/src` deseni dışında) · build **8/8 SOĞUK ×2** · web JS **390.629** · CSS **33.778** · mutasyon **4/4** · D5: paket `dist` ✅ (import + çağrı) · web paketi ✅ · **imaj koşturulmadı** (CI'da; SKILL kuralı gereği sayıları sonraki kayda) |
| **Son commit** | `feat(lint): no-untyped-arbitrary-value kuralı + SORUN-002 çözüldü; DZ-21 metni, SKILL CI kuralı, §0.5 sayımı, §16.2, SORUN-003 (6.6c)` on `feature/faz-06-design-system` (kapı tabanı `ea970b5`; içerik ve kayıt **tek commit**) — CHECKPOINT `son_commit_baslik` ile aynı; temiz ağaçta ve CI'da nöbetçi karşılaştırır |
...
> ✅ **KAPANIŞ (Faz 6.6c, 2026-09-13) — 6.6b raporunun *"yazım anında
> bilinmiyor"* bıraktığı iki alan ölçüldü.** Kayıt commit'i **`ea970b5`**; CI
> koşusu **#121** (`gh run view 34720397917`): **6/6 yeşil** (Kalite kapıları
> amd64 + arm64 · Entegrasyon amd64 + arm64 · İmaj amd64 + arm64), Kalite
> kapılarında `Test Files 117 passed` · **`Tests 1702 passed (1702)`** — skipped
> **0**, `inventory-guards ③ atlanan: yok` (temiz ağaçta `son_commit_baslik`
> karşılaştırıldı ve geçti) · fonksiyon **%85,71** (stmts 92,91 · branch 91,86
> · lines 92,88) · Entegrasyon `10 · 301`. 6.6b'nin yerel `test:coverage`
> kırmızısının **ortam** olduğu iddiası CI'da doğrulandı (aynı komut, yüksüz,
> 1702/1702); beş testin kimliği ve payı **SORUN-003**. Bu satır SKILL.md'nin
> yeni kuralının ilk uygulaması: CI koşusu 6.6b'nin teslimatı değildi, sayılar
> bir sonraki alt görevin (bu) kaydına girdi. Arşiv raporu **değiştirilmedi**.

> ⚠️ **DÜZELTME (Faz 6.6b, 2026-09-12) — 6.6 kaydının *"6.6'nın on dosyası
> etiketli yazıldı"* iddiası ÇÜRÜTÜLDÜ; arşiv raporu, SONUÇ bloğu ve günlük
> #29 DEĞİŞTİRİLMEDİ.**
> `docs/reports/faz-06/6.6-on-alan-ozel-bilesen.md` §2 (*CurrencyValue:
> `font-[family-name:…]` etiketi ölçüldü*) doğru; §5 *"etiketsiz `font-[var(`
> on dosyada kaldı mı"* sorusunu **açık** bırakmıştı (dürüst) — ama günlük #29
> ve SORUN-002'nin ilk satırı aynı soruyu *"on dosyası etiketli yazıldı"* diye
> **kapatmıştı**. 6.6b ölçtü: `packages/ui/src/components/form-indicator.tsx`
> **6 etiketsiz** keyfi `var()` sınıfı taşıyor (:138 · :139 · :146 · :148),
> **5'i** yanlış özelliğe derleniyor (2× `font-[var(--font-ui)]` →
> `font-weight:` · `text-[var(--text-2xs)]` + 2× `text-[var(--text-xs)]` →
> `color:`). Dokuzu etiketli. Aynı satırın *"20 dosya"* sayısı da renk
> sınıflarını ve yorum satırlarını içeriyordu — bozuk sınıf taşıyan dosya
> **12**, geçiş **34** (6.6b SONUÇ ⑦). **SORUN-002 kütük satırı yerinde
> düzeltildi** (kusur 6.4/6.5 **ve 6.6**; sahibi 6.6c); günlük #29 o günün
> kaydı olarak duruyor, düzeltmesi #38/#39 ve bu blok. Raporun §5 *"ilk
> ölçülebilecek yer 6.7 açılışı"* demişti — 6.7 değil **6.6b** ölçtü
> (kullanıcı kararı); denetimin kendisi `docs/reports/faz-06/6.6b-denetim.md`.
> 6.6'nın *"imaj ÖLÇÜLMEDİ"* alanı da burada kapandı: CI #120 (`74c5179`)
> İmaj amd64 + arm64 **yeşil**.

> ✅ **KAPANIŞ (Faz 6.6-ön, 2026-09-11) — Faz 4'ün kırmızı CI koşusu
> `33419337117` YENİDEN KOŞTURULDU.** Faz 4 kaydı §6 ve Faz 5 kaydı §11 bu
> koşuyu *"yeniden denenmedi — kullanıcının kararı"* diye açık bırakmıştı.
> Kullanıcı koşuyu yeniden başlattı; ölçüldü (`gh run view 33419337117 --json
> jobs,attempt,conclusion`): koşu **#87**, commit **`0459fa5`**, **attempt 2**,
> 2026-09-11 01:12 UTC → **altı işin altısı yeşil** (Kalite kapıları amd64+arm64
> · Entegrasyon amd64+arm64 · İmaj amd64+arm64); adım günlüklerinden **868/63**
> (birim) ve **241/8** (entegrasyon). İfade tam olarak şu: **yeniden koşturuldu,
> altı iş de yeşil; ancak bu, o commit'in ağacının BUGÜNKÜ koşuda geçtiğini
> gösterir — özgün kırmızının sebebi (Docker Hub kimlik doğrulama zaman aşımı,
> kod dışı) kayıtlarda kalmadı.** Faz 4 ve 5 kayıtları append-only olduğu için
> **değiştirilmedi**; kapanış burada.
>
...
|---|---|---|---|---|---|
| SORUN-003 | 6 (6.6b) | **BEŞ AĞIR TEST YÜK ALTINDA ZAMAN AŞIMINA DÜŞÜYOR — zamana duyarlı, mantığa değil.** 6.6b'nin yerel `test:coverage` koşuları (FC26 ≈ 6–7 çekirdek yerken) 2 → 5 kırmızı verdi, hepsi *"Test timed out"*; CI #117–#121 yeşil. Kimlikleri ve **yüksüz** süreleri (6.6c'de `--reporter=verbose` ile ölçüldü): `apps/web/src/main.boot-failure.test.tsx › i18n çökerse React monte EDİLMİYOR…` **2.440 ms / 5.000** · `apps/web/src/main.test.tsx › #root varsa uygulamayı oraya monte eder` **2.361 ms / 5.000** (ikisi ×2 pay — en dar) · `tools/eslint-local-rules/no-hardcoded-path.test.mjs › ÜRÜN dosyası kimliğinde kural GERÇEKTEN ötüyor (uçtan uca)` 3.126 ms / 30.000 · `no-bare-jsx-text.test.mjs › aynı` 3.301 ms / 30.000 (×9) · `tools/i18n-check/index.test.mjs › GERÇEK DEPO bugün TEMİZ` 479 ms / 5.000 (×10). Ortak özellik: gerçek iş yapıyorlar (React monte + i18next init · `new ESLint()` + tipli `lintText` · depo çapında `i18n:check`); yük altında ilk onlar düşer. Duvar saatine bağlı bir iddia (`Date.now`) **yok** — duyarlılık yalnızca bütçe payı. Gürültülü bir CI koşucusunda aynı şey olursa *"ortam"* demek bu satır olmadan bir **varsayım** olurdu; bu satırla bir **ölçüm**. | Orta | 🟡 Açık — **sahibi 6.12**, ROADMAP 6.12 kapsamında adıyla: *"bütçe mi (iki web testine açık `testTimeout`), yapı mı, kabul edilen yerel gürültü mü"* — fazın bütün koşu verisiyle karar. Bütçe 6.6c'de **dokunulmadı** (K12). | — (6.12'de karar) |
| SORUN-002 | 6 (6.6) | **6.4/6.5 — VE 6.6'NIN BİR DOSYASININ — YAZI BOYUTU VE YAZI TİPİ SINIFLARI ÜRETİM CSS'İNDE YANLIŞ ÖZELLİĞE DERLENİYOR.** Yazar ajanlar 6.6'da ölçtü (`@tailwindcss/node` 4.3.3 `compile`), ana oturum derlenmiş CSS'te doğruladı (`apps/web/dist/assets/index-*.css`): etiketsiz `font-[var(--font-ui)]` → **`font-weight:var(--font-ui)`** (yazı tipi değil), `text-[var(--text-sm)]` → **`color:var(--text-sm)`** (boyut değil). Tailwind 4 `var()` taşıyan keyfi değeri belirsiz sayıp ilk uyan yardımcıya düşürüyor; doğrusu tip ipucu (`font-[family-name:…]`, `text-[length:…]`). Renkler (`text-[var(--text-primary)]`) tesadüfen doğru (varsayılan yorum renk). **Ölçek 6.6b'de yeniden sayıldı (kod satırları, yorumlar hariç): 34 geçiş / 12 dosya** — 15× `font-[var(--font-ui\|mono)]` → `font-weight:`, 19× `text-[var(--text-2xs\|xs\|sm\|base\|lg)]` → `color:`; bozuk önek tam iki (`font-[var(…)]` · `text-[var(--text-<boyut>)]`), `border-`/`gap-`/`shadow-`/`duration-`/`z-`/`rounded-`/`bg-` derlenmiş CSS'te doğru çözülüyor. *(6.6 kapanışı "20 dosya" ve "6.6'nın on dosyası etiketli yazıldı" demişti — ikisi de ÇÜRÜTÜLDÜ: 20 sayısı renk sınıflarını ve yorum satırlarını da içeriyordu; `form-indicator.tsx` (6.6) 6 etiketsiz sınıf taşıyor, 5'i yanlış özelliğe derleniyor — kusur 6.4/6.5 **ve 6.6'da**. 6.6'nın diğer dokuz dosyası kodda temiz; beşi hatayı JSDoc'ta anlatıyor.)* tailwind-merge tarafı da etkilenmiş: etiketsiz `font-[var(…)]` `font-medium` ile aynı gruba düşüp siliniyor. **D5'in bu biçimi:** 6.4/6.5'in web paketi ölçümü yalnızca **bayt** saydı, CSS'in **ne ürettiğine** bakmadı — testler yeşil, üretim sessizce yanlış. | Yüksek | ✅ **Çözüldü — 6.6c (2026-09-13).** Nöbetçi **önce**: `local/no-untyped-arbitrary-value` (ESLint yerel kuralı; token türü `tokens.generated.css`ten türetilir — ikinci liste yok; kapalı-güvenli) gerçek depoda **34 hata / 12 dosya** verdi (grep sayımıyla dosya dosya birebir), sonra 34 geçiş etiketlendi (`family-name:` 15 · `length:` 19) → `lint` 0 (303 dosya). Derlenmiş CSS (soğuk build): `font-weight:var(--font-*)` **0** · `color:var(--text-<boyut>)` **0** · `font-family:var(--font-ui\|mono)` 2+2 · `font-size:var(--text-2xs\|xs\|sm\|base\|lg)` 5 — CSS 33.844 → **33.778 B**. ⚠️ İkinci bulgu: Tailwind `@source` dizini **düz metin** tarıyor, **yorumlardaki** örnek sınıflar da yardımcı üretiyordu (5 bileşen + 3 test dosyasının JSDoc'u) — 15 yorum geçişi `[…var(` biçimine çevrildi, ölü yardımcılar gitti. twMerge 3.6.0 belirtisi de kapandı: etiketli boyut + renk **ikisi de kalıyor** (ölçüldü). Sahibi 6.6 kapanışında 6.7'ydi → 6.6b → 6.6c (kullanıcı kararları 2026-09-12). | 6 (6.6c) |
...
|---|---|---|---|---|
| **BORÇ-016** | **6 (6.6)** | **K5 KAPILARININ ÜÇ KÖR NOKTASI DAHA** (kapıcı ZAAF-3/4/5, 6.6'da ölçüldü): ① `glossary-check` ④ (sözlük §7/§8 ↔ `common.json`) **anahtar-bağımsız** — `ui.kitSwatch.kind.home` ↔ `away` değerleri takas edilince 22/22 yeşil; terimin doğru anahtarda olduğu iddia edilmiyor ② **yer tutucu adları** korumasız — `{{level}}` → `{{lvl}}` → `i18n:check` temiz, bileşen testleri yeşil (fixture `common.json` okumuyor); 6.6'da yedi yer tutucu elle hizalı ③ **cümle içindeki terim** — `ui.starRating.unknown` = *"Yıldız Derecesi bilinmiyor"*, sözlükte satırı yok, K-5 tek yönlü. | BORÇ-009 ile aynı sınıf ve aynı ev: 6.11 K5 kapılarının kör noktalarını **gerçek vakalar** üzerinde ölçüp *"kapı mı disiplin mi"* diye karar verecek; üç bulgu o ölçümün girdisi. 6.6 kapanışında ayrı bir kapı yazmak, 6.11'in kararını onun yerine vermek olurdu (SAPMA-026). | **6.11** — ROADMAP 6.11 kapsamında adıyla |
| **BORÇ-015** | **6 (6.6)** | **`packages/ui/package.json`da `sideEffects` BEYANI YOK** (kapıcı ZAAF-1, M14 ile ölçüldü): 6.6'nın `apps/web`in hiç render etmediği on bileşeninin üst düzey ifadeleri Rolldown'da yan etkili sayılıp web paketine giriyor — JS **384.962 → 390.629 B (+5.667)**; `"sideEffects": false` ile **385.463** (`@fms/ui` payı 8.145 → 2.994 B). Hiçbir kapı paket baytını ölçmüyor. | Paket bütçesinin sahibi 6.10 (`perf:budget`, JS/CSS satırları); `tokens.css` CSS `@import` ile geldiği için `false` mı `["*.css"]` mi kararı ölçümle orada verilir. | **6.10** — ROADMAP 6.10 kapsamında adıyla |
| **BORÇ-014** | **6 (6.6)** | **`packages/ui/src/components/index.ts` (paket yüzeyi) ELLE TUTULAN ENVANTER, NÖBETÇİSİ YOK** (kapıcı ZAAF-2, M15): `DateChip` dışa aktarım bloğu silinince 577 test, `tsc`, `eslint`, `i18n:check` hepsi yeşil — hiçbir test barrel'dan import etmiyor. 6.6'nın `build-exports.mjs`i tek seferlik bir betikti, kapı değil. Bir bileşen `dist`ten sessizce düşebilir; ilk fark edecek yer Faz 17'nin typecheck'i. | 6.9'un *"envanter ↔ hikaye çift yönlü"* nöbetçisi bileşen envanterini diskten zaten kuracak; barrel iddiası oraya eklenir (iki envanter tek nöbetçi). | **6.9** — ROADMAP 6.9 kapsamında adıyla |
| **BORÇ-013** | **6 (6.6)** | **`inventory-guards` §③ `asama` ile `durum` arasında TUTARLILIK denetlemiyor** — ölçüldü (6.6 açılışı, danışman): `scripts/inventory-guards.test.mjs` :294-295 (`asama ve durum kapalı kümelerden` vakası) yalnızca **küme üyeliği** sınıyor. `asama: kapandi` + `durum: devam` bugün **geçer** — ve `asama`, kurtarma oturumunun en çok yaslandığı alan (6.6-ön kesintisinde okunan ilk satır oydu). Değişmez tamamen **içsel**, uydurma bir kaynak gerektirmiyor: `asama === 'kapandi'` ⟺ `durum === 'tamamlandi'`. | 6.6 alt görevi on bileşenlik bir arayüz işi; nöbetçiye iddia eklemek 6.10'un kapı-çıktısı işiyle **aynı sınıf** (bir kapının söylediği ile baktığı arasındaki fark) ve orada `inventory-guards`a zaten dokunuluyor (`perf:budget` deseni). İki alt görevde aynı dosyaya iki ayrı iddia eklemek yerine tek yerde. | **6.10** — ROADMAP 6.10 kapsamında adıyla |
| **BORÇ-012** | **6 (6.4-ön)** | **`arch:check`İN BAŞARI ÇIKTISI KAPSAMINI SÖYLEMİYOR.** Ölçüldü: temiz koşuda tek satır basılıyor — `✓ arch:check temiz (349 ms)`. Kaç kural değerlendirildi, kaç dosya tarandı: **yok**. Karşılaştırma aynı koşuda yapıldı: `gaps:check` (20 satır · 3 kapalı · 17 taranan), `debt:check` (11 · 5 · 6) ve `i18n:check` (59 anahtar · 34 kullanım · 336 dosya) üçü de kapsamını **basıyor**. ⚠️ Aracın **kendi başlığı** tehlikeyi adıyla yazıyor (`index.mjs`:33 ve :177 — *"Bir denetleyicinin çıktısı, dosyaya BAKILDIĞINI söylemez; liste daralırsa kapı sessizce kör olur"*) ve 2.1'de `.cts` uzantısı listeden düşünce bir dosya denetimden **tamamen** kaçmıştı — gate yine *"temiz"* diyordu. | **Bugün kör değil, ama koruma yanlış katmanda.** Kapsam bir **teste** (`DOKUZ kural`) ve `PROJECT_MEMORY`nin kalıcı bloğuna yazılı; ikisi de gerçek ve ikisi de kırılıyor. Eksik olan şey **çıktının kendisi**: bir kapı çıktısını okuyan insan, kapsamı görmek için başka bir dosyaya bakmak zorunda. Bugün düzeltilmedi çünkü `runArchCheck()` yalnızca `violations` döndürüyor — sayaçları dışa vermek imzayı değiştirir ve onu çağıran 54 testi ilgilendirir; bu, bir **denetim** alt görevinin kapsamı değil (K12). | **6.10** — o alt görev zaten kapı çıktısı ve kapı kanıtı biçimiyle uğraşıyor (`perf:budget`in üç parçalı kanıtı). Düzeltme oraya düşüyor ve ROADMAP 6.10 maddesinde **adıyla** yazılı. ⚠️ Ölçüt: `arch:check` temiz koşuda **kural sayısını ve taranan dosya sayısını** basmalı; *"0 bulundu"* ile *"hiçbir şeye bakmadı"* çıktıdan ayırt edilebilmeli |
| **BORÇ-011** | **6 (6.3)** | **PAKETLER ARASI TESTLER `dist`i ÖLÇÜYOR, KAYNAĞI DEĞİL — ve `pnpm test` HİÇ BUILD ETMİYOR.** Ölçüldü (6.3, mutasyon ④): `packages/ui/src/theme/turkish-glyphs.ts`teki bir kod noktası bozuldu, `pnpm test` koşturuldu → **138 test geçti, hiçbiri kırılmadı**. Sebep: `apps/web/src/app/fonts.test.ts` `@fms/ui`yi paketin `exports` haritasından, yani **`packages/ui/dist/index.js`**ten çözüyor; kaynak değişmişti, `dist` bayattı. `pnpm build --filter @fms/ui` koşturulunca **aynı mutasyon 4 testi kırdı**. ⚠️ Kök `test` betiği düz `vitest run` ve **`turbo.json`da bir `test` görevi YOK** (ölçüldü: yalnızca `typecheck` ve `build` tanımlı), yani hiçbir yerde `^build` bağımlılığı yok. | **Bugün CI'da GÖRÜNMÜYOR ve bu yanıltıcı bir güven:** `ci.yml`in sırası `typecheck → lint → format → test:coverage → build`, ve `pnpm typecheck` turbo üzerinden `dependsOn: ["^build"]` taşıyor — yani testler koştuğunda `dist` **tesadüfen** taze. Kırılganlık **yerelde**: bir geliştirici `packages/ui/src`i düzenleyip `pnpm test` koşarsa **bayat kodu ölçer** ve yeşil görür. ⚠️ Bu risk 6.3'ten önce de vardı (`apps/web` → `@fms/shared`) ama 6.3 onu **keskinleştirdi**: bir paketteki test, **başka bir paketin sahip olduğu bir envanteri** iddia eden ilk vaka. 6.3'te çözülmedi çünkü çözüm (vitest `alias` ile `@fms/*`i `src`e yöneltmek ya da kök `test`i turbo'ya taşımak) **her paketi** etkileyen bir yapılandırma kararı ve bu alt görevin kapsamı tema (K12). | **6.4** — ilk bileşenler `apps/web`den `@fms/ui`ye bakmaya başlayacak ve bayat `dist` orada *"benim bileşenim mi, önbellek mi"* sorusunu doğurur. Bedel ölçüldü: iki seçenek de küçük (vitest `resolve.alias` ya da kök betiğin turbo'ya taşınması), ama ikisi de **her projeyi** etkiliyor, yani kendi mutasyonuyla kanıtlanmalı: kaynağı boz, **build ETMEDEN** `pnpm test` kırılsın. ✅ **ÖDENDİ — 6.3b** (`6.4` yerine, kullanıcı kararıyla **6.4'ten önce**: token testleri de `@fms/ui`yi çözecekti ve bayat `dist` üzerinde koşsalardı mutasyonları **sessizce geçersiz** kılardı). **Seçilen çözüm ③ `resolve.alias`**; ② (`pnpm test` önce build etsin) **reddedildi** çünkü kök nedeni çözmez, her koşuya bir derleme ekleyerek üstünü örter ve testleri hâlâ bir artefakta bağlar. ③ sınıfı **ortadan kaldırıyor**: birim testin ölçmesi gereken şey **kaynak**; `dist`in ve `exports` haritasının doğruluğu **ayrı bir sorudur** ve onu ölçen kapılar zaten var — nitekim 6.3b'de `@fms/ui`nin `apps/web/Dockerfile`da hiç derlenmediğini **imaj derlemesi** yakaladı, birim testler değil. ⚠️ **İlk deneme hiçbir şey yapmadı:** alias `defineConfig` kökünde duruyordu ve mutasyon **hâlâ** kırmadı — Vitest 4'ün `projects` girdileri ayrı Vite yapılandırmaları ve kök `resolve`u **devralmıyorlar**; alias **sekiz TS projesine tek tek** verildi (*"yazılmış ama hiçbir şey yapmayan bir ayar"*, `ignoredBuiltDependencies` tuzağının aynısı). **İKİ YÖNLÜ KANIT:** ① 6.3'ün mutasyon ④'ü **build etmeden** koşturuldu → **4 test kırıldı** (6.3'te **0** kırmıştı) ② `packages/ui/dist` **tamamen silindi** → **1275/1275 geçti**, yani testler gerçekten `dist` okumuyor. `vitest.integration.config.ts` aliası **almıyor** — entegrasyon testleri `dist` üzerinden çözmeyi bilerek yapıyor |
| **BORÇ-010** | **5 (5.6)** | **`local/no-hardcoded-path` kuralının KABLOLAMA KANARYASI yok.** 5.5 yeni kural için gerçek depoda koşan bir kanarya yazdı (kural sökülür/`off` yapılır/muafiyet genişlerse bir test kırılır) ve **aynı mekanizmayla Faz 1.4'ün kuralının da kablolandığını ölçtü** — ama o iddia bir teste **yazılmadı**. Yani bugün `local/no-hardcoded-path` `eslint.config.js`ten sessizce kalksa `pnpm lint` **0 der** ve hiçbir test bunu söylemez; kuralın 23 senaryoluk `RuleTester` testi kuralı **izole** koşturuyor (D3). | 5.5'in kapsamı **kriter 1'in kuralıydı** (K12); Faz 1.4'ün kuralına test eklemek o alt görevin kapsamı değildi. ⚠️ **Ve gözlem 5.6'ya kadar SAHİPSİZDİ:** yalnızca `docs/reports/faz-05/5.5-*.md` içinde yaşıyordu — ROADMAP'te, kütüklerde ve `SPEC-COVERAGE-GAPS`te **0 eşleşme** (5.6'da ölçüldü). *"Raporda kalan bir gözlemin sahibi yoktur"* (BORÇ-008'in 4.7'deki dersi); bu satır ona bir ev veriyor. ✅ **ÖDENDİ — 6.1.** `tools/eslint-local-rules/no-hardcoded-path.test.mjs`e gerçek depoda koşan bir kanarya eklendi (7 vaka). **Borcun kendisi mutasyonla kanıtlandı:** kural `eslint.config.js`ten **sessizce** çıkarıldığında `pnpm lint` **exit 0** verdi (`--print-config` → `no-hardcoded-path: undefined`, `no-bare-jsx-text: [2]` — iki yönlü kontrol) ve **yalnızca yeni kanarya** kırıldı (4 vaka; `no-bare-jsx-text`in kanaryası **etkilenmedi**). 6.1 öncesi bu mutasyon **hiçbir şeyi kırmazdı**. **Mutasyon 5/5:** ① `index.js`ten sökme ② `error`→`warn` ③ yol `ignores`a ④ muafiyet genişler ⑤ muafiyet tamamen kalkar. ⚠️ **Kopyalanmadı, uyarlandı** (4.8 idiomu): bu kuralın muafiyeti **İKİ EKSENLİ** (birim testler **+ yol tanım yerleri**) ve `no-bare-jsx-text`in ikinci ekseni **yok** — *"bir kuralın iki ekseni varsa, birini doğrulamak diğerini doğrulamaz"*, bu yüzden **iki ayrı muafiyet vakası** var ve mutasyon ⑤ ikisini birden kırdı. ℹ️ ①'in yan etkisi ölçüldü ve yazıldı: kural `index.js`ten silinince config **hiç yüklenmiyor** (ESLint eklentide bulamıyor) ve `no-bare-jsx-text`in kanaryası da kırılıyor — yani ① kaba bir mekanizmayla yakalıyor, borcun asıl senaryosunu kanıtlayan mutasyon **②**'dir | **6** — ✅ **ÖDENDİ 6.1'de** (ayrıntı solda). ⚠️ **BU HÜCRE 6.4-ön'E KADAR HİÇ YOKTU:** satır 5.6'da **dört** hücreyle yazıldı, oysa tablo başlığı **beş** sütun bildiriyor; Markdown eksik hücreyi sessizce boş gösteriyor ve gözle fark edilmiyordu. `debt:check` ilk koşusunda kırıldı ve satırı adıyla gösterdi — bir ayrıştırıcının insanın göremediğini görmesinin somut vakası |
| **BORÇ-009** | **5 (5.6)** | **K5 kapılarının ORTAK KÖR NOKTASI: modül düzeyi metin sabitleri.** Dosya tepesinde duran `const TAB_LABELS = { logs: 'Kayıtlar' }` JSX'e render edilir ve kullanıcı görür, ama **ne** `local/no-bare-jsx-text` (JSX içine bakıyor) **ne de** `i18n:check` (anahtar kümesine bakıyor) onu yakalar. 5.4'te tam bu sınıftan bir ihlal iki aracın da dışında kaldı; yakalayan şey BORÇ-003'ün onları **adıyla sayması** oldu. | **İki ayrı ölçüm bir heuristiği çürüttü, gerekçe tahmin değil.** ① 5.5: *"modül düzeyi dize haritası JSX'te kullanılıyorsa bildir"* kuralı `DebugPanel.tsx`teki **dört** haritanın **dördüne** öterdi (`TABS` iç kimlikler · `LEVEL_COLORS` CSS · iki i18n anahtar tablosu — yani çözümün kendisi). ② 5.6: kural anahtar kümesiyle daraltıldı ve yine ölçüldü — `apps/web/src`te **17** modül düzeyi dize sabiti var, **5'i** JSX'e akıyor, heuristik **3'ünü yanlış bildirirdi** (`DEV_PANEL_SENTINEL` · `TABS` · `LEVEL_COLORS`) ve **gerçek pozitif 0**. Bugün modül düzeyinde düz metin taşıyan tek sabit `I18N_BOOT_FAILURE_MESSAGE` ve o JSX'e **akmıyor** (5.4'ün bilinçli K5 muafiyeti). *"Yanlış pozitif üreten bir kapı kapatılır ve bir daha hiçbir şey yakalamaz."* Eksik bilgi **anlamsal**: *"bu dize düz metin mi, teknik kimlik mi?"* — AST'de de anahtar kümesinde de yok. | **6** — ve gerekçe yapısal, ertelemek değil: bugün **sıfır** gerçek vaka var, ilk gerçek vakalar tasarım sistemi bileşenleriyle gelecek. O gün elde **örnek** olacak ve bir heuristik ölçülebilir hâle gelecek; bugün ölçülemez. ⚠️ Faz 6'ya gelindiğinde ilk iş **yeniden ölçmek**: heuristik hâlâ 0 gerçek pozitif veriyorsa çözüm bir kapı değil bir **bileşen inceleme disiplinidir** ve öyle yazılır (SAPMA-026 — kimsenin belirlemediği alana değer uydurma) |
...
| BORÇ-003 | 2 | ✅ **ÖDENDİ — 5.4.** Metinler `t()`den geliyor; `title` propu **`titleKey`e** dönüştü (kapalı bir anahtar birleşimi), yani çağrı yerinin de K5'e uyması **tip seviyesinde zorunlu** — bir dize kalsaydı çağıran taraf yine sabit metin yazabilirdi. ⚠️ **`DebugPanel` de ÇEVRİLDİ ve kütükteki gerekçe ÇÜRÜTÜLMEDİ, GENİŞLETİLDİ:** *"panel dev-only, üretim paketinde yok"* **doğruydu ve doğru kalıyor** — K5 kullanıcıya görünen yüzeyi koruyor, panel orada değil. Değişen şey **ikinci bir mekanizmanın** aynı satırlara bakması: 5.5'in ESLint kuralı **JSX'e** bakıyor, üretim paketine değil. **İki mekanizma, iki farklı kapsam.** Alternatif bir `eslint-disable` bloğuydu ve o kısmi korumadır: bugünkü on iki dizeyi affederken yarın eklenecek on üçüncüyü de sessizce affederdi. 🆕 Panelin **modül düzeyi** sekme metinleri envanter aracının **kör noktasındaydı** ve yalnızca bu satır onları adıyla saydığı için yakalandı. ⟨aşağıdaki özgün kayıt⟩ **`ErrorBoundary` yedek arayüzündeki Türkçe metinler koda gömülü** (`apps/web/src/components/ErrorBoundary.tsx`: başlıklar, "Bu bölüm yüklenemedi…", "Tekrar dene", bildirim durumu). K5 arayüzde sabit Türkçe metni yasaklıyor. **⚠️ 2.8'DE KAPSAM GENİŞLEDİ:** `apps/web/src/components/dev/DebugPanel.tsx` de aynı sınıf metin taşıyor (sekme adları, üç boş sekmenin açıklaması, "Temizle", "Kapat"). **Ama önceliği DAHA DÜŞÜK ve bu bilinçli:** panel **dev-only** — üretim paketinde hiç yok (kaynak haritasıyla kanıtlandı), yani hiçbir kullanıcı o metinleri görmüyor. Faz 5'te `ErrorBoundary` çevrilirken panel **atlanabilir**; K5'in koruduğu şey kullanıcıya görünen yüzey. | i18n Faz 5'te geliyor; **BORÇ-005 ile aynı sınıf** (o sunucu hata gövdesi, bu tarayıcı yedek arayüzü). Sınırın çalışması için metin şart: i18n'i beklemek, Faz 5'e kadar çöken her ekranın **boş** kalması demekti. Metinler `TODO(Faz 5)` yorumlarıyla işaretlendi ve tek bileşende toplandı — Faz 5 işi bir dosyada `t()` çağrılarına çevirmeye iner. `title` zaten **prop**, yani çağrı yerleri hazır. | **5** — i18n kurulurken. ⚠️ **VE 4.11'DE ÖLÇÜLDÜ: bu satır ROADMAP'in Faz 5 bölümünde HİÇ GEÇMİYORDU** — `BORÇ-003` ROADMAP'te yalnızca **Faz 2** bölümünde vardı, yani borcun **açıldığı** yerde, **ödeneceği** yerde değil. G-13'ün ve G-17'nin aynı sınıfı: *"kapsam taşıması kütüğe kayıtla bitmez, hedef fazın kapsamında görünmeli."* ✅ Faz 5 kapsamına **ve bir kabul kriterine** yazıldı (4.11). |
| BORÇ-005 | 2 | ✅ **ÖDENDİ — 5.4.** `MESSAGE_BY_KIND` ve `UNEXPECTED_MESSAGE` **silindi**; `exceptionMessageFor` de gitti. ⚠️ **GÖVDE SÖZLEŞMESİ DEĞİŞTİ ve bu bir API yüzeyi değişikliğidir:** `message` alanı gövdeden **çıkarıldı** — kütüğün kendi çözümü buydu (*"tablo silinir, istemci `t()` ile üretir"*). Tüketicisi **sıfırdı** ve ölçüldü: `api.ts` hata gövdesini **hiç parse etmiyor** (`response.json()` yalnızca başarı yolunda; hata yolunda ondan önce fırlatılıyor). ⚠️ **Yedek `kind` üzerine DEĞİL `status` üzerine kuruldu — SAPMA-038:** `kind` gövdeye hiç girmiyor, yalnızca log bağlamında var; `status` gövdede var ve `exceptionMessageFor` zaten ona bakıyordu. O eşleme artık `errors.json`daki `status.*` ailesi. ⟨aşağıdaki özgün kayıt⟩ **Hata gövdesindeki Türkçe metinler koda gömülü** (`MESSAGE_BY_KIND`, `apps/api/src/common/filters/global-exception.filter.ts`). K5 arayüzde sabit Türkçe metni yasaklıyor. | i18n Faz 5'te geliyor; 2.6'nın BORÇ-003'üyle **aynı sınıf** borç. Metin `AppError.message`'tan alınamıyor çünkü o alan bilinçli olarak **geliştirici mesajı** (`errors.ts`: *"loga ve Sentry'ye gider, çevrilmez, kullanıcıya gösterilmesi hedeflenmez"*) — doğrudan gövdeye konsaydı iç ayrıntı sızardı. Tablo bir **yedek**: sözleşmenin aslı `code` + `context` ve ikisi de gövdede dönüyor, yani Faz 5 işi `t('errors:' + code, context)` yazmaya iner, fırlatma yerlerini gezmeye değil. Metinler bilerek **genel** tutuldu ki hataya özgü cümle `code` üzerinden gelsin. | **5** — i18n kurulurken tablo silinir, istemci `code`+`context`ten üretir. ⚠️ **BORÇ-003 ile aynı ölçüm (4.11): bu satır da ROADMAP'in Faz 5 bölümünde hiç geçmiyordu.** ✅ Faz 5 kapsamına ve aynı kabul kriterine yazıldı. |
...
|---|---|---|---|---|---|
| SAPMA-045 | `karar` | 6 (6.6-ön) | **SUNUCU MODU KARARI — oyun `SERVER_MODE=private` ile çalışacak; yalnızca izin listesindeki hesaplar oynar.** Kararı veren: **kullanıcı**, tarih: **2026-09-11**. `CLAUDE.md` §1.1 *"herkese açık kayıt, ücretsiz, reklamsız"* diyordu; §16.3 başarı tanımı *"10+ gerçek kullanıcı 1 hafta test etmiş"* istiyordu. İkisi de **davetli kurulum** kararıyla çelişiyordu — ve anayasa **kendi ortam dosyasıyla da** çelişiyordu: `.env.example` `SERVER_MODE=private` diyor ve gerekçesini yazıyor; `docs/spec/08` §10.1 *"Varsayılan `private`"* diyor. Yani sapma spec'ten değil, anayasanın **hedef cümlesinden**. ⚠️ **YALNIZCA İFADE:** hiçbir uygulama kararı, hiçbir kod. Mod sistemi Faz 13'te spec'in yazdığı gibi kurulur; bu karar onun **hangi modda işletileceğini** söyler. | **İki sonucu var ve ikisi de hedef fazlara devredildi:** (a) **KVKK metinleri public'e geçilmedikçe gerekmiyor** — `docs/LEGAL/` Faz 50'de yazılır ama yayın kriteri değil (b) **gerçek arma/portre maruziyeti davetli kurulumda çok daha dar** — Faz 7–9'un gerçek varlık kararları bu bilgiyle alınır. Kriter tarafında: *"10+ gerçek kullanıcı"* davetli kurulumda anlamsız; **silinmedi, işlevi yerine kondu**: *"en az bir tam sezon uçtan uca oynanmış ve oynanabilir bulunmuş"* (DZ-15 — bir kriteri işlevini yerine koymadan silmek, kriteri sessizce düşürmektir). ℹ️ `karar` tipi: koşullar değişirse (public'e geçiş) yeniden değerlendirilir. | ✅ `CLAUDE.md` §1.1 (hedef + ölçek satırı) · ✅ `CLAUDE.md` §16.3 (kriter yerine kondu) · ✅ `docs/ROADMAP.md` **Faz 7 · Faz 8 · Faz 9 · Faz 50 KAPSAMINDA adıyla** (`SAPMA-045`, dördü de `grep` ile doğrulandı) · ✅ ROADMAP Faz 50 *"Kabul testi"* satırı aynı kriterle · ℹ️ `docs/spec/08` §10.1 ve `docs/spec/10` §13 **zaten `private` varsayılanı yazıyordu — değişmedi** · ℹ️ ROADMAP Faz 13 hedefi (*"Herkese açık kayıt — ama sunucu üzerinde tam kontrol sizde"*) ve Faz 47 (*"herkese açık ve adil karşılaştırma"*, *"açık kayıt gereği"*) **bilerek dokunulmadı**: ikisi de **yeteneği** tarif ediyor (kayıt teknik olarak açık, mod sistemi, moderasyon), işletme modunu değil; K12 · ℹ️ `docs/MASTER-SPEC.md` donmuş arşiv, dokunulmadı |
| SAPMA-044 | `düzeltme` | 6 (6.4-ön) | **`spec/09` §11.5'İN "FAZ KAPANIŞ KOMUTLARI" LİSTESİ EKSİKTİ — CI'da koşan iki kapı orada hiç geçmiyordu.** Ölçüldü: blok 13 komut sayıyor (`typecheck` · `lint` · `test --coverage` · `build` · `test:db` · `test:e2e` · `validate:world` · `validate:save` · `sim:balance` · `sim:seasons` · `i18n:check` · `perf:budget` · `arch:check`), ama **`gaps:check` yok** — oysa 4.11'de yazıldı, 5.6'da CI'a bağlandı ve `SESSION-TEMPLATE` adım 21 onu adıyla koşturuyor. `debt:check` aynı alt görevde doğdu. Yani spec'in *"kapanışta bunları koştur"* listesi, deponun gerçekten koşturduğu kapıların **ikisini** taşımıyordu. | **Kısmi bir liste D3 yanılsaması üretir** ve bu blok tam olarak satır satır uygulanan bir yerdir: bir faz kapanışında okuyan kişi listeyi tamamlar, yeşil görür ve iki kapı **sessizce** atlanır. Bu bir `karar` değil `düzeltme` — spec'in **tam envanter** iması ölçümle çürütüldü. ⚠️ Aynı sınıfın üçüncü tekrarı: G-01 (`perf:budget` spec istiyor, hiçbir faz kurmuyor) ve `arch:check` (Faz 1 Ç3) da *"komut yazıldı, koşacağı yer yazılmadı"* idi. ℹ️ `format:check` bilerek eklenmedi — o bir **kapı** ama §11.5 faz kapanışının **doğrulama** komutlarını sayıyor ve biçim `lint` zinciriyle birlikte her commit'te koşuyor; eklemek listeyi CI adım listesinin kopyası yapardı (iki liste bir gün ayrışır). | ✅ `docs/spec/09-quality-protocol.md` §11.5 — iki satır + gerekçe kutusu. ✅ `docs/SESSION-TEMPLATE.md` adım 21 `debt:check`i de koşturuyor. ✅ Kablolama tarafı koşan bir nöbetçide: `scripts/inventory-guards.test.mjs` ① (`:check` deseni, `perf:budget`in desene girmediği ROADMAP 6.10'da adıyla yazılı). ⚠️ **KULLANICI ONAYI 6.4'te ALINDI ve gerekçe BİR KADEME KESKİNLEŞTİ — kalıcı ders bu:** *"**Envanter tipi bir spec bölümü ile NORMATİF bir bölümün barı aynı değildir.**"* §11.5 bir **envanterdir** — *"faz kapanışında şu komutlar koşar"* diyor ve **var olan** komutları sayar; depoda koşan bir kapıyı eklemek spec'in **niyetini** değiştirmiyor, **listeyi gerçeğe hizalıyor**, ve eksik bırakmak listeyi **yanlış** yapıyordu. Karşılaştırma: 6.1'in §11.6'ya eklediği *"üretim paketi boyutu"* bir **metrik** ve **normatif** — yeni bir bütçe kalemi yaratıyordu ve SAPMA-042'yi hak ediyordu. `SESSION-TEMPLATE`in *"spesifikasyonda eksik varsa **sor**"* kuralı bir fazın spec'in **niyetini** sessizce yeniden şekillendirmesine karşıdır; bir envanteri gerçeğe hizalamak o sınıfa **girmiyor**. ⚠️ Bu ayrım yazılmasaydı bir sonraki faz bunu *"spec düzenlemeleri sormadan yapılır"* diye okurdu — **o zaman gerçekten bir sınır aşılırdı** |
| SAPMA-043 | `düzeltme` | 6 (6.2) | **6.1'İN YAZDIĞI KABUL KRİTERİ, SPEC'İN ÇÖZMEDİĞİ BİR ŞEYİ İSTİYORDU — ve kaynak BENDİM, spec değil.** 6.1 kriter 5'i şöyle daraltmıştı: *"Nitelik ısı skalasının 8 kademesi, renk körlüğünün **üç tipinde** WCAG AA kontrast aritmetiğiyle ayırt edilebiliyor."* *"Üç tipinde"* bir **renk dönüşümü** (Brettel/Viénot/Machado sınıfı CVD simülasyon matrisi) ima ediyor. 6.2'de `spec/05` §7.2 okundu: *"1–20 nitelikler renkle kodlanır. **Renk körlüğü modunda** ek olarak **sayı kalınlaşır** ve **arka plan deseni** eklenir."* Yani spec renkleri **hiç değiştirmiyor** — mekanizması renge bağımlı olmayan **ikinci ve üçüncü kanal** (ağırlık + desen). Kriter, spec'in mekanizmasıyla kapanamıyordu. ⚠️ Ve ROADMAP'in 6.2 alt görev satırı da aynı hatayı taşıyordu (*"+ renk körlüğü 3 tipi"*). | **İki dürüst çıkış vardı ve HİZALAMA seçildi: kriter spec'e uyduruldu, spec genişletilmedi (otorite #1).** Gerekçe bir tercih değil bir **ölçüm**: sekiz bandın komşu kontrastları 6.2'de hesaplandı — `1-3↔4-6` **1,71** · `4-6↔7-9` **1,66** · `7-9↔10-11` **1,37** · `10-11↔12-13` **1,13** · `12-13↔14-15` **1,09** · `14-15↔16-17` **1,04** · `16-17↔18-20` **1,16**. En yüksek fark **1,71:1**, yani **tam renkli görüşte bile** parlaklık komşu bantları ayırmıyor. Spec'in yedekli kodlaması bir süs değil **mekanizmanın kendisi**, ve bir CVD simülasyonu bir *mekanizma* değil bir *doğrulama tekniği* olurdu — onun yeri gerçek tarayıcı, yani **Faz 49**. ⚠️ **Emsal SAPMA-039'un tersi yönü:** orada bir **ad** ölçümle çürümüştü, burada bir **kriter metni**; ikisinde de düzeltilen şey kod değil **iddia**. ⚠️ **Ve kaynağın kendisi bu oturumdu:** 6.1'in metnini ben yazdım ve spec'i o gün bu ayrıntıda okumamıştım — *"kendi yazdığın plan kaynak değildir"* (D7) kendi bir önceki alt görevim için de geçerli. | ✅ `docs/ROADMAP.md` → Faz 6 kabul kriteri 5 yeniden yazıldı + gerekçe bloğu · 6.2 alt görev satırından *"renk körlüğü 3 tipi"* **çıkarıldı** · payların bölünmesi yazıldı (**6.2**: ölçek + kontrast aritmetiği, kapandı · **6.6**: ağırlık + desen). Kriter **[ ] kalıyor** |
| SAPMA-042 | `düzeltme` | 6 (6.0 → 6.1) | **`spec/09` §11.6 PERFORMANS BÜTÇESİ TABLOSU, ROADMAP'in FAZ 6'YA VERDİĞİ METRİĞİ TAŞIMIYORDU — VE KALAN METRİKLERİN HİÇBİRİ BU FAZDA ÖLÇÜLEMİYOR.** ROADMAP Faz 6 kapsamı `pnpm perf:budget`in bu fazda ölçeceği alt kümeyi *"DataTable render, ekran geçişi, **üretim paketi boyutu**"* diye sayıyor. 6.0'da ölçüldü: §11.6'nın metrik satırlarında `paket\|bundle\|boyut` → **0 eşleşme**, ROADMAP §0.4'ün kısaltılmış tablosunda da **0**. Ve kalan 15 metriğin tamamı ya gerçek bir tarayıcı (LCP · Lighthouse ×2 · 2D fps ×2 · bellek), ya gezilebilir bir uygulama (ekran geçişi → Faz 17), ya bir ekran (kadro tablosu → Faz 18), ya motor/veritabanı (maç ×3 · tur · sezon · transfer araması · kayıt yazma) istiyor. **Faz 6'da ölçülebilen: 0 / 15.** | **Bu satır olmadan G-01'in kapısı kurulur, yeşil yanar ve hiçbir bütçeye BAKMAZ** — SAPMA-024'ün (*"bakacak bir şey bulamayan kapıya ✅ yazılmaz"*) tam biçimi, ve kabul kriteri 2 yalnızca bir fixture'la kanıtlanıp gerçek depoda **hiçbir şeyi korumazdı**. ⚠️ **Bütçenin BİÇİMİ uydurulmadı (SAPMA-026):** mutlak bir bayt eşiği bugün **türetilemez** çünkü Faz 6 Tailwind + Radix + 28 bileşen ekliyor ve paket meşru olarak büyüyecek. Bu yüzden bütçe bir **regresyon kapısı**: taban dosyada kayıtlı (prose'da değil — bir sayı prose'da yaşarsa bayatlar), ihlal tabana göre **> %10** büyüme, ve tabanın güncellenmesi bir **commit**, yani görünür bir karar. δ = %10 bir **kalibrasyon** olarak etiketlendi (§16.2'nin denge katsayılarıyla aynı sınıf); dayanağı depodaki tek ölçülmüş vaka — Faz 1'in `NODE_ENV` sızıntısı **429 kB → 228 kB**, yani kaza **+%88**. **Taban 6.1'de yeniden ölçüldü** (`Cached: 0` soğuk build): **380.908 bayt** — devir notundan alınmadı. | ✅ `docs/spec/09-quality-protocol.md` §11.6'ya satır + gerekçe bloğu. Kapının kendisi **6.10**'da (G-01); ölçüm ve negatif test orada |
| SAPMA-041 | `düzeltme` | 6 (6.0 → 6.1) | **ROADMAP FAZ 6'NIN KABUL KRİTERİ, KENDİ KAPSAMIYLA ÇELİŞİYORDU: kapsam 28 bileşen sayıyor, kriter "30+" istiyor.** 6.0'da programatik sayıldı: temel bileşenler **18** (Button…Skeleton) + alan-özel bileşenler **10** (`AttributeBadge`…`DateChip`) = **28**. Kriter 1 *"Storybook'ta **30+** bileşen"* diyordu. Yani kriter, fazın **kapsamının kendisiyle** kapanamıyordu. | **Sayı, listeyi ona uydurma baskısı üretir.** Kapsamda bileşen sayılabilecek iki şey daha var (DataTable + tema sağlayıcısı) ve 28 + 2 = **tam 30**, yani kriter **sıfır marjla** karşılanırdı: 28'in biri birleştirilse kriter düşer, ve *"30'a saymak"* için bileşen icat etme baskısı doğardı — **bir kriteri kapatmak için kapsam esnetilmez.** Çözüm deponun kendi idiomu: **bir SAYI değil bir LİSTE** (4.5'in `VISIBLE_ATTRIBUTES`i · 5.1'in ek vakaları · on namespace listesi · `docs/glossary.md`nin terim sayısını prose'a **yazmama** kararı). ⚠️ **Ve eşleşme ÇİFT YÖNLÜ iddia edilir** (5.7'nin §14 ↔ glossary testinin dersi): yalnızca *"envanterdeki her bileşenin hikayesi var"* yetmez — *"hikayesi olan her bileşen envanterde"* de gerekir, yoksa envantere eklenmeden yazılmış bir bileşen **görünmez**. Envanter **tek yerde** yaşar. | ✅ `docs/ROADMAP.md` Faz 6 kabul kriteri 1 yeniden yazıldı + kriter daraltması tablosu. Envanter ve çift yönlü test **6.9**'da (Storybook) yazılır |
| SAPMA-040 | `düzeltme` | 5 (5.7) | **`77 + 57 = 134` İKİ ENVANTERİN AYRIK OLDUĞUNU VARSAYIYORDU VE BU HİÇ ÖLÇÜLMEMİŞTİ.** 5.0'ın SONUÇ (f) bloğu sözlüğün ≥120 eşiğini *"tek terim uydurmadan"* şöyle karşılıyordu: `CLAUDE.md` §14 **77** + nitelik envanteri **57** (47 görünür + 10 gizli) = **134**. Toplama işlemi ancak iki küme **ayrık**sa geçerli ve o gün kesişim sorulmadı. 5.7'de programatik ölçüldü: **kesişim 1** — `HIDDEN_ATTRIBUTES`in `injuryProneness` üyesi ile §14'ün `Injury Proneness \| Sakatlığa Yatkınlık` satırı **aynı terim**. Gerçek sayı **133**. ℹ️ Aynı ölçüm 5.0'ın **saymadığı üç envanteri** de buldu (`STAFF_ATTRIBUTES` 16 · `MANAGER_ATTRIBUTES` 8 · `POSITION_LEVELS` 5, üçü de §14 ile 0 kesişimli) — yani havuz aslında **162**. | **Kriter etkilenmiyor** (133 ≥ 120), yani bu bir kusur değil bir **sayı düzeltmesi** — ama düzeltilmezse bir sonraki okuyucu 134'ü yeniden türetir ve sözlükte olmayan bir terim arar. ⚠️ **Asıl kazanç sayı değil, dedupe'un GÖRÜNÜR olması:** iki envanteri körü körüne birleştiren bir sözlük `Injury Proneness`i **iki kez** taşırdı ve *"hiçbir terim tekrarlanmıyor"* testi bunu ancak yazıldıktan sonra yakalardı. Ölçüm önce yapıldığı için terim **çekirdek biçimiyle bir kez** yaşıyor ve karar bir testte adıyla sabit. **Emsal SAPMA-034** (ROADMAP'in kayan migration numaraları): orada da kayıt yanlış değil **bayattı** ve düzeltme kodu değil belgeyi hizaladı. ⚠️ **5.0'ın SONUÇ bloğu GERİYE DÖNÜK DEĞİŞTİRİLMEDİ** — o bir alt görev kaydı; düzeltme 5.7'nin SONUÇ'unda ve burada yaşıyor. | ✅ `docs/ROADMAP.md` → 5.7 SONUÇ bloğu (kesişim, üç ek envanter, 162'lik havuz) + kabul kriteri 5'in metni. Sözlük tarafı: `docs/glossary.md` §4 dedupe'u adıyla açıklıyor, `tools/glossary-check/index.test.mjs` onu iddia ediyor |
| SAPMA-039 | `düzeltme` | 5 (5.5) | **ESLint KURALININ ADI VE FAZIN KAPSAM CÜMLESİ, KENDİ ÖLÇÜMÜYLE ÇELİŞİYORDU.** `tools/eslint-local-rules/index.js` Faz 1.4'ten beri ikinci kural için `no-hardcoded-turkish` adını ayırmıştı; ROADMAP'in Faz 5 kapsam cümlesi *"JSX içinde çıplak **Türkçe** metin **yasak**"* diyordu; ana dosyalar bloğu `no-hardcoded-turkish.js` gösteriyordu. **5.4 o sıfatı iki yönde birden çürüttü ve ölçüm ROADMAP'in 5.5 maddesine zaten yazılmıştı:** `ErrorBoundary.tsx`teki `Tekrar dene` gerçek bir K5 ihlali ve **hiçbir Türkçe'ye özgü karakter taşımıyor** (`çğışöü` taraması → **0**); `App.tsx`in `base` / `api prefix` etiketleri **İngilizce** ve yine de ihlal, çünkü kullanıcı onları ekranda görüyor. Yani kural **dile bakamaz**; yakalayabildiği şey *"JSX'te çıplak metin"*. Ad → **`no-bare-jsx-text`**. | **Ölçüm bir yerde yazılı, ad başka bir şey söylüyordu — ve ikisi bir arada duran bir belge, okuyanı adın söylediğine inandırır.** Bir kural örneklerinden geriye okunursa yanlış öğrenilir (F3): `no-hardcoded-turkish` adını gören biri bir gün *"benim etiketim İngilizce, bu kural beni ilgilendirmiyor"* der ve kuralın 33 ihlalinin **21'i** (`App.tsx` + `main.tsx`) tam olarak o sınıftandı. **Bir ad bir sözleşmedir; ölçüm adı çürütürse ad düzeltilir.** Emsal SAPMA-034 (ROADMAP'in kayan migration numaraları): orada da kayıt yanlış değil **bayattı**, ve düzeltme kodu değil **belgeyi** hizaladı. ⚠️ Kuralın **davranışı** değişmedi — değişen tek şey adının o davranışı doğru anlatması. | ✅ `docs/ROADMAP.md` Faz 5 → kapsam cümlesi (*"Türkçe"* sıfatı kaldırıldı ve gerekçesi yazıldı) · ana dosyalar bloğu (`no-bare-jsx-text.js`) · 5.5 maddesinin başlığı ve SONUÇ bloğu · kabul kriteri 1. Kod tarafı: `tools/eslint-local-rules/index.js` (ayrılan satır + kayıt) · `eslint.config.js` · kuralın kendi başlığı |
...
|---|---|---|---|---|---|
| #43 | 6.6c (SINA) | **Kod satırlarında 0 etiketsiz sınıf kalmışken derlenmiş CSS'te yanlış çiftler HÂLÂ VARDI** (`font-weight:var(--font-ui\|mono)` · `color:var(--text-2xs\|sm)`) — soğuk build, ilk ölçüm. | Tailwind 4 `@source '…/packages/ui/src'` dizinini **düz metin** tarıyor: 6.6'nın beş bileşeni + üç testi hatayı **JSDoc'ta örnekle anlatıyordu** ve o örnekler yardımcı üretiyordu (üretim CSS'inde ölü kurallar). `no-bare-jsx-text`in *"yorumlar düğüm değildir"* güvencesi ESLint için doğru, Tailwind'in tarayıcısı için **değil**. | 15 yorum geçişi `[…var(` biçimine çevrildi (ASCII dışı aday reddediliyor — depoda `font-[var(…)]` emsali zaten böyle davranıyordu); ikinci soğuk build → yanlış çift **0**, CSS 33.844 → 33.778 B. | **Yorumdaki örnek sınıf da derlenir.** Bir sınıf adını yorumda yazarken tarayıcının aday sayamayacağı biçim kullanılır; *"derlenmiş CSS'te 0"* iddiası **yorumlar dâhil** ölçülür. D5'in bu biçimi: kaynak temiz, çıktı kirli — çıktı ölçülmeden *"düzeldi"* denmez. |
| #42 | 6.6c (YAZ) | **`pnpm lint` yeni kuralın KENDİ dosyasında ısırdı:** `preserve-caught-error` — `catch (error)` içinde `throw new Error(...)` `cause` taşımıyordu (35 problem = 34 hedef + 1 bu). | Anayasa kuralı (§1.3 hata kuralları) ESLint 10 çekirdeğinde; hata mesajına `error.message`i **metin olarak** gömmüştüm, nedeni zincirlemek yerine. | `new Error(mesaj, { cause: error })`; mesajdan `error.message` çıkarıldı. | Bir kapı yazarken kapının kendisi de kapılardan geçer; *"34 hedef ötüyor"* diye sevinmeden **35. satır okunur** (ham çıktı, sayı değil). |
| #41 | 6.6c (YAZ) | **RuleTester iki geçersiz vakada kırmızı:** *"Hydrated message … does not match"* — kural doğru mesajı üretiyordu, çıktı bunu gösteriyordu. | `errors[].data` **kısmi** verilince RuleTester şablonu yalnızca verilen alanlarla hidratlıyor ve `{{token}}` yer tutucularını olduğu gibi bırakıp karşılaştırıyor — D6'nın vakası: kırmızı test, kod değil. | İki vakaya **tam** `data` (altı alan) yazıldı; artık mesajın önerdiği ipucu (`length:` / `family-name:`) **adıyla** iddia ediliyor. | `data` ya tam verilir ya hiç; yarım `data` bir iddia değil, bir tuzak. Kırmızı testte önce *"hangisi yanlış"* — çıktıdaki gerçek mesaj kodun doğru olduğunu gösteriyordu. |
| #40 | 6.6b (kayıt) | **Kirli ağaçtaki 6.7 notu VAR OLMAYAN bir lint kuralını şimdiki zamanda iddia ediyordu** — *"`local/no-untyped-arbitrary-value` lint kuralı reddediyor"*; depoda böyle bir kural yok (`tools/eslint-local-rules/`: `no-bare-jsx-text` · `no-hardcoded-path`, `ls` ile bakıldı). | İlk oturum 6.6b planını yazarken ②'nin (SORUN-002) nöbetçisini **yazılmış gibi** adlandırdı — D1'in ROADMAP biçimi: ölçüm yokken makul görünen bir ad, ve geleceği şimdiki zamanda anlatan bir cümle. | Not *"6.6c'de çözülür"* + *"nöbetçinin biçimi 6.6c'nin ölçümüyle seçilir"* oldu; kural adı silindi (commit edilmeden). | **DZ-10:** yazılmamış bir kural *"reddediyor"* diye yazılmaz. Bir plan maddesi geleceği **şimdiki zamanda** anlatıyorsa D1'dir — ROADMAP'te de, SONUÇ bloğunda da. |
| #39 | 6.6b (ölçüm) | **Devir notunun iki sayısı ölçümle uyuşmadı:** *"star-rating'de beş Türkçe dize"* → on dosyada **29** satır (star-rating **8**); *"form-indicator 6 etiketsiz"* → 6 etiketsiz, **5'i** yanlış özelliğe derleniyor (biri renk, tesadüfen doğru). | Devir notu bir **sonuç cümlesini** taşıyordu, ölçüm çıktısını değil (D7); *"etiketsiz"* ile *"yanlış özelliğe derlenen"* aynı küme değil ve tek sayıya sıkıştırılmıştı. | Sayılar yeniden sayıldı (Node betiği: yorum satırı hariç, tırnak içi Türkçe harf, `RangeError` bağlamı 29/29) ve **iki küme ayrı** yazıldı (etiketsiz 6 · bozuk 5); sonuç cümleleri (K5 temiz · form-indicator sapma) **değişmedi**. | **DZ-01'in devir biçimi:** not doğru sonucu taşısa bile sayı yeniden sayılır; iki farklı kümeye tek sayı verilmez — ölçek ile sapma ayrı satırdır. |
| #38 | 6.6b (ölçüm) | **Ham `grep` SORUN-002 ölçeğini 44 geçiş / 17 dosya verdi — devir notu 34 / 12 diyordu.** | **D2:** desen `(font\|text)-\[var\(` **yorum satırlarını** da yakaladı — 6.6'nın beş dosyası hatayı JSDoc'ta **anlatıyor** (`attribute-badge` · `club-crest` · `currency-value` · `date-chip` · `player-portrait`), sınıf taşımıyor. Ayrıca `grep -P` yerel ayarda çalışmadı (*"supports only unibyte and UTF-8 locales"*). | Yorum satırları (`^\s*(\*\|//)`) dışlandı → **34 / 12**, devir notuyla birebir; iki sayı da rapora yazıldı (ham + ayıklanmış). Türkçe harf taraması `grep -P` yerine Node betiğiyle (`\u` kaçışları) yapıldı. | **Beklenmedik sonuçta önce araç (DZ-02):** sapma devir notunda değil desendeydi. Bir sınıf deseni yorumlarda da geçer; **6.6c'nin nöbetçisi kod satırına bakmalı**, yoksa bu beş dosyada yanlış öter — 6.6c ① bunu taşıyor. |
| #37 | 6.6b (süreç) | **İlk oturum bağlam dolunca KESİLDİ — denetim bitmişti, kayıt yazılamadı.** Ağaç kirli kaldı: ROADMAP'te commit edilmemiş 6.6b bloğu (+41 −9), başka dosya yok. | 6.6b altı madde taşıyordu: ① denetim **okuma-ağır** (rapor §2–§5 + on dosya + testler + derlenmiş CSS), ②–⑥ düzeltme **yazma-ağır**; ikisi tek bağlama sığmadı. §0.5 ajan sayar (10 < 15 ✓), bağlamı saymaz. | Kullanıcı kararı: 6.6b = denetim (bu kayıt), **6.6c** = ②–⑥ (ROADMAP'te beşi adıyla, DZ-15 — hiçbiri düşmedi). Kesilen oturumun ölçümleri **devir notu** olarak geldi; ucuz olanlar yeniden sayıldı (D7 — #38/#39 bu sayımdan çıktı), pahalı olan yoktu. | Okuma-ağır bir denetimle yazma-ağır bir düzeltme aynı alt göreve konmaz. Devir notu **sonuç** değil **ölçüm listesi** olarak yazılır ki yeniden sayılabilsin; bu turda öyle geldi ve işe yaradı. |
| #36 | 6.6 (kayıt) | **CI'da §③ `son_commit_baslik` KIRMIZI** (koşu #119, iki mimaride birebir: 1 failed / 1701 passed) — kod commit'i `bbf3e5e` tek başına push edilince. | **Nöbetçinin tanımı, kusuru değil:** §③ *"temiz ağaçta `son_commit_baslik` = HEAD'in başlığı"* der; §1.4 bir alt görevin birden fazla commit (içerik + hafıza) taşımasına izin verir. 6.4/6.5/6.6-ön'de ikisi birlikte push edildiği için CI yalnızca son hâli gördü; bu kez içerik commit'i ara push oldu. | Kayıt commit'i CHECKPOINT'i güncelleyip kapatıyor; nöbetçi **gevşetilmedi**. | **Kabul edilen davranış, kütüğe yazıldı:** ara push = §③ kırmızı, kayıt commit'i kapatır. Bir gün biri *"§③ gereksiz kırmızı veriyor"* derse cevap burada. Yan etki: `quality` kırmızıyken `İmaj` işi koşmuyor → D5 imaj yüzeyi o koşuda ölçülemez. |
| #35 | 6.6 (SINA) | **`denetci` HİÇ KOŞMADI** — kapıcı 32 dk sonra teslim etti, denetçi başladıktan sonra dördüncü oturum limiti (6pm) onu kesti; kayıtçı hiç başlamadı. | Oturum limiti workflow ajanlarını anında öldürüyor; sıralı zincirde (kapici → denetci → kayitci) ikinci halka ölünce üçüncüsü başlamıyor. | Kayıt turu ana oturum tarafından **elle** yazıldı (kullanıcı talimatı); denetim **yapılmadı, "ölçülemedi" diye yazıldı** (DZ-01), ilk ölçülebilecek yer 6.7 açılışı. | Bir alt görevin denetim aşaması atlanmış gibi geçilmez; raporun DURUM'u 🟡 ve KARAR maddesi *"6.7 açılışında `denetci` 6.6'nın ÇIKTI'sını denetler"*. |
| #34 | 6.6 (YAZ) | **Oturum limiti fan-out'u DÖRT kez sıfırladı** (3am · 8am · 1pm sıfırlamaları yazarları, 6pm denetçiyi): beş yazar aynı anda ölüyor, StructuredOutput teslim edilemiyor, ~1,33 M token kesilen koşularda. | Workflow ajanları ana oturumun kullanım havuzunu paylaşıyor; beş paralel Opus ajan havuzu birlikte tüketiyor. Ajan ölünce dosyalar diskte kalıyor ama ÇIKTI'sı yok. | *"Mevcut durum"* promptuyla devam koşuları (dosya durumunu ana oturum ölçüp yazdı); C çiftinin ÇIKTI'sı yazar tarafından hiç teslim edilemedi → ana oturum ölçtü (`grep` dışa aktarımlar, `vitest --reporter=verbose` sayılar). | Fan-out'un büyüklüğü havuzla sınırlı; beş Opus yazar ≈ bir sıfırlama penceresi. Sonraki koşularda ya daha az paralel yazar ya kısa görevler; devam promptu **her zaman** diskteki durumu ölçülmüş hâliyle taşır. |
| #33 | 6.6 (YAZ) | **API güvenlik katmanı `[reasoning_extraction]` beş yazarı üç kez başlamadan düşürdü** (0 token, ~4,7 s). | İki bisect workflow ile ayrıştırıldı (5 ajan, ~157 k token): tetikleyici = `gelistirici` sistem promptundaki *"denetci ve kapici yalnızca ÇIKTI'yı görür — GEREKÇE'yi görmez"* ifadesi **+** StructuredOutput şemasında *"neden böyle"* isteyen alan (`gerekce` / `tasarimNotlari`). Aynı ajan şemasız → TAMAM; aynı ajan `{ok, notlar:"kısa not"}` → TAMAM; `olcumcu`/`planci` `rationale` alanlı şemalarla → TAMAM. | Gerekçe alanı şemadan çıkarıldı (tasarım notları dosya başlığında — depo geleneği zaten buydu), ajan tanımında *GEREKÇE → TASARIM NOTLARI* ve *"görmez"* ifadesi *"denetim ÇIKTI üzerinden yapılır"* oldu. | **Yapılandırılmış çıktıdan "neden" isteme + "denetleyen gerekçeyi görmez" sistem promptu = bayrak.** DZ-21'in ifadesi anayasada duruyor (olcumcu/planci onunla çalıştı); ajan tanımı ve şema düzeyinde kaçınılır. |
| #32 | 6.6 (YAZ) | **`morale-icon.test.tsx` 3 kırmızı:** *"expected [] to include 'text-[var(--danger)]'"*. | SVG elemanında `className` bir `SVGAnimatedString`, dize değil — test `.className.split(' ')` ile boş dizi okuyordu; bileşen doğruydu. | `classOf(el) = el.getAttribute('class')` yardımcısı; 22/22. | Sözleşmeye/devam promptuna yazıldı: *"SVG'de `getAttribute('class')`"*. D6'nın vakası — kırmızı test, kod değil. |
| #31 | 6.6 (YAZ) | **`position-map.test.tsx` 1 kırmızı:** *expected 'levelNatural' to be 'levelnatural'*. | Test yardımcısı `capitalizedCode` mevki kodları (`GK` → `Gk`) için yazılmıştı; küçük harfli seviye adlarına (`natural`) uygulanınca baş harfi büyütmüyordu. Bileşen doğruydu. | Yardımcı seviye adları için ayrı (`capitalize`), 30/30. | Aynı sınıf (D6). Bir test yardımcısı iki farklı girdi şekline uygulanırken **ikinci şekil için ayrıca sınanır** (F5'in test tarafı). |
| #30 | 6.6 (iskele) | **`i18n-keys.test.ts` ② üç vakada kırıldı** (YÖN ① sekiz "kayıtsız", YÖN ② sekiz "hayalet", referans eşitliği `undefined`) — ilk çok kelimeli bileşen dosyası gelince. | 6.5'in `moduleName`i dosya adını olduğu gibi grup adı sayıyordu; beş modülün beşi tek kelimelikti. Anahtar segmenti tire taşıyamaz (`/^[a-z][a-zA-Z0-9]*$/`), dosya adı kebab-case (§1.3) — köprü yoktu. | `groupNameOf` (kebab → camelCase) tek yerde; kapsam iddiasına `attributeBadge` (anahtarlı) + `dateChip` (anahtarsız) adıyla; planci'nin bulgusuyla *"modül başına en fazla bir `*_KEYS`"* iddiası **hata oluşabilecek hâldeyken** eklendi (DZ-12). | Kırılması **beklenen** kapıydı ve kırıldı — kanarya gerçek depoda öttü. `inventory-guards` ④ da iskelede kırmızıydı (*"ui tarafında sabit yok"*), yazar sonrası yeşil; negatif kanıt c1 (`ST` düşürüldü) → ④ + `tsc` + `build`. |
| #29 | 6.6 (bulgu) | **Etiketsiz Tailwind keyfi değerleri YANLIŞ ÖZELLİĞE derleniyor** — yazar ajanlar ölçtü (`@tailwindcss/node` 4.3.3 `compile`), derlenmiş CSS'te doğrulandı: `font-[var(--font-ui)]` → `font-weight:`, `text-[var(--text-sm)]` → `color:`. | Tailwind 4 `var()` taşıyan değeri belirsiz sayıp ilk uyan yardımcıya düşürüyor; tip ipucu (`family-name:` · `length:`) gerekiyor. 6.4/6.5 bunu görmedi çünkü D5'in web paketi ölçümü yalnızca **bayt** saydı, CSS'in ne ürettiğine bakmadı. | 6.6'nın on dosyası etiketli yazıldı; mevcut 20 dosya **dokunulmadı** (K12) → **SORUN-002**, sahibi 6.7 (ilk iş). | **D5'in yeni biçimi:** *"paket büyüdü"* bir ölçümdür ama *"doğru CSS üretildi"* değildir; bir kez ölçülen şey kapı olur (SORUN-002'nin çözümüne nöbetçi dâhil). |
| #28 | 6.6-ön (kurtarma) | **Devir notu *"§③ şu an KIRMIZI olmalı"* dedi; ölçüm YEŞİL verdi** — CHECKPOINT bayattı (`asama: 3-yaz`, `yarim_kalan` eski, `son_commit_baslik` var olmayan bir commit). | Nöbetçi yalnızca **kaynağı olan** dört alana bakar (faz · alt_gorev/durum · taban_commit · dal); `son_commit_baslik`i **temiz ağaçta** karşılaştırır (dosya kendi commit'ini taşıyamaz); anlatı alanlarının bakılacak kaynağı **yok**. Devir notu nöbetçiden, nöbetçinin **bastığı kapsamdan** fazlasını bekledi. | Bulgu yazıldı, nöbetçi **değiştirilmedi**: anlatı alanlarına kapı yazmak uydurma bir kaynak gerektirirdi. Kapsam satırı zaten basılıyor (*"kapsam: faz=… durum=… taban=…"*). | **Bir nöbetçiden beklenen şey, nöbetçinin bastığı kapsamdan okunur** — devir notundan değil (D7: devir notu kaynak değildir). "Bayat" ile "tutarsız" farklı şeyler; nöbetçi ikincisini yakalar. |
| #27 | 6.6-ön | **`format:check` KIRMIZI:** `scripts/inventory-guards.test.mjs` Prettier biçiminde değildi. | Test `Edit` ile yazıldı ve `pnpm format` koşturulmadan zincire girildi. | `prettier --write` + `format:check`/`lint` yeniden (0). Kapının **neye baktığı** `--file-info` ile ölçüldü: bu commit'te `settings.json` (json) · `ci.yml` (yaml) · `inventory-guards.test.mjs` (babel) **bakıldı**; `.gitignore` (parser yok) ve tüm `.md` (`.prettierignore`) **bakılmadı**. | `Edit`/`Write` ile yazılan her `.mjs/.ts` zincirden önce `prettier --write`. Kapı **baktı** — ve baktığı dosya listesi rapora girdi (DZ-03). |
| #26 | 6.6-ön | **Kırılması beklenen kapı KIRILMADI:** `.claude/agents/` + `.claude/skills/` eklendi, `inventory-guards` §② yeşil kaldı. | Nöbetçi yalnızca `apps/` · `packages/` · `tools/` · `scripts/` · `docs/` tarıyordu; kök altındaki `.claude/` **hiçbir yüzeyde değildi**. | §②'ye `.claude/` yüzeyi eklendi (`agents`/`skills` **adıyla** beklenir); ağaç güncellendi; M3 (`agents/` satırı silindi) → **1 test** kırıldı. `docs/CHECKPOINT.md` için §② **gerçekten kırıldı** ve belge haritasıyla kapandı (M4 yeniden ısırdı). | *"Kırılmazsa sebebini ara"* — D3'ün bu biçimi: kapı doğru çalışıyor ama **sorulmayan** bir dizin var. Yeni bir **kök** dizin açan her alt görev §②'nin dizin listesine bakar. |
| #25 | 6.6-ön | **Ajan ve skill dosyalarının YÜKLENDİĞİ yaratıldıkları oturumda ölçülemedi.** `Agent(subagent_type:'olcumcu')` → *"not found"* · `Skill('faz-yurut')` → *"Unknown skill"* · alt süreçte `claude -p` → *"organization has disabled subscription access"*. | Harness `.claude/agents/` ve `.claude/skills/`i **oturum başında** okuyor; dosyayı yaratan oturum onu yükleyemez. Üçüncü yol kuruluş politikasına takıldı. | **"Yüklendi" yazılmadı.** YAML şekli doğrulandı (7/7 — bir *yüklendi* kanıtı **değil**) ve ölçüm ilk ölçülebilecek yere adıyla devredildi. **Kurtarma oturumu ölçtü:** altı ajan sistem listesinde (açıklamalar frontmatter'dan birebir), `olcumcu` haiku ile çağrıldı ve **döndü** (gövde başlığı + `Bash, Glob, Grep, Read` + gövde iddiası; 0 araç, 5,0 s), `Skill('faz-yurut')` dosyanın tamamını yükledi. | **DZ-10'un "bir sonraki başlangıç" biçimi:** etkisi ancak yeniden başlatmada görülen bir ayar (ajan, skill, hook) için ölçüm o başlangıca **adıyla** devredilir; o gün "dosya var" bir kanıt değildir. Sessizce *"yüklendi"* yazılsaydı 6.4-ön D listesinin altıncı vakası olurdu. |
| #24 | 6.5 | **`DEPENDENCY-WATCH`e ÜÇ SÜRÜMÜ YANLIŞ yazdım** (`react-toast` 1.2.16 · `react-avatar` 1.1.12 · `react-progress` 1.1.9). | Kurulum çıktısındaki bir sürümü **hatırlayıp** yazmak; pnpm hepsini basmıyor ve zihin en son görüleni genelliyor. | `package.json`ın `dependencies` bloğu **okundu** ve üçü düzeltildi (**1.2.23 · 1.2.6 · 1.1.16**). | *"Bir sayıyı devir notundan alma, kaynaktan SAY"* — ve **kendi birkaç dakika önceki gözlemin de bir devir notudur**. Sürüm yazan her satır `package.json`dan okunur. |
| #23 | 6.5 | **`Progress`in gösterge formülü TERSTİ** — `translateX(-percent)` yazıldı, yani **dolu** bir çubuk tamamen gizleniyordu. | Kaydırma miktarı ilerlemenin **tümleyeni** (`100 - percent`); ilk yazımda ikisi karıştırıldı. Belirti render'da görünmüyordu çünkü test yoktu. | Formül saf bir fonksiyona ayrıldı (`indicatorOffsetPercent`) ve **beş vakayla** sabitlendi (uçlar · farklı `max` · aralık dışı · belirsiz · `max<=0`). | **Bir hesabı JSX'in içinde bırakmak onu test edilemez yapar.** Saf yarıyı ayırmak, hatayı bir birim testine açıyor — ve o test yazılırken hata **kendiliğinden** görünüyor. |
| #22 | 6.5 | **`process.stdout.write` ile "kapsamı bas" idiomu `packages/ui`de LİNT'İ KIRDI** (üç dosya, dokuz hata). | İdiom `scripts/inventory-guards.test.mjs`ten kopyalandı — ama `scripts/` ve `tools/` **K8'den muaf**, `packages/ui` **değil**; ayrıca `types: []` yüzünden `process` tipi de yok. | Basma **kaldırıldı**, yerine **iddia** kondu: beklenen modüller **adıyla** aranıyor, denetlenen çift **sayısı** iddia ediliyor. K8 ve `types: []` **açılmadı**. | ⚠️ **BİR İDİOM TAŞINABİLİR DEĞİLDİR.** *"0 bulundu ile bakılmadı ayrılsın"* bir **amaç**; onu sağlayan **araç** pakete göre değişir — kapıda `stdout`, testte `expect`. Emsali kopyalarken **muafiyet listesi de kopyalanmıyor**. |
| #21 | 6.5 | **İlk mutasyon serisinde İKİSİ hiçbir şeyi kırmadı** (portal söküldü → 0 · `aria-valuenow` bozuldu → 0). | ⚠️ İkisi de **üç ihtimalden ikincisi**: mutasyon ölçtüğüm yola dokunmuyordu. Portal için testler modalın **nerede** monte edildiğini hiç sınamıyordu (`screen` bütün `body`ye bakıyor); `aria` için ise mutasyon yalnızca yerel bir değişkeni değiştiriyordu, Radix'e giden `rest.value`yu değil — üstelik belirsiz vakada iki yol **aynı** sonucu veriyordu. | Dialog testine **portal iddiası** eklendi (`container.contains(dialog) === false`); `aria` mutasyonu **geçersiz** ilan edilip yerine `Skeleton`ın `aria-hidden`ı seçildi. İkisi de tekrar koştu → **1'er test** kırıldı. | **Bir mutasyon seçimi de bir ölçüm tasarımıdır:** ölçmek istediğin yola dokunmayan bir mutasyon, kodun sağlam olduğunu değil **deneyin yanlış kurulduğunu** gösterir. |
| #20 | 6.5 | **`UI_KEYS` nöbetçisi PAKETİN BÜTÜN TESTLERİNİ İKİ KEZ KOŞTURDU.** Belirti sessizdi: kırmızı yoktu, yalnızca sayı **11 → 67** çıktı. | `import.meta.glob('./*.tsx', { eager: true })` **`*.test.tsx` dosyalarını da** gerçekten değerlendirdi; onların `describe`/`it` blokları **nöbetçinin dosyasına** kaydoldu. | Desen `['./*.tsx', '!./*.test.tsx']` oldu (negatif desen **dizi** ister — `.d.ts` bildirimi de genişletildi) ve **taramanın sınırı ayrıca iddia edildi**. | ⚠️ **Nöbetçinin kendisi ölçüm aracını bozabilir (D2).** Yanlışı gösteren şey **kapsam satırı** oldu (*"18 modül"* = 9 bileşen + 9 test) — bir kapının kaç şey taradığını söylemesi, işte tam bunun için. |
| #19 | 6.4 | **`test:db` "EXIT=0" dedi ama HİÇ SAYI BASMADI** — bir an için *"kapı bakacak bir şey bulamadı"* sanıldı (Docker mı düştü?). | **Ölçüm aracının kendi hatası:** kapı zinciri arka planda **iki kez** başlatıldı ve ikisi de **aynı log dosyasına** yazdı; ikinci koşu birincinin çıktısını kırptı. Kapı sorunsuzdu. | Zincir **borusuz ve tek başına** yeniden koşturuldu → `Test Files 10 · Tests 301`, exit 0. | ⚠️ **D2'nin üçüncü vakası ve bu kez suçlu benim betiğim.** Bir kapının *"sessiz"* görünmesi kapının değil **kaydın** hatası olabilir; çıkış kodu ile çıktı **ayrı ayrı** doğrulanır, ve paylaşılan bir log dosyasına eşzamanlı yazan iki koşu bir ölçüm değil bir yarıştır. |
| #18 | 6.4 | **`Edit` aracı bir YORUM satırını kaçış dizisine çevirdi.** `combobox.tsx`te bir JSDoc satırı diske `/** Yukarıdaki gerekçenin ...` diye yazıldı — yani kaynakta **düz metin olarak `ı`** duruyordu, Türkçe karakter değil. | Dosya bir `'̇'` kaçış dizisi kazandıktan sonra düzenleme aracı **bütün** ASCII olmayan baytları kaçışa çevirdi. Ne `lint` ne `format` bunu yakalar (sözdizimi geçerli, yalnızca bir yorum). | Satır bir Node betiğiyle (`writeFileSync`, UTF-8) düzeltildi ve kaçış yalnızca **kod noktası sabitinde** bırakıldı. | ⚠️ **Ölçüm aracının kendisi kaynağı bozabilir (D2).** Bir dosyaya kaçış dizisi girdikten sonra **aynı dosyadaki diğer düzenlemeler gözden geçirilir**; belirti sessizdir çünkü hiçbir kapı yorumun okunabilirliğine bakmıyor. |
| #17 | 6.4 | **`pnpm typecheck` TS6059 ile kırıldı:** *"`vitest.setup.ts` is not under rootDir `src`"*. | `packages/ui/tsconfig.json` `rootDir: "src"` taşıyordu ve kurulum dosyası `src/` **dışında** (ürün kodu değil, kapsam paydasına girmemeli). `apps/web`de bu sorun yok çünkü o paket **emit etmiyor**. | `rootDir`/`outDir` **emit eden** yapılandırmaya (`tsconfig.build.json`) taşındı; `tsconfig.json` yalnızca tip denetliyor. | **Emsal Faz 3.2a'da zaten kayıtlıydı** (`packages/db`): *"`allowDefaultProject`e satır eklemek asıl sebebi GİZLER"* ve gerçek çözüm `rootDir`i taşımaktı. Kural: **tip denetimi geniş, emit dar.** |
| #16 | 6.4 | **Mutasyon: Switch topuzunun konum sınıfları silindi → 211 testin HİÇBİRİ kırılmadı.** Oysa bileşenin yorumu *"durum konumla da taşınıyor"* diye bir **iddia** yazıyordu. | Test yalnızca `data-state` özniteliğini iddia ediyordu; **konum kanalı** (`translate-x`) hiç sınanmıyordu. Yani iddia yazılıydı, denetlenmiyordu. | Test **güçlendirildi**: iki kanal ayrı ayrı iddia ediliyor. Mutasyon tekrarlandı → **1 test kırıldı**. | *"Bir mutasyonun hiçbir şeyi kırmaması üç şey demek olabilir"* — burada **birincisi**: nöbetçi yoktu. Bir yorumda yazan her davranış iddiası, onu **kıran** bir teste bağlanır. |
| #15 | 6.4 | **RadioGroup ok tuşu testi kırıldı: odak taşındı ama SEÇİM takip etmedi.** | **KOD DEĞİL ORTAM** (D6). Radix `document` keydown'da bir bayrak kuruyor, öğe odağı alınca `click()` ediyor, **keyup bayrağı sıfırlıyor** (`react-radio-group/dist/index.mjs`:338-373 okundu). `user-event` keyup'ı **gecikmesiz** gönderiyor ve ertelenmiş odak ondan sonra geliyor. | Üç ölçümle ayrıştırıldı (tam basım → seçim yok · **basılı tutma → seçim var** · ok+Space → seçim var). Test *"tuş basılıyken"* biçimine çevrildi, sınır ve sahibi (**Faz 17**) adıyla yazıldı. | **Kırmızı test ≠ kod yanlış.** Sahte bir gecikme eklemek testi bir zamanlama varsayımına bağlardı; doğru cevap sınırı **yazmak** ve gerçek tarayıcıya devretmekti. |
| #14 | 6.4 | **`filterOptions(…, 'iSPA')` BOŞ döndü** — ve dosyanın **kendi yorumu** bunun çalıştığını iddia ediyordu. | **KOD YANLIŞTI.** `'İspanya'.toLowerCase()` tek bir `i` değil, `i` **+ U+0307 BİRLEŞEN NOKTA** üretiyor (kod noktası kod noktası ölçüldü); `.includes('ispa')` false diyor ve fark **ekranda görünmüyor**. | `foldForSearch()` eklendi: küçült, sonra U+0307'yi ele (**kaçış diziyle** yazılı). `ü` (U+00FC) önceden birleşik olduğu için dokunulmuyor — aksana duyarlılık **korundu**; aksana duyarsız arama **Faz 32'ye** bırakıldı (SAPMA-026). | *"Türkçe küçük/büyük harf dönüşümü GÜVENLİ DEĞİLDİR"* uyarısının üçüncü vakası — ve bu kez tuzağa düşen, uyarıyı **yorumda anlatan** kodun kendisiydi. **Bir gerekçe yazmak onu doğrulamaz.** |
| #13 | 6.4-ön | **KAPI↔CI EŞLEŞMESİNİ ÖLÇEN KENDİ BETİĞİM YANLIŞ POZİTİF VERDİ.** `format` ve `test` betikleri *"CI'da var"* çıktı; oysa CI'da `pnpm format` (yazıcı kip) ve düz `pnpm test` **yok** — `format:check` ve `test:coverage` var. Bir an için *"her betik bağlı"* sonucu çıkarılacaktı. | Desen `pnpm format\b` idi ve `\b` **iki kelime karakteri arasındaki sınır**; `format:check`te `t` ile `:` arasında bir sınır **var**, yani `pnpm format` dizesi `pnpm format:check` içinde eşleşiyor. Aynısı `test` → `test:coverage` ve `build` için. | Ölçüm `grep -o -E` + `sort \| uniq -c` ile **tam eşleşme** olarak tekrarlandı; nöbetçi testinde desen satır sonuna sabitlendi (`^\s+run: pnpm <ad>\s*$`). | ⚠️ **D2'nin bu oturumdaki vakası:** ölçüm aracının kendisi yanlış cevap üretti ve cevap **makul göründüğü için** yakalanması zordu. Bir eşleşme iddiası, desenin **ne eşleşmediğini** de göstermeden yazılmaz. |
| #12 | 6.4-ön | **`prettier --check <yok sayılan dosya>` "All matched files use Prettier code style!" DEDİ** — ve bu cümle iki farklı şeye birden uyuyor: *"dosyaya bakıldı, temiz"* ve *"hiçbir dosya eşleşmedi"*. `.prettierignore` girdilerinin gerçekten ısırdığı bu çıktıyla **kanıtlanamazdı**. | Prettier'ın başarı cümlesi **kapsamı taşımıyor**: eşleşen dosya sayısı basılmıyor. Yok sayılan bir dosya için de temiz bir dosya için de aynı metin çıkıyor. | `npx prettier --file-info <yol>` kullanıldı — `{"ignored": true/false}` döndürüyor. Beş girdi tek tek ölçüldü: `*.md` ✅ · `tokens.generated.css` ✅ · `LICENSE` ✅ · `pnpm-lock.yaml` ✅ · `packages/db/drizzle/**` ✅ (⚠️ **dizin yolu `ignored:false` diyor**, dosya yolu `true` — sorunun kendisi yol tipine duyarlı). | **SAPMA-024'ün araç tarafındaki eşi.** *"0 bulundu"* ile *"hiçbir şeye bakmadı"* ayrımı yalnızca bizim kapılarımızın değil **kullandığımız üçüncü taraf araçların** da sorusudur; bir aracın başarı cümlesi kapsamı taşımıyorsa kapsam **ayrı bir komutla** iddia edilir. |
| #11 | 6.3b | **`grep -c` küçültülmüş CSS'te `@font-face` sayısını "1" gösterdi** (gerçek **13**) ve `latin-ext` aralığını **0** dedi. Bir an için *"fontlar üretime ulaşmıyor"* sanıldı. | İki ayrı ölçüm hatası üst üste: ① `grep -c` **satır** sayar, eşleşme değil — küçültülmüş CSS **tek satır** ② minifier `U+0100-02BA`yı `U+100-2BA` yazıyor, desen eşleşmedi. | `grep -o … \| wc -l` ile yeniden sayıldı ve minifier biçimi kullanıldı: **13 `@font-face`**, `latin-ext` **2 kez**, `U+131` **2 kez**, iki `latin-ext` woff2 dosyası da `dist`te. | *"Bir araç cevap verebilir ama EKSİK verebilir."* Sayım aracının **neyi** saydığı (satır mı, eşleşme mi) ve verinin **hangi biçimde** olduğu (küçültülmüş mü) ayrıca doğrulanır. |
| #10 | 6.3b | **Web imajı çalıştırılınca varlık yolu `/C:/Program Files/Git/fms/assets/…` çıktı** ve konteyner `path matcher must begin with '/'` diyerek düştü. | Git Bash'in MSYS yol dönüşümü: `-e PUBLIC_BASE_PATH=/fms` ve `--build-arg PUBLIC_BASE_PATH=/fms` argümanlarındaki `/fms`i **Windows yoluna** çevirdi. Kod ve imaj doğruydu; **ölçüm ortamı** yanlış girdi verdi. | `MSYS_NO_PATHCONV=1` ile hem derleme hem koşu tekrarlandı; JS **200**, CSS **200**, kök **404**. | ⚠️ Bu ortama özgü ve **CI'da yok** (Linux runner). Windows'ta bir konteynere `/` ile başlayan bir değer geçirilecekse `MSYS_NO_PATHCONV=1` gerekir — yoksa hata **kodda aranır**. |
| #9 | 6.3b | **Alias yazıldı, mutasyon yine hiçbir şeyi kırmadı.** `resolve.alias` `defineConfig`in kökündeydi ve `pnpm test` bayat `dist`i okumaya devam etti. | Vitest 4'ün `projects` girdileri **ayrı Vite yapılandırmaları** ve kök `resolve`u **devralmıyorlar**. Yazılmış ama hiçbir şey yapmayan bir ayar. | Alias sekiz TypeScript projesine **tek tek** verildi; mutasyon sonra **4 test** kırdı. | ⚠️ **`pnpm-workspace.yaml`ın `ignoredBuiltDependencies` tuzağının birebir aynısı** — bu depoda ikinci kez. Bir yapılandırma yazıldıktan sonra **etkisi ölçülür**; "ayar dosyada var" onun okunduğunu göstermez. |
| #8 | 6.3b | **CI KIRMIZI** (`33937132528`): `İmaj (amd64)` ve `İmaj (arm64)` işleri *"Rolldown failed to resolve import `@fms/ui`"* ile düştü. Yerelde `pnpm build` **geçiyordu**. | `apps/web/Dockerfile` derlenecek paketleri **elle sayıyordu** (`@fms/shared` + `@fms/web`) ve `@fms/ui`yi hiç derlemiyordu; `exports` var olmayan bir `dist/index.js`i gösteriyordu. Yerelde turbo `dependsOn: ["^build"]` ile sırayı kendisi kuruyor, imaj o grafiği taşımıyordu. | `pnpm --filter @fms/web... build` — `...` soneki bağımlılıkları **bildiriyor**, saymıyor. Aynı tuzağı taşıyan `apps/api/Dockerfile` da düzeltildi. | ⚠️ **D5'in kapsamı ölçüldü:** 6.3 D5'i `packages/ui/dist` için yaptı, **imaj yüzeyi için yapmadı**. *"Build et ve çalıştır"* — hangi **artefaktı** çalıştırdığın da bir iddiadır; bu depoda üç ayrı üretim yüzeyi var (paket `dist`i, web paketi, imaj). |
| #7 | 6.3 | **Mutasyon ④ HİÇBİR ŞEYİ KIRMADI — ve üç ihtimalden ikincisi çıktı.** `turkish-glyphs.ts`te bir kod noktası bozuldu, `pnpm test` **138/138 geçti**. | Mutasyon **ölçtüğüm yola dokunmuyordu**: `fonts.test.ts` `@fms/ui`yi `dist`ten çözüyor, ben **kaynağı** değiştirmiştim. *"Bir mutasyonun hiçbir şeyi kırmaması üç şey demek olabilir: nöbetçi yok · mutasyon ölçtüğün yola dokunmuyor · ya da kod gereksiz."* Burada **ikincisi**. | `pnpm build --filter @fms/ui` koşturuldu, mutasyonun **dist'e ulaştığı** `grep` ile doğrulandı, ve aynı mutasyon **4 testi kırdı**. | ⚠️ Asıl bulgu mutasyon değil, **onun ortaya çıkardığı yapı**: `pnpm test` hiç build etmiyor ve paketler arası testler bayat `dist` ölçebiliyor → **BORÇ-011**. *"Bir mutasyonun yerine oturduğu ayrıca doğrulanır"* kuralı burada **dosyada değil, TEST EDİLEN ARTEFAKTTA** doğrulanmalıymış. |
| #6 | 6.3 | **Prettier üretilmiş CSS'i yeniden yazdı ve tazelik nöbetçisi kırıldı.** `pnpm format` sonrası `css-projection.test.ts` *"diskteki dosya yeniden üretilenle aynı"* iddiasında düştü. | İki araç aynı dosyanın biçimine karar veriyordu: `generate-theme-css.mjs` üretiyor, Prettier yeniden biçimlendiriyor. Üretilmiş bir artefakt için bu sonsuz bir sürtünme (üret → biçimlendir → tekrar üret). | Dosya `.prettierignore`a eklendi, gerekçesi `packages/db/drizzle/` emsaliyle birlikte yazıldı. | ⚠️ **Bu bir hata değil, nöbetçinin İLK GERÇEK YAKALAMASI.** Kırılma, tazelik testinin çalıştığının kanıtı — bir üretilmiş artefakt eklenirken `.prettierignore` aynı commit'te düşünülür. |
| #5 | 6.3 | **`?raw` içe aktarımı BOŞ DİZE döndürdü ve test sessizce yanlış şeyi ölçtü.** Tazelik nöbetçisi *"üretilmiş dosya boş değil"* vakasında düştü; dosya diskte **3147 bayt**tı. | Vitest'in varsayılanı `css: false` ve o kip CSS kimliklerini **`?raw` dahil** kesiyor. Hata **vermiyor**, `''` dönüyor — yani `typeof` `string`, `length` **0**. Bir sonda ile ölçüldü. | `vitest.config.ts`in `ui` projesine `css: true` eklendi, gerekçesi orada yazılı. `node:fs`e dönmek **reddedildi**: `packages/ui`nin `types: []` kararı K1'in ilk savunma hattı ve bir test rahatlığı için açılmaz. | ⚠️ **D2 + falsy tuzağının birleşimi:** ölçüm aracı cevap verdi, **eksik** verdi, ve eksikliği bir **falsy değer** olarak sundu. *"Falsy bir değer «özellik yok» anlamına da gelebilir."* Bir içe aktarımın gerçekten içerik getirdiği, ilk kullanımda **ölçülür**. |
| #4 | 6.2 | **Kendi yazdığım bir test KÖRDÜ ve yeşil geçiyordu.** *"EN AZ gereken kadar açıklaştırıyor"* başlıklı vaka `ensureContrast`i **iki kez** çağırıp yalnızca `lightenPercent > 0` iddia ediyordu — yani **%100 açıklaştıran** bir uygulamayla da geçerdi. | Minimallik bir **karşılaştırma** iddiası ve karşılaştıracak ikinci nokta (bir adım öncesi) hesaplanmıyordu. Testin **başlığı** iddiayı doğru anlatıyordu, gövdesi anlatmıyordu — *"ölçümün TARİFİ de bir iddiadır"*, ve tarif ile gövde ayrışmıştı. | `blendTowardWhite` dışa aktarıldı ve vaka gerçek kontrolü yapıyor: `contrastRatio(blendTowardWhite(renk, p-1), zemin) < hedef`. Gerekçesi fonksiyonun başlığına yazıldı. | *"Pozitif testler kör bir kontrolle de geçer"* — ve bu kez körlük **testin kendisindeydi**. Bir testin başlığı bir iddia yazıyorsa, gövdesinin o iddiayı **kırabilecek** bir vakası olmalı. |
| #3 | 6.2 | **`pnpm lint` yeni kodda 7 hata verdi** (`no-unnecessary-condition` · `restrict-template-expressions` ×2 · `non-nullable-type-assertion-style` ×2 · import/export sıralama ×2). | `packages/ui` bu depoda **ilk kez** tip-farkında `strictTypeChecked` altında gerçek kod aldı. `as const` bir demette `ATTRIBUTE_BANDS[0]?.min` gereksiz, literal tipli sabitler şablon dizesinde `String()` istiyor, ve `noUncheckedIndexedAccess` altında `candidates[0] as string` bir tip **iddiası**. | Hiçbir kural **susturulmadı**: `?.` kaldırıldı · `String()` eklendi · `pickAccessibleForeground` yeniden yazıldı (`const [first, ...rest]` + `undefined` kontrolü, tip iddiası **hiç yok**) · sıralama `lint:fix` ile. | Kural gevşetmek yerine kodu düzeltmek, `packages/ui`nin **6.4'ten itibaren yüzlerce dosya** alacağı düşünülünce daha ucuz: bir muafiyet bugün bir satır, altı ay sonra bir kapsam. |
| #2 | 6.1 | **`LINT_SONUCU=$?` yine `0` dedi — ve bu sefer mutasyon YERİNDEYKEN.** Kural `eslint.config.js`ten çıkarılmışken çıkış kodu okundu ve `0` göründü; iddia doğruydu ama **kanıt yanlış yerden** geliyordu. | Boru hattında `$?` **son komutun** kodunu verir: `pnpm lint 2>&1 \| tail -3` → okunan şey `tail`in kodu. **6.0'da tam bu tuzağa düşülmüş ve kaydedilmişti — ikinci kez tekrarladı.** | `pnpm lint > /dev/null 2>&1; echo $?` ile yeniden ölçüldü: **gerçekten 0**. İddia değişmedi, **kanıtı** değişti. Ayrıca `eslint --print-config` ile iki yönlü doğrulandı (`no-hardcoded-path: undefined` · `no-bare-jsx-text: [2]`). | ⚠️ **Disiplin bu tuzağı iki kez durduramadı.** Çıkış kodu iddia edilecekse komut **borusuz** koşar; boru zorunluysa `${PIPESTATUS[0]}`. Kaydedildi ki üçüncü kez *"yeni bir sorun"* sanılmasın. |
| #1 | 6.1 | **Mutasyon ① beklenenden FAZLASINI kırdı:** kural `index.js`ten sökülünce `no-bare-jsx-text`in kanaryası da (3 vaka) kırıldı. | `eslint.config.js` `local/no-hardcoded-path`e atıfta bulunuyor; kural eklentide yoksa ESLint **config'in tamamını** reddediyor. Yani ① *"kural kayıtlı mı"*yı değil *"config yükleniyor mu"*yu ölçüyor — kaba bir mekanizma. | Mutasyon **silinmedi**, kaba olduğu **yazıldı**; borcun asıl senaryosu için ayrı bir mutasyon eklendi (②: kural satırı `eslint.config.js`ten silinir). O mutasyon **yalnızca** yeni kanaryayı kırıyor. | *"Bir mutasyonun bir şeyi kırması, ÖLÇMEK İSTEDİĞİN şeyi kırdığı anlamına gelmez."* Kırılanların **hangileri** olduğu sayılır, kaç tane olduğu değil. |
