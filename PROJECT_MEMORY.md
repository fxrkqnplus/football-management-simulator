...
| | |
|---|---|
| **Aktif faz / alt görev** | ▶️ **FAZ 5 — i18n. 5a/5b bölünme çizgisi; ⚠️ KONTROL NOKTASI 5.5 SONUNDA — süre ÖLÇÜLÜR (§0.5), 2 günü aşarsa 5b ayrı dal.** Sıradaki: **5.5 — ESLint `local/no-hardcoded-turkish` + kanarya** → kriter 1. 🆕 **Envanter bugün 0**, yani kural `error` açıldığında `pnpm lint` **yeşil kalmalı**; kalmıyorsa fark kuralın aracı aşan kapsamıdır ve yazılır |
| **Son tamamlanan** | ✅ **5.4 — BORÇ-003 + BORÇ-005 ödendi. KRİTER 7 KAPANDI** (iki yarısı da). Commit: `5086b74`. **Envanter 33 → 0**; araç AST tabanlı ve **kendi karşı kontrolü** var (11 test, iki yönlü). 🆕 **ROADMAP'in saydığından büyük çıktı** (`App.tsx` 6 → **19**, `main.tsx` listede **yoktu**) çünkü **kural dile bakamaz**. 🆕 **Aracın kör noktası bulundu:** modül düzeyi metin sabitleri — yakalayan şey BORÇ-003'ün eski cümlesi oldu. 🆕 **i18n artık ÖNYÜKLEME ÖN KOŞULU** (fırlatmıyor, sessizce `undefined` — ölçüldü). **D5 10/10 TAM · mutasyon 3/3** |
...
| **CI** | ✅ Dalda **5 koşu, 5 yeşil** (liste sorgusu); son: `33703479808` (`f43a22a`). ⏳ 5.4'ün koşusu yazım anında bilinmiyor. ✅ Merge sonrası `develop`: `33674720478` (`2b5075e`) success |
| **Kapılar** | ✅ **typecheck 10/10 SOĞUK** · **build 8/8 SOĞUK** (cache ikisi için ayrı ayrı) · lint 0 · format 0 · arch **9 kural** · **test 1041/72** (1029/70 idi) · **test:db 301/10** · **gaps 20/3/17/0 ✗** · 🆕 **envanter 0 ihlal** · **mutasyon 3/3** · **görünmez karakter 0** · 🆕 **D5 10/10 TAM** — derlenmiş paket **çalıştırıldı**, ekranda **hiç ham anahtar yok**; 5.3'ün yarım D5 borcu kapandı |
| **Kapsam** | fonksiyon **%80,70 (364/451)** — satır %88,88 · ifade %88,99 · dal %89,74; eşik **70**. ⚠️ **Payda 5.4'te DEĞİŞMEDİ ve sebebi yapısal:** `bootI18n` **+1**, silinen `exceptionMessageFor` **−1**. Satır/dal kapsamı **yükseldi**. Marj: `364 / 0,70 = 520` → payda en fazla **520**, bugün **451**, **boşluk 69**. Eşik düşürülmedi, dosya dışlanmadı, import testi yazılmadı |
...
| **Kütükler** | Açık sorun **0** · teknik borç **7 satır, ikisi 5.4'te KAPANDI** → **açık 5** (001·002·004 → Faz 16 · 007 → Faz 12 · 006 → Faz 50) · SAPMA **38** (Faz 5'in payı üç: 036·037·038; **5.4 yeni SAPMA açmadı**) · boşluk **20**, açık **17**; **G-13 → Faz 5 + Faz 17** ✓ |
