...
| | |
|---|---|
| **Aktif faz / alt görev** | ▶️ **FAZ 6 — Tasarım Sistemi ve Bileşen Kütüphanesi.** **6.0 ✅ · 6.1 ✅.** Sıradaki: **6.2 — tasarım token'ları** (`spec/05` §7.1–7.4, kontrast aritmetiği). Bölünme çizgisi **6a = 6.0–6.5 / 6b = 6.6–6.12**; §0.5 kontrol noktası **6.5 sonunda** |
| **Son tamamlanan** | ✅ **6.1 — BORÇ-010 ÖDENDİ.** 🆕 **Borcun VARLIĞI önce kanıtlandı:** kural `eslint.config.js`ten sessizce çıkarıldı → `pnpm lint` **exit 0**, `--print-config` → `no-hardcoded-path: undefined` / `no-bare-jsx-text: [2]` (iki yönlü), ve **yalnızca yeni kanarya** kırıldı. 6.1 öncesi bu mutasyon **hiçbir şeyi kırmazdı**. 🆕 **Mutasyon 6/6**, her birinin yerine oturduğu `grep -c` ile ayrıca doğrulandı, geri almalar **dosya yedeğinden + md5**. 🆕 **İki eksen ayrı nöbette** (birim test **+ yol tanım yeri**) — muafiyet kalkınca **ikisi birden** kırıldı. 🆕 **6.0'ın dört kararı kaydedildi; kalıcı evleri:** ROADMAP Faz 6 → 6.0 SONUÇ + **kriter daraltması tablosu** (beş yarı, üçü hedef faza **adıyla** eklendi) · `spec/09` §11.6 · **SAPMA-041 / SAPMA-042** |
| **Tarih / ilerleme / dallar** | 2026-09-04 · **5 / 50 faz (%10)** · `main` → `develop` (`2ec1548`) → **`feature/faz-06-design-system`** (taban `2ec1548`, **2 commit**: `3073deb` · `df2245a`) |
| **CI** | ✅ Dalda **1 push koşusu, success** — `33888291301` (`df2245a`, liste sorgusuyla okundu). ⚠️ **Faz 4'ün KIRMIZI koşusu (`33419337117`, İmaj/amd64, Docker Hub auth — kod dışı) HÂLÂ YENİDEN DENENMEDİ**, kullanıcının kararı. ⚠️ Kalite kapıları **iki mimaride** koşuyor (`ubuntu-24.04` + native `ubuntu-24.04-arm`, K14) — tarayıcı kararının bedeli buradan geliyordu |
| **Kapılar** | ✅ typecheck **10/10 SOĞUK** (`Cached: 0`) · build **8/8 SOĞUK** (`Cached: 0`, cache build'den **hemen önce** silindi — ayrı ayrı) · lint **0** (borusuz `$?`) · format **0** ⚠️ `.md`ye bakmıyor · arch **9 kural** · **test 1127/75** (+7, dosya sayısı değişmedi) · **test:db 301/10** · **gaps 20·3·17·0 ✗** · **i18n temiz, 309 dosya** |
| **Kapsam** | fonksiyon **%80,70 (364/451)** — **değişmedi**; satır 1192/1341 · ifade 1287/1446 · dal 560/624; eşik **70**. Marj `364/0,70 = 520` → boşluk **69**. ✅ Beklenti tuttu: `tools/eslint-local-rules/` `coverage.include` (`tools/*/src/**`) **dışında** — payda büyümedi |
...
| **Ortam** | PostgreSQL **18.6** · Node **24.19.0** · pnpm **11.23.0** · TS `~6.0.3` · Docker **4.89.0**/engine 29.7.2 · jsdom **30.0.1** (`matchMedia` yok · `var()` çözülmüyor · rect 0×0). **Üretim paketi 380.908 bayt** — soğuk build sonrası **yeniden** ölçüldü, devir notundan alınmadı; §11.6 tabanı bu |
| **Kütükler** | Açık sorun **0** · teknik borç **10 satır, 4 ÖDENDİ → açık 6** (**BORÇ-010 kapatıldı**; kalan: 001·002·004 → Faz 16 · 007 → Faz 12 · 006 → Faz 50 · **009 → 6.11**) · **SAPMA 42** (041 · 042 bu alt görevde açıldı) · boşluk **20**, açık **17** · günlük **2 satır** |
| **Bloke eden var mı?** | Hayır. ⚠️ Üç şey duruyor: ① `main.test.tsx` jsdom yarışı **6.4'ten itibaren** her alt görevde izlenecek ② Faz 4'ün kırmızı CI koşusu ③ `$?`'nin boru hattında yanlış okunması **iki kez** tekrarladı (günlük #2) — üçüncüsü olmasın diye kaydedildi |
