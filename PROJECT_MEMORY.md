...
| | |
|---|---|
| **Aktif faz / alt görev** | ▶️ **FAZ 5 — i18n. Yedi kriterin ALTISI kapandı** (1·2·3·4·5·7); kalan **tek** kriter 6. Sıradaki: **5.8 — G-13 kararı**: `competitions.name_key` / `rivalries.name_key` için çeviri kaynağının **NEREDE yaşadığı** ve Faz 17'nin üç seçeneğinden hangilerinin **mümkün** olduğu yazılır → kriter 6. ⚠️ **ŞEMA DEĞİŞMEZ** (master tablolar, migration Faz 12'nin zeminini etkiler) ve **mekanizma Faz 17'nin işi**. 🆕 5.3'ün fiyat notu önce okunur: çeviri artık **yalnızca istemci JSON'ında**, yani seçenek ②/③ aynı dizeyi veritabanında **kopyalamak** demek |
| **Son tamamlanan** | ✅ **5.7 — `docs/glossary.md`. KRİTER 5 KAPANDI.** Commit: `da6bd17`. **133 terim**, ve sayı **prose'da değil** dosyayı ayrıştıran bir testte (`tools/glossary-check/`). 🆕 **`77 + 57 = 134` BİR EKSİK ÇIKTI:** iki envanterin ayrık olduğu hiç ölçülmemişti, kesişim **1** (`injuryProneness`) → **SAPMA-040**. 🆕 **5.0'ın saymadığı üç envanter daha** var (havuz aslında **162**), yazılmadı (K12) ama sözlüğün §6'sı sahipleriyle sayıyor. 🆕 **Sözlük ↔ locale kararı verildi**, nöbetçi **Faz 18'e** yazıldı (bugün `squad.json` boş — bakacak şeyi olmayan kontrol onay değildir). **Mutasyon 5/5** |
| **Tarih / ilerleme / dallar** | 2026-09-03 · **4 / 50 faz (%8)**, Faz 5 **açık, 8/11 alt görev bitti**. `main` → `develop` → **`feature/faz-05-i18n`** (taban `2b5075e`). ✅ PR #5 merge edildi · ✅ `GH007` push blokajı kalktı. **§0.5 5.5'te koşturuldu: 0,859 gün < 2 → bölünme ATEŞLENMEDİ**; 5b ayrı dal **açılmıyor**, 5.8-5.9 aynı dalda ve aynı PR'da |
| **CI** | ✅ Dalda **8 koşu, 8 yeşil**; 5.6'nınki `33786419740` **success**. 🆕 **VE ADIM ADIM DOĞRULANDI (5.6'nın açık ucu kapandı):** `Çeviri kaynağı denetimi` ve `Spec boşluğu ↔ ROADMAP tutarlılığı` adımları **her iki mimaride de koştu ve success** — 5.6 bunu *"ölçülemedi, çıkarım"* diye işaretlemişti. ⚠️ Hâlâ kanıtlanmayan tek şey: **başarısız bir adımın işi kırdığı**. ⏳ 5.7'nin koşusu yazım anında bilinmiyor |
| **Kapılar** | ✅ **typecheck 10/10 SOĞUK** (`Cached: 0`) · **build 8/8 SOĞUK** (`Cached: 0`; cache **build'den hemen önce** silinir — günlük #1) · **lint 0** (önbellek silinmiş) · format 0 ⚠️ *(`.md` `.prettierignore`da — bu alt görevin ANA ÜRÜNÜNE hiç bakmadı, SAPMA-024)* · arch **9 kural** · **test 1120/75** (1100/74 idi, **+20** sözlük) · **test:db 301/10** · **gaps 20/3/17/0 ✗** · **i18n:check TEMİZ — 305 dosya** (yeni 133 terimlik Türkçe belgede gömülü görünmez karakter **0**; `.md` boşluğunu kapatan tek kapı bu) · **mutasyon 5/5** (her birinin yerine oturduğu ayrıca doğrulandı) |
| **Kapsam** | fonksiyon **%80,70 (364/451)** — satır %88,88 · ifade %89,00 · dal %89,74; eşik **70**. ⚠️ **5.7'de HİÇ DEĞİŞMEDİ ve bu ölçüldü:** üretilen şey bir `.md` (kod değil) ve `tools/glossary-check/` `.mjs` + `src/` altında değil → `coverage.include` (`tools/*/src/**`) onu görmüyor. `arch-check` · `i18n-check` · `bash-text-guard` · `eslint-local-rules` ile **aynı statü**; bu araca özel bir istisna **açılmadı**. Marj: `364 / 0,70 = 520` → payda en fazla **520**, bugün **451**, **boşluk 69**. Eşik düşürülmedi, dosya dışlanmadı, import testi yazılmadı |
...
| **Kütükler** | Açık sorun **0** · teknik borç **10 satır, ÜÇÜ ÖDENDİ** → **açık 7**: 001·002·004 → Faz 16 · 007 → Faz 12 · 006 → Faz 50 · 🆕 **009** (K5 kapılarının ortak kör noktası — modül düzeyi metin sabitleri) ve 🆕 **010** (`no-hardcoded-path` kablolama kanaryası yok) → **ikisi de Faz 6**. ⚠️ İkisi de *"raporda kalan bir gözlemin sahibi yoktur"* gereği eve taşındı; 010 5.6'ya kadar **yalnızca 5.5'in raporunda** yaşıyordu (ölçüldü: ROADMAP · kütükler · GAPS → 0 eşleşme) · SAPMA **40** (🆕 **040** — `77+57=134` varsayımı, kesişim ölçüldü) · boşluk **20**, açık **17**; **G-13 → Faz 5 + Faz 17** ✓ |
