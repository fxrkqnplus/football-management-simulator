...
| | |
|---|---|
| **Aktif faz / alt görev** | ▶️ **FAZ 6 — Tasarım Sistemi.** **6.0 ✅ · 6.1 ✅ · 6.2 ✅ · 6.3 ✅ · 6.3b ✅ · 6.4-ön ✅ · 6.4 ✅.** Sıradaki: **6.5 — temel bileşenler II** (Dialog, Sheet, Popover, Tooltip, Toast, Badge, Avatar, Progress, Skeleton). §0.5 kontrol noktası **6.5 sonunda**; çizgi **6a = 6.0–6.5 / 6b = 6.6–6.12** |
| **Son tamamlanan** | ✅ **6.4 — shadcn/ui uyarlaması + dokuz temel bileşen.** `packages/ui` ilk kez **JSX** taşıyor; `jsx: "react-jsx"` eklendi (6.3’ün öngörüsü) ve **`types: []` KORUNDU** — ölçüldü: `react-jsx` onu genişletmeyi gerektirmiyor. Listedeki dokuzun dokuzu yazıldı, **fazlası yok** (Popover bir bağımlılık olarak kullanıldı, **dışa aktarılmadı** — o 6.5’in işi) |
| **Varyant adları: SAYI kaynaktan, adlar bu turdan** | Ölçüldü: `spec/05`te bileşen tanımı **yok** (`varyant`ın tek geçtiği yer §7.6’nın **gol kutlamaları**), ROADMAP *"6 varyant"* diyor ama altısını **saymıyor**. Adlar uydurulmadı, **kuralla** türetildi (kapsam maddesi *"shadcn/ui uyarlaması"* diyor, shadcn’in Button’ı tam altı varyant yayınlıyor): `default · destructive · outline · secondary · ghost · link`. ⚠️ **"6" hiçbir yerde ELLE yazılmıyor** — tek kaynak `BUTTON_VARIANTS`, kapsayıcılık `Record<ButtonVariant, string>` ile **tip seviyesinde** |
| **`i18n:check` ikinci kök + ön ek sözleşmesi** | 🆕 `packages/ui/src` taranıyor (6.0 ⑥’nın kararı, **6.3’e atanmıştı**): taranan kaynak **12 → 38 dosya, 1 → 2 kök** · tanımlı **59 → 63** · kullanılan **34 → 38**. ⚠️ **Anahtarların YERİ ölçümle değişti:** merkezî bir modülde toplanınca kapı üçünü birden *"kullanılmayan"* dedi (`t(X.y)` yalnızca **aynı dosyada** çözülüyor); aileyi beyan etmek `i18n-dynamic-keys.ts` kural ①’e göre **yasaktı**, bu yüzden anahtar bileşeninin yanına taşındı ve `i18n-keys.ts` artık **topluyor** |
| **jsdom doldurmaları — 4 yazıldı, 3’ü ölçümle doğrulandı, 1’i ÖLÜ çıktı** | Her biri tek tek söküldü: `scrollIntoView` → **3 test** · işaretçi yakalama üçlüsü → **2 test** · `ResizeObserver` → **4 test** kırıldı. **`matchMedia` → 0 test** kırıldı ve **silindi**. `getBoundingClientRect` **0×0 bırakıldı** — sahte geometri, Slider’ın sürükleme hesabını uydurma sayılar üzerinden geçirirdi. Her doldurmanın **neyi taklit ETMEDİĞİ** `vitest.setup.ts`te yazılı |
| **İki kırmızı, iki ayrı sınıf (D6)** | (a) `filterOptions(…, ‘iSPA’)` boş döndü → **KOD yanlıştı**: `‘İspanya’.toLowerCase()` `i` **+ U+0307 birleşen nokta** üretiyor; ⚠️ dosyanın **kendi yorumu** tersini iddia ediyordu. (b) RadioGroup ok tuşunda seçim değişmedi → **ORTAM yanlıştı**: Radix’in bayrağını `keyup` sıfırlıyor ve `user-event` onu **gecikmesiz** gönderiyor; üç ölçümle ayrıştırıldı, tam basım döngüsü **Faz 17**’ye yazıldı |
| **Mutasyon 3/3 — ikisi test/kapı ZAAFI buldu** | ① varyant silindi → **2 test** · ② etiket anahtarı bozuldu → `pnpm test` **0**, ama **`i18n:check` YAKALADI** (exit 1, dosya+satır; doğrulandı) · ③ Switch topuzunun **konum sınıfları** silindi → **0** kırıldı, yani iddia yazılıydı ama denetlenmiyordu → test **güçlendirildi**, mutasyon tekrarlandı → **1 test**. Yerine oturma **md5**, geri alma **dosya yedeğinden** |
| **Tarih / ilerleme / dallar** | 2026-09-05 · **5 / 50 faz (%10)** · `main` → `develop` (`2ec1548`) → **`feature/faz-06-design-system`** (taban `2ec1548`, **10 commit**) |
| **CI** | ✅ **6.4-ön’ün koşusu `33965051107` YEŞİL** ve **adım listesi okundu**: `Teknik borç ↔ ROADMAP tutarlılığı` **gerçekten koştu**, amd64+arm64. ⚠️ Faz 4’ün kırmızı koşusu (`33419337117`) **hâlâ yeniden denenmedi**, kullanıcının kararı. ⏳ 6.4’ün koşusu yazım anında bilinmiyor |
| **Kapılar** | ✅ typecheck **11/11** · build **8/8 SOĞUK** (`Cached: 0 cached, 8 total`) · lint **0** · format **0** · arch **9 kural** · **test 1380/97** (+65) · **test:db 301/10** · **gaps 20·3·17·0 ✗** · **debt 12·5·7·0 ✗** · **i18n temiz, 38 dosya·2 kök** · **D5: paket `dist`i derlendi VE KOŞTURULDU** (87 dışa aktarım, altı varyant, dört anahtar, Unicode düzeltmesi derlenmiş çıktıda doğrulandı) |
| **Kapsam / paket** | fonksiyon **%83,61 (444/531)** — yükseldi (6.4-ön’de %82,62 / 409-495). Marj `444/0,70 = 634` → boşluk **103**. Eşik düşürülmedi, dosya dışlanmadı. ⚠️ Paket: JS **383.976 → 384.346 (+%0,10)** ama CSS **17.064 → 26.406 (+%54,8)** — §11.6’nın bütçesi **yalnızca ana JS**’i sayıyor, **CSS’in bütçe satırı YOK**; eşik uydurulmadı, karar **6.10**’a yazıldı |
| **İzlenen risk + kalan** | `main.test.tsx` jsdom yıkım yarışı **bu turda GÖRÜLMEDİ** (`Errors` satırı **0**) — ve bu tur onu ilk kez gerçekten sınadı: `packages/ui`ye **19 dosya, 211 test** geldi ve kendi `cleanup()` kancası bağlandı. ⚠️ **Kriter 1 ve 6 `[ ]` KALIYOR** (Storybook envanteri → 6.9, axe → 6.8). Lockfile **682 → 731 (+49)**: user-event **+1** · Radix **+46** · cva/clsx/tailwind-merge **+3** · i18n **+0** · cva **kaldırıldı −1** |
