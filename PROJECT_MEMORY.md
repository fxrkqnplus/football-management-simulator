...
| | |
|---|---|
| **Aktif faz / alt görev** | ▶️ **FAZ 5 — i18n.** ✅ **§0.5 KONTROL NOKTASI KOŞTURULDU (5.5 sonunda): 0,859 gün < 2 gün → BÖLÜNME ATEŞLENMEDİ.** 5b ayrı dal **açılmıyor**; 5.6…5.9 aynı dalda, aynı PR'da. Sıradaki: **5.6 — `tools/i18n-check/` + `pnpm i18n:check` + CI** (aynı commit'te `pnpm gaps:check` de CI'a bağlanır; bugün workflow'larda 0 eşleşme) → kriter 2. 🆕 5.5 ona iki iş bıraktı: **kuralın kör noktası** (modül düzeyi metin sabitleri) ve **görünmez karakter taraması** (ESLint dizeleri atlıyor, `.md`ye hiç bakmıyor) |
| **Son tamamlanan** | ✅ **5.5 — ESLint `no-bare-jsx-text` + kanarya. KRİTER 1 KAPANDI.** Commit: `e7d46ae`. 🆕 **ADI DEĞİŞTİ** (`no-hardcoded-turkish` → `no-bare-jsx-text`, **SAPMA-039**): kural **dile bakamaz**, 5.4 bunu iki yönde ölçmüştü. 🆕 **İKİ UYGULAMA BIRAKILMADI — `tools/i18n-inventory/` EMEKLİ EDİLDİ**; emeklilikten önce anlaşma ölçüldü (**33 = 33**, dosya dosya, uyuşmazlık yok) ve aracın negatif anlamları kuralın `valid[]`ine taşındı. 🆕 **Kanarya gerçek depoda, diske yazmadan**: ESLint API'si gerçek `eslint.config.js`i çözüp var olan bir dosyanın **kimliği** altında lint ediyor. **Mutasyon 4/4** |
| **Tarih / ilerleme / dallar** | 2026-09-03 · **4 / 50 faz (%8)**, Faz 5 **açık, 6/11 alt görev bitti**. `main` → `develop` → **`feature/faz-05-i18n`** (taban `2b5075e`). ✅ PR #5 merge edildi · ✅ `GH007` push blokajı kalktı. **§0.5: `84a6d5f` → `e7d46ae` = 0,859 gün**, eşik 2 → **aşılmadı**, faz **15 commit**. Bölünme çizgisi (5a/5b) **hazır kalıyor ama kullanılmıyor** |
| **CI** | ✅ Dalda **6 koşu, 6 yeşil** (liste sorgusu); 5.4'ünki `33754421329` (`f0e708e`) **success**. ⏳ 5.5'in koşusu yazım anında bilinmiyor — güncel durum bir liste sorgusuyla okunur. ✅ Merge sonrası `develop`: `33674720478` (`2b5075e`) success |
| **Kapılar** | ✅ **typecheck 10/10 SOĞUK** (`Cached: 0`) · **build 8/8 SOĞUK** (`Cached: 0`; cache **build'den hemen önce** silinir — günlük #1) · **lint 0** (önbellek silinmiş, kural bağlandıktan **sonra**) · format 0 ⚠️ *(denetlenen küme TypeScript; `.md` `.prettierignore`da — SAPMA-024)* · arch **9 kural** · **test 1062/72** (1041 idi: **−11** envanter, **+32** kural) · **test:db 301/10** · **gaps 20/3/17/0 ✗** · **mutasyon 4/4** · **görünmez karakter 0** (tarayıcının **öz denetimi 9/9**) |
| **Kapsam** | fonksiyon **%80,70 (364/451)** — satır %88,88 · ifade %88,99 · dal %89,74; eşik **70**. ⚠️ **5.5'te HİÇ DEĞİŞMEDİ ve sebebi ölçüldü:** `coverage.include` `tools/*/src/**` istiyor; `tools/eslint-local-rules/` altında `src/` **yok** ve dosyalar `.js`/`.mjs` — ne yeni kural ne testi paydaya giriyor, emekli edilen araç da girmiyordu. Marj: `364 / 0,70 = 520` → payda en fazla **520**, bugün **451**, **boşluk 69**. Eşik düşürülmedi, dosya dışlanmadı, import testi yazılmadı |
...
| **Kütükler** | Açık sorun **0** · teknik borç **8 satır, ÜÇÜ ÖDENDİ** (003·005 → 5.4, **008 → 4.11**) → **açık 5** (001·002·004 → Faz 16 · 007 → Faz 12 · 006 → Faz 50). ⚠️ *Bir önceki ANLIK DURUM* "7 satır, ikisi kapandı" *diyordu; kaynaktan sayım* **8 / 3** *veriyor — açık sayısı yine 5, ama sayım düzeltildi.* · SAPMA **39** (Faz 5'in payı **dört**: 036·037·038·**039**) · boşluk **20**, açık **17**; **G-13 → Faz 5 + Faz 17** ✓ |
| **Bloke eden var mı?** | Hayır. ⚠️ Üç şey duruyor, üçü de bloke etmiyor: ① `main.test.tsx` jsdom yıkım yarışı **KAPANMIŞ SAYILMIYOR**, gerçek sınavı Faz 6 (aşağıdaki kalıcı bloğa bak) ② **metin hiçbir kabuk argümanından geçmez** — commit mesajı bile `Write` + `git commit -F` ile veriliyor ③ 🆕 **`git checkout` bir geri alma aracı DEĞİL** (günlük #9): commit edilmemiş iş yanındayken mutasyonu geri alırken kullanıldı ve **kendi değişikliğimi sildi**; ölçüm sessizce geçersiz oldu, yakalayan şey mutasyonun yerini doğrulayan ayrı bir adımdı |
