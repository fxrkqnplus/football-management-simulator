...
| | |
|---|---|
| **Aktif faz / alt görev** | **FAZ 4 — Veritabanı Şeması II.** Liste ROADMAP'te (`8203ac1`, K11). **FAZ 4a (4.0 → 4.7) KAPANDI**, **FAZ 4b açık**. ⏸️ **SIRADAKİ: 4.10** — kriter 3'ün ölçümü (`ANALYZE` · A=5.000 / B=sentetik hacim · seçici + seçici olmayan iki terim · mimari etiketi). ⚠️ **4.9 O FAZIN YÖNTEMİNDE BİR DELİK ÖLÇTÜ ve ROADMAP 4.10 kapsamına tabloyla birlikte yazdı:** madde *"`ANALYZE` şart (`reltuples != -1` denetlenir)"* diyordu; ölçüm o denetimin **`ANALYZE` hiç çağrılmadan da yeşil verdiğini** gösterdi — `autovacuum=on`/`naptime=60s` ile 5.000 satırlık INSERT'ten **~15 sn sonra** istatistik kendiliğinden doluyor. 4.10 bunun yerine **`last_analyze`i** denetler (o yalnızca elle `ANALYZE` ile dolar) ve plan ölçümünü seed'den **hemen sonra almaz**: aynı sorgu 15 sn arayla iki farklı plan verebilir ve fark **sessizdir**. 🆕 Zemin hazır: **kriter 3'ün yüklemine uyan satır 27 / 5.000 (%0,54)** — seçici, sıfır değil |
| **Son tamamlanan** | ✅ **4.9 — 5.000 sahte oyuncu seed'i (`people` + `players`). MIGRATION YOK** — zincir **12**, tablo **22**, FK **32**, indeks **6**, sequence **14**; beşi de sabit (D5'te ayrıca ölçüldü). Üreteç `tools/data-cli/src/seed/player-generator.ts`: **saf**, indeks-türevli, **17 numaralı bağımsız akış** (sıralı tüketim değil — araya bir alan eklemek sonraki her alanı kaydırırdı ve 4.10'un **27**'si sessizce başka bir sayıya dönerdi). 🆕 **`SeededRng` REPODA YOK — ölçüldü:** `packages/engine/src/index.ts` gövdesi `export {};`, sınıf yalnızca beş yorumda geçiyor, `DebugPanel.tsx` *"Faz 22'de gelecek"* diyor → `packages/shared`a RNG yazmak tüketicisi olmayan bir mimari değişiklik olurdu (K12). 🆕 **`SEED_REFERENCE_DATE = 2026-07-01` UYDURULMADI:** ROADMAP Faz 16'nın *"1 Temmuz 2026 başlangıç"* takviminden okundu ve gerekçe **koşan bir testle** bağlandı (`readDoc('docs/ROADMAP.md')` — satır kaybolursa test kırılır, bir yorumda dursaydı sessizce bayatlardı). 🆕 **Doğum tarihi çevrimi İKİNCİ KEZ YAZILMADI:** `ageRangeToBirthDateRange(age, age, ref)` ile o yaşın penceresi alınıp gün içinden seçiliyor — ve tutarlılık bir iddia değil **ölçüm**: kriter 3'ün yüklemi hem tarih aralığıyla hem `age` alanıyla sayıldı, **ikisi de 27** ve **aynı satırlar**. `@fms/db` barrel'ına tek satır export eklendi (`TRANSFER_SEARCH_INDEXES` bilerek dışarıda) |
| **⚠️ 4.9'UN EN PAHALI BULGUSU — `reltuples` BİR DURUM DEĞİL, BİR YARIŞ (günlük #35)** | 3.9'un `ANALYZE` dersi *"`reltuples = -1` → planlayıcı kör"* diye bir **DURUM** olarak öğrenilmişti. Buraya kopyalanan iddia testcontainers'ta **geçti**; D5 aynı iddiayı gerçek ve uzun ömürlü bir PostgreSQL'e karşı koşturdu ve **`5000`** döndü. Mekanizma **tahmin edilmedi**, ayrı bir sonda betiğiyle dört anda ölçüldü: `autovacuum=on`/`naptime=60s` · seed'den hemen sonra `-1` · **+15 sn `5000`** ve `last_autoanalyze` damgalı · +60 sn'de `last_analyze` **hâlâ YOK**. Yani değer, ölçümün seed'den **kaç saniye sonra** alındığına bağlı ve testcontainers yarışın **kısa tarafında** bitiyor. **İki sonuç:** ① yazdığım test kırılgandı, iddia **değişmez** olana çevrildi (`last_analyze IS NULL` = *"seed `ANALYZE` çağırmıyor"*) + karşı kontrol (`n_live_tup = 5000`) ② **4.10'un yöntemi değişti** — `reltuples != -1` denetimi `ANALYZE` hiç çağrılmadan da yeşil verir, yani **baktığını göstermeden** onaylar (D3'ün ölçüm tarafındaki biçimi). **Genel kural: zamana bağlı bir değeri iddia eden test, o değerin YARIŞ olup olmadığı ölçülmeden yazılmaz.** ℹ️ D5 ikinci koşusunda `-1` verdi — aynı betik, farklı değer: yarışın en doğrudan kanıtı |
...
| **⚠️ 4.6'NIN EN PAHALI BULGUSU — İKİ KATMANLI SESSİZ KÖRLÜK (günlük #22)** | `0009` zincire ilk kez **bileşik PK** getirdi ve snapshot karşılaştırması onu **hiç görmüyordu**. İki ayrı mekanizma, ikincisi birincisi düzeltilirken bulundu: ① `drizzle-kit` bileşik PK'yi ayrı bir `compositePrimaryKeys` nesnesinde yazıyor ve o durumda **her sütunun `primaryKey`i `false`** — `snapshotToFacts` oraya bakmıyordu ② alan eklendikten sonra test **yine** kırıldı: `compareSnapshotToReal` olguları `new Map(facts.map(f => [f.path, f]))` ile indeksliyor, yani **aynı yolda tekrarlanan olgularda sonuncusu kazanıyor** — ve çok sütunlu PK zaten gerçek şema tarafında da böyle üretiliyordu. Çözüm: PK artık **tek bir olgu** (`primaryKeyColumns`, sıralı virgüllü liste), `columnNames` deseninin aynısı. **Üç birim testiyle kanıtlandı, biri negatif.** ⚠️ **4.3'ün `udtName` kuralının genişlemiş hâli:** o gün *"yeni bir TİP AİLESİ"* yazılmıştı; buradaki yeni şey bir tip değil bir **YAPI TÜRÜ**. Ve ikinci katman daha genel: **bir karşılaştırıcı olguları anahtara göre indeksliyorsa, aynı anahtarın tekrarlanabildiği ilk gün sessizce körelir** |
...
| **4.9'un commit'leri** | **`cd415eb`** (içerik: `player-seed-data.ts` + `player-generator.ts` + iki test dosyası, `seed-sql.ts`in iki yeni bağ listesi, `seed-world.ts` dört ifadeye, iki entegrasyon testi, `@fms/db` barrel export'u, ROADMAP, `SPEC-COVERAGE-GAPS` G-20, `spec/09` §11.5) · **`<bu commit>`** (kayıt: ANLIK DURUM, günlük #34–#36, rapor arşivi) |
...
| **⚠️ CI — 4.8 YEŞİL, AMA SERİ 4.7'DE KIRILDI VE O KOŞU HÂLÂ AÇIK** | ✅ **4.8'in koşusu: `33450312143`** (`5b8b4a5`, `push`, `completed`, **`success`**) — liste sorgusuyla işlendi (`event`/`status`/`conclusion` tek tek okundu). ⚠️ **`33419337117`** (`0459fa5`, 4.7) **`conclusion: failure`** olarak **duruyor ve YENİDEN DENENMEDİ** — dışa dönük bir eylem, kullanıcının kararı. Sebebi kod dışıydı (İmaj/amd64, Docker Hub auth zaman aşımı; aynı commit'te arm64 başarılı) **ama bir iptal değil bir başarısızlık**: *"iptal ne yeşil ne kırmızıdır"* kuralı burada geçerli değil ve `1c93890`'dan beri süren **27 ardışık yeşil** seri orada **sonlandı**. **Bugünkü sayaç: 1 ardışık yeşil** (`33450312143`). Tam sayım 4.11'de işlenir. ⏳ 4.9'un koşusu yazım anında bilinmiyor |
...
| **typecheck / lint / format / build / arch** | ✅ typecheck **10/10**, **8 cache miss** (gerçekten baktı — önbellekten gelen bir onay değil, SAPMA-024) · lint 0 (ilk koşuda **2 hata** — export sırası ve `no-unnecessary-type-conversion`; `eslint-disable` ile susturulmadı, **yeniden yazıldı**) · **format 0** (ilk koşuda **6 dosya**, `prettier --write`) · **build 8/8 SOĞUK** (`.turbo/cache` silindi, `Cached: 0` **doğrulandı**, `pwd` = `/c/fms` önceden denetlendi — #31 beşinci kez ısırmadı, turbo **11,65 s**) · **arch 9 kural** temiz (332 ms). ⚠️ **`format` bu commit'in `.md` dosyalarına BAKMADI** — `.prettierignore` `*.md` taşıyor (SAPMA-024) ve commit dört Markdown dosyası değiştirdi; denetlenen küme **TypeScript** |
...
| **Kapı kapsamı — ÖLÇÜLDÜ, varsayılmadı** | ⚠️ **4.9'da DA asıl doğrulayıcı kapı değildi — YEDİNCİ tekrar.** `typecheck` 5.000 satırlık bir üreteci, iki tabloya yazan **28 sütun bağını** ve `ageRangeToBirthDateRange`in yeni tüketicisini hiçbir şey söylemeden geçirdi; **8 cache miss** olduğu için *"bakacak bir şey yoktu"* bahanesi yok. Doğrulayan şeyler: **gerçek PostgreSQL'in reddi** (`integer out of range` → günlük #34), **katalog** (`pg_stat_all_tables`, `information_schema.columns`, `pg_class.reltuples`), ve **D5** (üç uyuşmazlık buldu, üçü de **benim beklentimde**: sütun sayısı 16 sanılmıştı, katalog **17** dedi — #30/#32'nin sınıfı ama bu kez bir sütun **adında** değil bir **beklenti sayısında**). 🆕 **Ve `pnpm test:db` yine tek başına İKİ bayat envanter yakaladı:** `seed-query-performance.itest.ts`in hacim iddiası ve `world-seed.itest.ts`in ifade sayısı. Hiçbir statik kapı ikisini de göremezdi |
...
| **D5 (4.6 arşiv)** | ✅ **20 / 20.** Vitest yok, testcontainers yok; `fms_d5_46b` adında ayrı bir veritabanı yaratıldı ve zincirin **on adımı** uygulandı. Ölçülenler: 18 tablo · **26 FK** · `player_positions` **5 sütun**, `player_traits` **4**, `player_stats_history` **33** · `xg` gerçekten **`numeric(6,2)`** ve kesinlik korunuyor (`10.45` → `'10.45'`) · **bileşik PK tanımları** katalogdan okundu (`PRIMARY KEY (player_id, "position")` / `(player_id, trait_code)`) · **POZİTİF 1:N** üç mevki kabul · **NEGATİF** bileşik PK tekrarı reddedildi · **NEGATİF** geçersiz mevki (`SW`) ve geçersiz seviye (`world_class`) reddedildi · **POZİTİF** `trait_code` kısıtsız (uydurma kod kabul edildi) · **SET NULL** kulüp silinince satır **durdu**, `club_id` **boşaldı** (2 satır, 0 dolu) · **CASCADE** yarışma silinince istatistik gitti · **CASCADE** kişi silinince mevki/trait/oyuncu gitti · `0009` çevrimi **birebir aynı** (`facts=3570 diff=0`) |
| **install** | ✅ **4.9 DA YENİ BAĞIMLILIK EKLEMEDİ** — `pnpm-lock.yaml` ve `pnpm-workspace.yaml` **diff yok** (`git status` ile doğrulandı). 🆕 **Ve ikinci kez bir bağımlılık kararı REDDEDİLDİ:** 5.000 satırlık deterministik üretim için bir RNG paketi (`seedrandom` vb.) kurulmadı — üreteç `Math.imul` tabanlı 12 satırlık saf bir karıştırıcı (`mix32`, splitmix32 sonlandırıcısı). ℹ️ pnpm 11.24.0 çıkmış; kilit 11.23.0'da, karar verilmedi (`DEPENDENCY-WATCH` Faz 16) |
...
| **test (4.6 arşiv)** | ✅ **822 test / 59 dosya** (781/55'ten **+41 / +4**): üç **envanter** birim testi dosyası (`player_positions` beş seviye + iki tablonun kesişmezliği · `player_stats_history` **33 sütun TAM SIRASIYLA** + nullable envanteri + tip kararları · `player_traits` CHECK yokluğunun **gerekçesi** — `spec/02` ve ROADMAP okunuyor) + **kancanın 13 testi** + snapshot'ın 3 bileşik PK testi. ⚠️ CHECK **yokluğu** yine katalog tarafına bırakıldı (birim testi Postgres'e sormaz — 2.3b) · ✅ **`pnpm test:db` 230 test / 8 dosya** (216'dan **+14**): 1:N pozitif (üç mevki) + bileşik PK tekrarı reddi ×2 · **NEGATİF** geçersiz mevki ve seviye · `trait_code` CHECK yokluğu **iki yönlü** (katalog boş **ve** uydurma kod kabul) · `player_stats_history` CHECK yokluğu · **CASCADE** kişi→üç tablo · **SET NULL** iki yönlü · **CASCADE** yarışma→istatistik · `0009` çevrimi · üç tablo kaybı (bayraksız **reddediliyor** + bayraklı rapor) · `0008`in sıfır katkısı · `fixtureChain` ile **sıfır kayıp** · `⑤ SESSİZ bozuk down (BİLEŞİK PK)` |
| **kapsam** | ⬆️ **İKİNCİ TURDUR YÜKSELİYOR — ve tahmin ÖLÇÜMLE DOĞRULANDI** — satır %86,77 → **%87,99** · ifade %86,76 → **%87,94** · fonksiyon %75,81 → **%78,80** · dal %88,92 → **%88,94**. Seri: 4.6 %75,91 → 4.7 %73,60 → 4.8 %75,81 → **4.9 %78,80**. Toplam 301/397 → **357/453** (pay **+56**, payda **+56**). 🆕 **4.8'in tespiti doğrulandı:** *"şema değişikliği payı sabit bırakıp paydayı büyütüyor; **kapsanabilir kod** payı yükseltiyor"* — 4.9 tamamen kapsanabilir kod getirdi ve iki yeni dosya fonksiyonda **%100**. ⚠️ **Eşik %70 DÜŞÜRÜLMEDİ, dosya dışlanmadı, import testi yazılmadı.** Marj **pay/paydayla** hesaplandı: `357 / 0,70 = 510` → payda en fazla **510**, bugün **453** → boşluk **57 fonksiyon** (33'ten **genişledi**). ℹ️ **KARAR 7 kapandı:** diskteki artefakt 290/397 gösteriyordu, 4.8'in raporu 301/397 diyordu; koşturuldu → **rapor DOĞRUYDU**, artefakt bayattı (raporun kendi tablosu 290'ı bir **ara ölçüm** olarak zaten yazıyordu). Kayıt düzeltmesi gerekmedi |
...
| **Şema durumu** | **22 master tablo** + **6 indeks** + **32 FK** + **14 sequence**. 🆕 **4.9 ŞEMAYA HİÇ DOKUNMADI — migration YAZILMADI** (KARAR 6) ve bu D5'te ayrıca ölçüldü: zincir **12 dosya** (`0000`…`0011`) sabit, `comparedFacts` alt sınırı **4.209**'da sabit, `EXPECTED_TABLE_COUNT` **22** ve `EXPECTED_FOREIGN_KEY_COUNT` **32** dokunulmadan. `world-seed.itest.ts`in *"seed YENİ MIGRATION yazmadı — zincir 12 adımda"* testi **değiştirilmeden yeşil kaldı** — kırılsaydı kapsam taşmış demekti. ℹ️ Aşağıdaki arşiv satırı `0011`in ayrıntısını taşıyor |
...
| **⚠️ G-17'nin 4.5 maruziyeti (arşiv)** | `people.id` ↔ `players.id` karışması **iki nitelik tablosuna** yayıldı: ikisi de `player_id` ile anahtarlı ve testlerin onlara yazması için kimliği **kişiden** çözmek gerekiyor. `fixtures.ts`e bu yüzden ayrı bir `playerIdOfPerson()` yardımcısı yazıldı — **iki kademeli** bir alt sorgu (iç sorgu kişiyi, dış sorgu oyuncuyu veriyor). İç sorgu tek başına kullanılsaydı `player_id` sütununa bir **kişi kimliği** yazılırdı, FK bunu **yakalamazdı** ve testler yeşil kalırdı. Bugünkü savunma yine bir **isimlendirme disiplini**; tip seviyesinde kapanması **Faz 12** (G-17) |
...
| **Teknik borç sayısı** | **8** — **4.9 da hiçbir borç açmadı ve BORÇ-008'i ödemedi** (vadesi **4.11**, ve bu doğru: 4.9 hiç CHECK yazmadı, yani yeni bir kopya doğmadı — **dokuz yerde sabit**). BORÇ-001, 002, 004 (Faz 16) · BORÇ-003, 005 (Faz 5) · **BORÇ-007 (Faz 12)** · BORÇ-006 (Faz 50) · **BORÇ-008 (Faz 4.11)** |
...
| **Boşluk (4.7 arşiv)** | **4.7 de hiçbir boşluk açmadı veya kapatmadı.** İki aday değerlendirildi ve ikisi de **boşluk çıkmadı**: ① *"`managers.spoken_languages` dil kodları için küme yok"* → `spec/01` bir küme yazmıyor ve **Faz 5 (i18n altyapısı) dil kümesinin sahibi**; bugün uydurmak SAPMA-026'nın yasağı, ve hedef faz o işi yapabiliyor (G-18'in dersi: hedefin yapabildiği **doğrulanır**) ② *"`managers.person_id` UNIQUE değil ama iki menajer kaydı anlamsız"* → `staff` ile aynı sınıf ve `spec/01` ikisi için de UNIQUE yazmıyor; bir kısıt uydurmak yerine fixture sözleşmesi olarak bırakıldı (`manager-attributes.ts` başlığında yazılı). ⚠️ **G-16 4.7'de AKTİF OLARAK KORUNDU:** `managers.user_id` yazılmadı ve **yokluğu birim testiyle iddia edildi** — Faz 13 sütunu eklediğinde test kırılacak. Önceki tur (4.6) da hiçbir boşluk açmamıştı |
...
| **Faz 4 kabul kriterleri (4.6 arşiv)** | **3 / 6 — 4.6 hiçbirini kapatmadı ve bu BEKLENEN.** 4.6 kriter **1** ve **6**'ya çalışıyor; ikisi de ancak sonraki alt görevlerde kapanabilir (kriter 1 → seed, **4.9**; kriter 6 → ER + kapanış, **4.11**). `docs/schema/world.md` bu turda **programatik olarak** yeniden üretildi (18 tablo / 26 FK, blok elle düzenlenmedi) ama faz sürerken yine bayatlayacak — kriter 6 hâlâ `[ ]`. **Yapılmayan "yapıldı" yazılmaz.** 🆕 **Kriter 3'ün zemini büyüdü:** `player_positions` bir gün transfer aramasının mevki filtresini besleyecek (Faz 32), ama indeks kararı **4.8**'in ve o alt görev kapsamı `spec/01`'in indeks satırından değil **kriter 3'ün sorgusundan** türetiyor. Önceki tur (4.5) **kriter 4 ve 5'i kapattı:** Kriter 4 (*"nitelik aralıkları CHECK ALMAZ"*) bir **negatif iddiayla** sabitlendi: `pg_constraint` katalogdan okunuyor ve boş liste bekleniyor — *"kısıt eklemeyi unuttuk"* ile *"kısıt bilerek konmadı"* aynı şemayı üretir, ayıran tek şey koşan bir iddiadır. Kriter 5 (*"ilişki değişmezleri"*) **iki ayrı kısıt** olarak yazıldı (birleşik değil — hangi değişmezin ihlal edildiği hata mesajından okunsun), reddi **negatif testle**, sınır dahilliği **karşı örnekle** kanıtlandı, ve D5'te derlenmiş `dist` ile ayrı bir gerçek PG'ye karşı yeniden koşuldu. Kalanlar: kriter 1 ve 3 **4.9/4.10**'a, kriter 6 **4.11**'e. `docs/schema/world.md` bugün güncel (4.5'te yeniden üretildi) ama faz sürerken yine bayatlayacak — kriter 6 hâlâ `[ ]`. Yapılmayan **"yapıldı"** yazılmaz |
