...
| | |
|---|---|
| **Aktif faz / alt görev** | ▶️ **FAZ 5 — i18n. On bir alt görev, 5a/5b bölünme çizgisi, kontrol noktası 5.5 sonunda (ölçümle).** Sıradaki: **5.4 — İHLAL ENVANTERİ + BORÇ-003/005 ödemesi** → kriter 7. ⚠️ **İlk iş envanter, ödeme değil**: kural taslağı `warn` koşturulup ihlal listesi **ölçülür** (kaba tarama yanlış negatif veriyor — `Tekrar dene` hiç Türkçe karakter taşımıyor). 🆕 **i18next'i `main.tsx`e bağlamak da 5.4'ün** — bugün modülün tüketicisi yok |
| **Son tamamlanan** | ✅ **5.3 — i18next + on namespace + dil algılama.** Kurulum: **642 → 647 paket, tam +5**, peer kapısı ötmedi. 🆕 **5.0'ın koşullu tipleme kararı DOĞRULANDI** (iki yönlü: yanlış anahtar `TS2345` ile kırdı, hata mesajı 17 anahtarı saydı; doğru anahtar geçti). 🆕 **Bir mutasyon HİÇBİR ŞEY yakalamadı ve en değerlisi o oldu:** `nonExplicitSupportedLngs` gereksiz çıktı, **silindi**; `supportedLngs`in gerçek işi (`i18n.language` normalleştirmesi) ölçülüp iddia düzeltildi. 🆕 **Çapraz doğrulama 17/17: `Premier League` ve `FA Cup` ISIRDI** → istisna tablosu 3 → **5**, vaka listesi 55 → **57** |
...
| **CI** | ✅ Dalda **4 koşu, 4 yeşil** (liste sorgusu); son: `33701088612` (`60a4964`). ⏳ 5.3'ün koşusu yazım anında bilinmiyor. ✅ Merge sonrası `develop`: `33674720478` (`2b5075e`) success |
| **Kapılar** | ✅ **typecheck 10/10 SOĞUK** · **build 8/8 SOĞUK** (cache ikisi için ayrı ayrı) · lint 0 · format 0 · arch **9 kural** · **test 1029/70** (1013/69 idi) · **test:db 301/10** · **gaps:check 20/3/17/0 ✗** · **mutasyon 3/3** (biri **hiçbir şey yakalamadı** ve bir ayarın silinmesine yol açtı) · **görünmez karakter taraması 0**. ⚠️ **D5 kısmi:** modülün tüketicisi yok, üretim paketinde **görünmüyor** — statik paketleme **kontrol deneyiyle** kanıtlandı, bağlama 5.4'ün |
| **Kapsam** | fonksiyon **%80,70 (364/451)** — satır %88,73 · ifade %88,81 · dal %89,29; eşik **70**. 5.3 paydayı 450 → 451, payı 363 → 364 yaptı (`createI18n` tek yeni fonksiyon; sabitler fonksiyon değil). Marj: `364 / 0,70 = 520` → payda en fazla **520**, bugün **451**, **boşluk 69 fonksiyon**. Eşik düşürülmedi, dosya dışlanmadı, import testi yazılmadı |
