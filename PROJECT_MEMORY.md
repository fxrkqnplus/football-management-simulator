...
| | |
|---|---|
| **Aktif faz / alt görev** | ▶️ **FAZ 5 — i18n. Yedi kriterin BEŞİ kapandı** (1·2·3·4·7); kalan ikisi 5.7 ve 5.8. Sıradaki: **5.7 — `docs/glossary.md`**, çekirdek `CLAUDE.md` §14'ün **77 terimi** + `docs/spec/02/03/04/07`den **toplanan ≥43** terim (uydurulmaz — SAPMA-026), sayı bir testle iddia edilir → kriter 5. ⚠️ Otorite yönü dosya başlığına yazılır (§14 kendi terimleri için **bağlayıcı**), ve senkron testi **çift** iddia eder: varlık **ve** Türkçe karşılığın birebir aynılığı |
| **Son tamamlanan** | ✅ **5.6 — `i18n:check` + CI. KRİTER 2 KAPANDI** (iki yarısı da). Commit: `8b5c52f`. 🆕 **Dört denetim** (eksik · kullanılmayan · boş çeviri · görünmez karakter), hepsi iki yönlü. 🆕 **KAPI KENDİ YAZARINI YAKALADI:** ilk koşu 8 "kullanılmayan" bildirdi, **dördü gerçekti** (`errors:status.*` — 5.4'te yazılan yedek ailenin **sıfır tüketicisi** var), dördü aracın kusuruydu (anahtar **prop** olarak taşınıyor). 🆕 **`gaps:check` de CI'a bağlandı** — 4.11'den beri hiçbir workflow'da yoktu. 🆕 **Kör nokta KAPATILMADI**, ölçümle → BORÇ-009. **Mutasyon 7/7** |
| **Tarih / ilerleme / dallar** | 2026-09-03 · **4 / 50 faz (%8)**, Faz 5 **açık, 7/11 alt görev bitti**. `main` → `develop` → **`feature/faz-05-i18n`** (taban `2b5075e`). ✅ PR #5 merge edildi · ✅ `GH007` push blokajı kalktı. **§0.5 5.5'te koşturuldu: 0,859 gün < 2 → bölünme ATEŞLENMEDİ**; 5b ayrı dal **açılmıyor**, 5.7…5.9 aynı dalda ve aynı PR'da |
| **CI** | ✅ Dalda **7 koşu, 7 yeşil** (liste sorgusu); 5.5'inki `33781430328` (`71198e6`) **success**. ⏳ 5.6'nın koşusu yazım anında bilinmiyor — güncel durum bir liste sorgusuyla okunur. 🆕 **`ci.yml`in kalite kapıları artık DOKUZ:** typecheck · lint · format · test:coverage · build · arch · **i18n** · **gaps** (+ ayrı `integration` işi) |
| **Kapılar** | ✅ **typecheck 10/10 SOĞUK** (`Cached: 0`) · **build 8/8 SOĞUK** (`Cached: 0`; cache **build'den hemen önce** silinir — günlük #1) · **lint 0** (önbellek silinmiş) · format 0 ⚠️ *(denetlenen küme TypeScript; `.md` `.prettierignore`da — SAPMA-024)* · arch **9 kural** · **test 1100/74** (1062/72 idi: **+34** araç, **+4** beyan) · **test:db 301/10** · **gaps 20/3/17/0 ✗** · 🆕 **i18n:check TEMİZ** — 59 tanımlı · 34 kullanılan · 3 dinamik ön ek · 12 kaynak · **301 dosya** görünmez tarama · **mutasyon 7/7** (her birinin yerine oturduğu ayrıca doğrulandı) |
| **Kapsam** | fonksiyon **%80,70 (364/451)** — satır %88,88 · ifade **%89,00** · dal %89,74; eşik **70**. ⚠️ **Fonksiyon paydası 5.6'da da DEĞİŞMEDİ ve sebebi ölçüldü:** `coverage.include` `tools/*/src/**` istiyor; `tools/i18n-check/` `.mjs` ve `src/` altında **değil** → paydada yok (tıpkı `arch-check` ve `bash-text-guard` gibi — bu araca özel bir istisna **açılmadı**). Beyan modülü `apps/*/src/**` olduğu için **girdi**: ifade 1445 → **1446**, ve o +1 **kapsandı**. Marj: `364 / 0,70 = 520` → payda en fazla **520**, bugün **451**, **boşluk 69**. Eşik düşürülmedi, dosya dışlanmadı, import testi yazılmadı |
...
| **Kütükler** | Açık sorun **0** · teknik borç **10 satır, ÜÇÜ ÖDENDİ** → **açık 7**: 001·002·004 → Faz 16 · 007 → Faz 12 · 006 → Faz 50 · 🆕 **009** (K5 kapılarının ortak kör noktası — modül düzeyi metin sabitleri) ve 🆕 **010** (`no-hardcoded-path` kablolama kanaryası yok) → **ikisi de Faz 6**. ⚠️ İkisi de *"raporda kalan bir gözlemin sahibi yoktur"* gereği eve taşındı; 010 5.6'ya kadar **yalnızca 5.5'in raporunda** yaşıyordu (ölçüldü: ROADMAP · kütükler · GAPS → 0 eşleşme) · SAPMA **39** (5.6 yeni SAPMA açmadı) · boşluk **20**, açık **17**; **G-13 → Faz 5 + Faz 17** ✓ |
