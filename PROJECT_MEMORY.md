...
| | |
|---|---|
| **Aktif faz / alt görev** | ▶️ **FAZ 5 — i18n. On bir alt görev, 5a/5b bölünme çizgisi, kontrol noktası 5.5 sonunda (ölçümle).** Sıradaki: **5.3 — i18next + on namespace + tarayıcı dil algılama** (`apps/web/src/app/i18n.ts` · `locales/tr/*.json`). ⚠️ **Paketler BURADA kurulur** (26.4.1 / 17.0.13 / 8.2.1) ve **5.0'ın tipleme kararı BURADA kontrol deneyiyle doğrulanır**: kasten yanlış bir anahtar `typecheck`i **kırmalı** |
| **Son tamamlanan** | ✅ **5.2 — `Intl` biçimlendiriciler. KRİTER 4 KAPANDI** (iki yarısı da: tarih **ve** para). Commit: `<bu commit'ten önceki>`. **Üç tuzak ölçüldü:** ① ayırıcı **U+00A0** (bölünmez), normal boşlukla yazılmış sözleşme sessizce yanlış olurdu ② `compactDisplay:'long'` para ile **çalışmıyor** → son eki biz küçültüyoruz ③ makinenin varsayılan zaman dilimi `Europe/Istanbul` ve `22:30Z` sınırında **gün değiştiriyor** → `DEFAULT_TIME_ZONE='UTC'`. **İki katman ayrı iddia edildi** (ICU'nun parçaları · bizim saf son işlemimiz). **Mutasyon 3/3 · D5 22/22** |
...
| **CI** | ✅ Dalda **3 koşu, 3 yeşil** (liste sorgusu): `33680116874` · `33681281420` · `33684397113` (`2c622c1`). ⏳ 5.2'nin koşusu yazım anında bilinmiyor. ✅ Merge sonrası `develop`: `33674720478` (`2b5075e`) success |
| **Kapılar** | ✅ **typecheck 10/10 SOĞUK** · **build 8/8 SOĞUK** (cache **ikisi için ayrı ayrı** silindi, ikisi de `Cached: 0`) · lint 0 · format 0 · arch **9 kural** temiz · **test 1013/69** (995/68 idi) · **test:db 301/10** · **gaps:check 20/3/17/0 ✗** · **D5 22/22** · **mutasyon 3/3**. 🆕 **Depo geneli gömülü `U+00A0` taraması: 0 dosya** (5.2'de üç dosyada bulunup düzeltildi) |
| **Kapsam** | fonksiyon **%80,66 (363/450)** — satır %88,66 · ifade %88,74 · dal %89,29; eşik `vitest.config.ts`ten **70**. 🆕 **5.2 paydayı 445 → 450, payı 358 → 363 yaptı** — beş yeni fonksiyonun **beşi de kapsandı**, oran yine **yükseldi**. Marj: `363 / 0,70 = 518` → payda en fazla **518**, bugün **450**, **boşluk 68 fonksiyon**. Eşik düşürülmedi, dosya dışlanmadı, import testi yazılmadı |
