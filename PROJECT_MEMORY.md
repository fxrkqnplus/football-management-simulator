...
| | |
|---|---|
| **Aktif faz / alt görev** | ▶️ **FAZ 6 — Tasarım Sistemi.** **6.0–6.4 ✅ · 6.5 ✅.** Sıradaki: **6.6 — on alan-özel bileşen**. ⚠️ **§0.5 kontrol noktası KOŞTU ve İKİ TANIM AYRIŞTI — karar bekliyor** (aşağıda). Çizgi: **6a = 6.0–6.5 tamamlandı** / **6b = 6.6–6.12** |
| **Son tamamlanan** | ✅ **6.5 — temel bileşenler II: Dialog · Sheet · Popover · Tooltip · Toast · Badge · Avatar · Progress · Skeleton.** Dokuzu da listeden, **fazlası yok**. `Popover` 6.4’te kurulmuş ama dışa aktarılmamıştı; yayınlandı ve **Combobox değişmedi** — kanıt bayt bayt: `dist` silinip `--force` ile (0 cached) derlendi, `combobox.js` md5 **`e98697f7…` → `e98697f7…`**, testleri **12/12** |
| **🆕 `UI_KEYS` nöbetçisi — bir İDDİANIN çürütülmesiyle** | 6.4 `i18n-keys.ts`in başlığına *"iki liste yok, ayrışmaları mümkün değil"* yazmıştı. **Ölçüldü, yanlış çıktı:** `ALL_UI_KEYS` türev ama `UI_KEYS` **elle tutulan kayıt defteri**, ve 6.4’ün yedi vakasının yedisi de **diske hiç bakmıyordu**. Nöbetçi **bileşenlerden ÖNCE** yazıldı (`import.meta.glob`, `node:fs` **değil** — `types: []` delinmedi) ve ilk gerçek unutma vakasında **gerçek depoda öttü**: `DIALOG_KEYS` diskte, kayıtta yok → *"expected [‘dialog’] to deeply equal []"*. İki yönlü + referans eşitliği |
| **⚠️ Nöbetçi KENDİ ölçüm aracını bozdu (D2)** | `‘./*.tsx’` deseni `*.test.tsx`i de içe aktardı; `eager: true` onların `describe` bloklarını nöbetçiye kaydetti ve paketin **bütün testleri iki kez** koştu. Belirti **sessiz**: kırmızı yok, sayı 11→67. Yanlışı gösteren şey **taranan modül sayısı** oldu (18 = 9 bileşen + 9 test). Desen `[‘./*.tsx’, ‘!./*.test.tsx’]` oldu ve **sınır ayrıca iddia edildi** |
| **⚠️ K8: `stdout` idiomu bu pakete TAŞINAMIYOR** | `inventory-guards.test.mjs` kapsamını **basıyor**; `scripts/`+`tools/` K8’den **muaf**, `packages/ui` **değil** (üstelik `types: []` yüzünden `process` tipi de yok). Üç testte basma **kaldırıldı**, yerine **iddia** kondu (beklenen modüller **adıyla**, denetlenen çift **sayısı**). **Bir idiom taşınabilir değildir:** amaç aynı, araç pakete göre değişir — kapıda `stdout`, testte `expect` |
| **Anlamsal renkler — 6.2’nin sahipsiz maddesi kapandı** | 6.2 dört token’ı denetlemeden bırakıp sahibini *"**6.4** (Badge, Toast)"* yazmıştı; **Badge ve Toast 6.5’te** — sahip doğru, **numara bayat**, ve 6.4 kararı adıyla kapatmadı. Karar `tokens/semantic-tone.ts`te: zemin **dolgu** (**GEREKÇE**, 6.4’ün `Button.destructive` emsali), ön plan **hesaplanmış** (**ÖLÇÜM**). `contrast-audit`in *"HENÜZ denetlenmiyor"* vakası **gerçek denetime** çevrildi: info **7,01** · success **6,12** · warning **9,47** · danger **4,94** — dördü de AA |
| **Sheet kararı + yeni doldurma YOK** | **Sheet ayrı bileşen** (GEREKÇE, ölçüm değil): ROADMAP listesi bir **envanter** ve ikisi orada **ayrı madde**; paylaşılan yarı gizlenmiyor — aynı Radix ilkeli, tek eksende fark (`side`). ⚠️ **Yeni jsdom doldurması GEREKMEDİ** — ölçüldü: hiçbir bileşen JS’ten `matchMedia` okumuyor, zamanlayıcılar **prop** üzerinden sıfırlandı (`delayDuration={0}`, `duration={Infinity}`). Böylece `theme-mode.test.ts`:83’ün **kontrol testi** korundu |
| **Mutasyon 3/3 — ilk turda ikisi geçersizdi** | ① `UI_KEYS` kaydı silindi → **4 test** · ② portal söküldü → **0** (mutasyon ölçtüğüm yola dokunmuyordu; `screen` bütün `body`ye bakıyor) → Dialog testine **portal iddiası** eklendi → **1 test** · ③ `aria-valuenow` mutasyonu **ayırt edici değildi** (belirsiz vakada iki yol aynı sonucu veriyor) → yerine `Skeleton`ın `aria-hidden`ı → **1 test**. Yerine oturma **md5**, geri alma **dosya yedeğinden** |
| **Tarih / ilerleme / dallar** | 2026-09-10 · **5 / 50 faz (%10)** · `main` → `develop` (`2ec1548`) → **`feature/faz-06-design-system`** (taban `2ec1548`, **12 commit**) |
| **CI + Kapılar** | ✅ **6.4’ün koşusu `33981376834` YEŞİL** ve **adım listesi okundu**: altı işin altısı, **İmaj işleri dâhil** (amd64+arm64) — yani 6.4’ün *"imaj koşturulmadı"* notu CI’da kapandı. ⚠️ Faz 4’ün kırmızı koşusu (`33419337117`) **hâlâ yeniden denenmedi**. Kapılar: typecheck **11/11** · build **8/8 SOĞUK** (`Cached: 0 cached, 8 total`) · lint **0** · format **0** · arch **9 kural** · **test 1443/107** (+63) · **test:db 301/10** · **gaps 20·3·17·0✗** · **debt 12·5·7·0✗** · **i18n 48 dosya·2 kök** · `main.test.tsx` yarışı **bu turda GÖRÜLMEDİ** (`Errors` **0**) |
| **Kapsam / paket — ve CSS’in ne ölçtüğü** | fonksiyon **%84,09 (460/547)** — yükseldi (6.4’te %83,61 / 444-531). Marj `460/0,70 = 657` → boşluk **110**. Eşik düşürülmedi, dosya dışlanmadı. ⚠️ **BULGU:** `@source` satırı geçici sökülüp yeniden derlendi → CSS **30.442 → 14.952**. Yani üretim CSS’inin **15.490 baytı (%50,9)** `packages/ui`den geliyor **ve `apps/web` o bileşenlerin hiçbirini render etmiyor** (0 eşleşme, ölçüldü). **CSS bir bileşen YAZILDIĞINDA büyüyor, KULLANILDIĞINDA değil**; JS kontrol deneyi (**+%0,16**, ağaç sarsma çalışıyor). Eşik **uydurulmadı**, not **6.10’un kapsamına** yazıldı |
| **⚠️ §0.5 — KARAR BEKLİYOR** | İlk commit `3073deb` (2026-09-04 18:11). **İki tanım ayrışıyor:** ilk→son commit **1,099 gün** (iki eşik de aşılmadı) · ilk→şimdi **6,060 gün** (ikisi de aşıldı). Fark bir ölçüm hatası değil: 6.4’ün son commit’inden bugüne **beş gün hiç commit yok**. Emsal de ikiye bölünüyor — Faz 3 kaydı **ilk→son**, Faz 4’ün 4.7 kontrol noktası **ilk→şimdi** (ama orada iş kesintisizdi, ilk kez bugün ayrışıyorlar). **Bölünme kararı KULLANICININ (K13)** — tanımı kendim seçmek kuralı sessizce esnetmek olurdu |
