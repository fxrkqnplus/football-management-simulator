...
| | |
|---|---|
| **Aktif faz / alt görev** | **FAZ 4 — Veritabanı Şeması II.** Liste ROADMAP'te (`8203ac1`, K11). **4.0 · 4.0b · 4.1 · 4.2 · 4.3 · 4.4 · 4.5 · 4.6 kapandı.** ⏸️ **SIRADAKİ: 4.7** — `staff` + `staff_attributes` + `managers` + `manager_attributes` (**`0010`** — numara SAPMA-034'te düzeltildi). `staff.role` **CHECK** (12 değer, `spec/01`'den SAYILACAK). ⚠️ **4.7 aynı zamanda §0.5 KONTROL NOKTASI:** geçen gün sayısı ölçülüp raporlanır; iki günü aştıysa 4.8–4.11 **Faz 4b** olarak ayrılır. ⚠️ `0008`in sınırı hâlâ miras: `migrateDown` çağıran her yeni test `narrowRefereePersonTypesForDown()` çağırmak zorunda (günlük #17) |
| **Son tamamlanan** | ✅ **4.6 — `player_positions` + `player_traits` + `player_stats_history` (`0009`) + `bash-text-guard` kancası (ayrı commit, kod işinden ÖNCE).** Envanter **15 → 18 tablo**, FK **21 → 26**. **Fazın ilk 1:N tabloları ve ilk BİLEŞİK PK'leri.** FK kuralı **KOŞTURULDU** → **5/5** (CASCADE ×4 + SET NULL), üretilen SQL'in `ON DELETE` satırlarıyla tek tek karşılaştırıldı. 🆕 **§3.1.2 ②'nin ayracı AYNI ALT GÖREVDE İKİ FARKLI CEVAP ÜRETTİ:** `position` (12 kod, `players.ts`ten **ithal** — iki kopya ayrışamasın) ve `level` (5 derece) **CHECK aldı**; `trait_code` **almadı**, çünkü küme `spec/02`'de **hiç tanımlı değil** (0 eşleşme, ölçüldü) ve ROADMAP *"~30"* diyor — sayılamayan bir küme kapalı iddia edilemez. 🆕 **`competition_id`in CASCADE alması sezgiye aykırı** ve 4.3'ün 4.4 için yaptığı yanlış tahminin sınıfı: kural ② **hedefin değil KAYNAĞIN** sınıfına bakıyor |
...
| **4.6'nın commit'leri** | **`4a1c40f`** (kanca: `tools/bash-text-guard` + `.claude/settings.json` + vitest projesi) · **`6a08776`** (içerik: üç tablo, `0009`, snapshot düzeltmesi, testler, ER, SAPMA-034'ün ROADMAP düzeltmesi) · **`<bu commit>`** (kayıt: ANLIK DURUM, günlük #20–#25, SAPMA-034, rapor arşivi). ⚠️ **4.5'in protokol ihlali TEKRARLANMADI:** ANLIK DURUM'u yazan commit alt görevin **son** commit'i ve rapor hash'leri de onunla birlikte yazıldı |
| **Tarih** | 2026-08-30 |
